#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Test deep insert with proper UI flow"""
import asyncio
import json
import time
import random
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Step 6: Fill in version name - look for the NEW ROW input (at the bottom)
        # The new row input has placeholder "请输入" (no specific name)
        all_inputs = await page.query_selector_all('input')
        print(f"\nAll visible inputs after add:")
        ...
        # The new row input is the LAST one in the version list - it should be the bottom of the table
        # Try inputs from the bottom that are empty and editable
        for inp in reversed(all_inputs):
            try:
                if await inp.is_visible():
                    ph = await inp.get_attribute('placeholder') or ''
                    val = await inp.input_value()
                    if val == '' and (ph == '请输入' or 'input' in ph.lower() or ph == ''):
                        await inp.fill(version_name)
                        print(f"\n  [OK] Filled input with placeholder='{ph}' value='{version_name}'")
                        break
            except:
                pass

        # Press Tab or click elsewhere to commit the edit
        await page.keyboard.press('Tab')
        await page.wait_for_timeout(1000)

        # Take screenshot before save
        await page.screenshot(path='/tmp/screenshot_before_save.png', full_page=True)
        ...
        await page.screenshot(path='/tmp/screenshot_after_save_v3.png', full_page=True)
