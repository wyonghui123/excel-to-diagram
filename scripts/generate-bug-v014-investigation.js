#!/usr/bin/env node
/**
 * T21: BUG-V014 调查 (no-op 记录) + cascade export dim scope 多 role 聚合验证
 *
 * 用户报告: TEST333 导出 product cascade 只导 1 个产品 17 个 version
 * 排查结论: 该现象是 BUG-V013 修复前的旧状态
 *          BUG-V013 修复 list 路径后, 实际 cascade export 走 query_service.search,
 *          而 query_service._try_apply_dimension_scope 已经聚合多 role,
 *          owner role (5970) 已覆盖所有 owner 看到的数据
 *
 * 验证: TEST333 有 2 roles:
 *   - 5433 (limited): dim scope 限定 product_id=475 (1 个 product)
 *   - 5970 (owner): dim scope 覆盖 460 个 domain (所有 owner 看到)
 * 多 role 聚合后, owner 数据全部能查到
 *
 * 实际导出: 5 sheets / 610 rows
 *   - 业务关系 54
 *   - 领域 460 (从所有 owner 看到的 product 17 version)
 *   - 子领域 14
 *   - 服务模块 29
 *   - 业务对象 53
 */
...
const OUTPUT = path.join(ROOT, 'e2e/business-flow/bug-v014-investigation.spec.js');

function main() {
  console.log('=== T21: BUG-V014 调查 (no-op) + 多 role 聚合验证 ===\n');
