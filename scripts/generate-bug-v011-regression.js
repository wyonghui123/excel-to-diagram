#!/usr/bin/env node
/**
 * T18: BUG-V011 回归 test 生成器
 *
 * 修复 cascade_delete 链路 4 个层级都不读 schema 的 bug
 *
 * 案例: TEST333 删 product SDLKFJL (含 1 个 version)
 *       旧代码报"存在 1 个子元素"或"FK 约束失败"
 *       实际 product.yaml associations[].cascade_delete=true 应级联
 *
 * 修复链:
 *   1. hierarchy_validation_interceptor._validate_delete: 读 associations
 *   2. action_executor._do_delete: validate_delete + _check_deletability
 *   3. manage_service.delete: deletability.condition
 *   4. cascade_interceptor._cascade_delete_children: 实际执行级联
 *      (还支持 dict 和 AssociationDefinition 两种格式)
 */
...
const OUTPUT = path.join(ROOT, 'e2e/business-flow/bug-v011-cascade-delete.spec.js');

function main() {
  console.log('=== T18: BUG-V011 回归 test 生成器 ===\n');
