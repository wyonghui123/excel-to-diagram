#!/usr/bin/env node
/**
 * T17: BUG-V010 回归 test 生成器
 *
 * 修复 _is_owner 在 V1.1.4 owner refactor 后的兼容性问题
 *
 * 案例: TEST333 是 product SDLKFJL 的 owner
 *       删除该 product 下的 version 时, 旧 _is_owner 查询失败
 *       异常被吞, 返回 False, 触发权限拒绝
 *
 * 模型源:
 *   - meta/services/condition_permission_service.py:557-574 (_is_owner 修复)
 *   - meta/schemas/product.yaml:23-31 (V1.1.4 owner 统一)
 *   - meta/schemas/version.yaml (V1.1.4 删 owner_id)
 */
...
const OUTPUT = path.join(ROOT, 'e2e/business-flow/bug-v010-owner-trace.spec.js');

function main() {
  console.log('=== T17: BUG-V010 回归 test 生成器 ===\n');
