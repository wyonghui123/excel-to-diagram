...
class PermissionDimensionEngine:
    ...
    def _load_resource_table_map(self) -> Dict[str, str]:
        ...
        """
        table_map: Dict[str, str] = dict(RESOURCE_TABLE_MAP)

        # 来源 2: hierarchies.yaml
        ...
        if os.path.exists(hierarchies_path):
            try:
                ...
                for h in hierarchies_data.get('hierarchies', []):
        ...
        if os.path.exists(mapping_path):
            try:
                ...
                for dim in mapping_data.get('dimension_object_mappings', []):
                    code = dim.get('dimension_code')
                    value_table = dim.get('value_table')
    ...
    def _generate_loaded_schema_report(self) -> None:
        ...
        for fname in SCHEMA_REPORT_FILES:
            path = os.path.join(self._schema_dir, fname)
            entry = {
                'path': path,
                'loaded': False,
                'top_level_keys': [],
            ...
            if os.path.exists(path):
                try:
                    ...
                    logger.warning(f"生成 schema 基线报告失败 [{fname}]: {e}")
            report['schemas'][fname] = entry

        report_path = os.path.join(os.path.dirname(__file__), 'loaded_schema_report.json')
        try:
    ...
    def _read_dimension_object_mapping(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not mapping_data:
                return result
            result['dimension_object_mappings'] = mapping_data.get('dimension_object_mappings', [])
            result['dimension_priority'] = mapping_data.get('dimension_priority', {})
            result['combination_policy'] = mapping_data.get('combination_policy', {})
        except Exception as e:
            logger.warning(f"dimension_object_mapping.yaml 解析失败，使用空映射 fallback: {e}")
        return result
    ...
    def get_combination_policy(self) -> Dict[str, Any]:
        ...
        """
        metadata = self._load_dimension_metadata()
        return metadata.get('combination_policy', {})
    
    def _apply_generic_dimension_auto_expand(
        self,
        dimension_code: str,
        value_ids: List[int],
    ) -> List[int]:
    ...
    def _get_all_children(self, resource_type: str, resource_id: int) -> List[Dict[str, Any]]:
        ...
        for child_type in child_types:
            child_table = self._resource_table_map.get(child_type)
            if not child_table:
                continue
    ...
    def _get_all_parents(self, resource_type: str, resource_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            parent_id = row[0]
            parent_table = self._resource_table_map.get(parent_type)
            if not parent_table:
                return parents
    ...
    def invalidate_cache(self, role_id: Optional[int] = None):
        ...
        Args:
            permission_set_id: 角色ID，如果为None则清空所有缓存
        """
        import asyncio
        try:
            loop = asyncio.get_event_loop()
