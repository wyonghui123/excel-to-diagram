...
class OrgService:
    ...
    def add_member(self, org_id: int, user_id: int, is_manager: bool = False) -> bool:
        ...
        [v088 2026-09-15 soft-delete Spec 19 M4] is_manager 入参已废弃:
          - P1 (历史): 写库 org_members.is_manager=1
          - P2 (v088 起): 拒绝入参, 返回 False + deprecation 警告
          - P3 (2027-Q1 v09x): DROP COLUMN
        受托管理员授权已切换为资源矩阵 org 行级规则 (OrgAdminScopeService).
        """
        if is_manager:
            import logging
            logging.getLogger(__name__).warning(
                '[v088 deprecated] add_member(is_manager=True) 拒绝; '
                '请改用资源矩阵 org 行级规则配置委托'
            )
            return False
        try:
            self.ds.execute(
    ...
    def is_org_manager(self, org_id: int, user_id: int) -> bool:
        """[v088 2026-09-15 DEPRECATED] 检查用户是否为组管理员 — 仅返回 False (读路径已废弃).

    历史: 读取 org_members.is_manager.
    P2 (v088 起): 该方法已无消费方 (grep 0 引用, OrgAdminGuardInterceptor 不读).
      保留仅为 API 兼容; 永远返回 False 触发上游走委托主源 (OrgAdminScopeService).
    P3 (2027-Q1 v09x): 删除该方法.
    ...
    def get_managed_orgs(self, user_id: int) -> List[int]:
        ...
        [v088 2026-09-15 Spec 19 M4 软删完成] 委托模型主来源切换完成：
        主来源 = 资源矩阵 org 行级规则（OrgAdminScopeService 动态解析）。
        v084 过渡期保留的 is_manager / manager_id fallback 段已删除（5 行）。
        受托管理员授权完全走「权限集 + 资源矩阵 + org 行级规则」一套载体。
        """
        managed = set()

        # 1. [主来源] 资源矩阵 org 行级规则（动态子树展开）
        try:
            from meta.services.org_admin_scope_service import OrgAdminScopeService
            scope = OrgAdminScopeService(self.ds).get_manageable_org_ids(user_id)
            if scope is None:
                # 通配：全组织（该用户权限集含 org '*' 规则）
                rows = self.ds.execute("SELECT id FROM orgs").fetchall()
                return [r[0] for r in rows]
            managed.update(scope or set())
        except Exception:
            logger.warning('get_managed_orgs: scope service failed', exc_info=True)

        # 2. [v088 软删完成] is_manager / manager_id 读路径已删除
        #   历史 (v084 M2 过渡期): 并集保留遗留数据 (M3 移除)
        #   当前 (v088 起): 完全依赖 OrgAdminScopeService 主源
        #   seed 遗留 (admin → org 1 系统管理员) 不再读出 (admin 走 is_admin() 全局放行)
    ...
    def _resolve_anchor_names(self, resource_type: str, codes: List[str]) -> Dict[str, str]:
        """[Spec 20 v6 2026-09-12] 把业务键锚点 `code IN ('a','b')` 的 code 列表
        解析成 {code: name}（维度表反查示例名）。

        与 _resolve_resource_names 同表映射（PREVIEW_RESOURCE_TABLE_MAP）；
        查不到/非维度 BO 返回 {}，调用方回落裸 code（code 本身业务可读）。
        """
        table = PREVIEW_RESOURCE_TABLE_MAP.get(resource_type)
        if not table:
            return {}
        code_list = [str(x).strip().strip("'\"") for x in codes if str(x).strip()]
        if not code_list:
            return {}
    ...
    def _humanize_condition(self, resource_type: str, condition: str) -> str:
        ...
        # [Spec 20 v6 2026-09-12] code IN ('a','b') → 业务键锚点（动态规则）中文化:
        #   键名同显「code（name）」, 与前端 tag format 一致; 查不到示例名回落裸 code。
        #   [FIX v6b] 仅处理 IN/NOT IN (枚举语义); LIKE 是模糊匹配 (值带 % 通配),
        #   不能译成「仅以下业务键」, 回落默认路径原样展示。
        if c.lower().startswith('code '):
            import re
            m = re.match(r"^code\s+(NOT\s+IN|IN)\s*\(([^)]*)\)\s*$", c, re.IGNORECASE)
            if m:
                op, raw = m.group(1).upper(), m.group(2)
                codes = [x.strip().strip("'\"") for x in raw.split(',') if x.strip()]
                if codes:
                    name_map = self._resolve_anchor_names(resource_type, codes)
                    items = [f"{k}（{name_map[k]}）" if name_map.get(k) else k for k in codes]
                    prefix = '排除以下业务键' if op.startswith('NOT') else '仅以下业务键'
    ...
    # [2026-09-05 权限预览] 权限集"数据范围"的第二种来源：维度范围（dimension scopes）
    # 现状：get_permission_preview / _build_ps_data_scope 只读 data_permission_rules + 旧表
    #   permission_set_data_permissions；而前端「权限配置」面板把 scope 同时落在
    #   permission_set_dimension_scopes（product/version/domain/sub_domain 等维度，canonical ID-form）。
    #   二者缺失 → staging 上三个权限集（财资管理子领域编辑/大财务架构数据查看/YonBIP产品查看）
    #   在「权限预览」矩阵全部显示"未配置"。此方法读取维度范围并归一为 id 列表。
    def _load_ps_dimension_scopes(self, ps_id: int) -> List[Dict[str, Any]]:
        try:
            cursor = self.ds.execute(
                """SELECT dimension_code, dimension_values, scope_mode, inherit_children
        ...
        for row in rows:
            ...
            if not code:
                continue
            scope_mode = row.get('scope_mode') or 'include'
            import json as _json
            raw = row.get('dimension_values') or '[]'
            try:
                values = _json.loads(raw) if isinstance(raw, str) else (raw or [])
            except Exception:
                values = []
            ids: List[int] = []
            wildcard = False
            for v in values or []:
                if isinstance(v, dict):
                    ...
                    ids.append(int(v))
            # scope_mode=all / ['*'] = 不限制，等于没有范围 → 跳过（与前端"不限制"判定一致）
            if scope_mode == 'all' or wildcard or not ids:
                continue
            out.append({
                'resource_type': code,
                'values': ids,
                'exclude': scope_mode == 'exclude',
                'inherit_children': bool(row.get('inherit_children')),
            })
        return out

    # [2026-09-05 权限预览] 矩阵行 label（scope-only 合成行无 _build_role_matrices 来源时的兜底）
    _RESOURCE_LABELS = {
        'product': '产品',
        'version': '版本',
        'domain': '领域',
    ...
    def _resource_label(self, resource_type: str) -> str:
        return self._RESOURCE_LABELS.get(resource_type) or self._humanize_resource_type(resource_type) or resource_type

    def _build_dim_scope_display(self, resource_type: str, values: List[int], exclude: bool) -> tuple:
        """维度范围 → (技术表达式, 中文展示, 值名列表)。语义与权限配置面板 loadScopeMatrix 一致。"""
        names = self._resolve_resource_names(resource_type, [str(v) for v in values]) or []
        expr = f"id {'NOT IN' if exclude else 'IN'} ({','.join(str(v) for v in values)})"
        if names:
            display = ('排除: ' if exclude else '包含: ') + '、'.join(names)
        else:
            display = ('排除: ' if exclude else '包含: ') + ','.join(str(v) for v in values)
        return expr, display, names
            ...
            for ps in self._get_org_permission_sets(org_id):
                ...
                cursor = self.ds.execute(
                    """SELECT resource_type, condition, condition_display, permission_level,
                              inherit_to_children, propagate_to_parents
                ...
                for row in rules:
                    ...
                    if key not in dp_map:
                        dp_map[key] = {
                            ...
                            # [2026-09-05 继承方向] 数据权限列表同步携带向上传播开关
                            'propagate_to_parents': bool(row.get('propagate_to_parents')),
                            'sources': [src],
                        }
                ...
                # [2026-09-05] 来源3：permission_set_dimension_scopes（维度范围, canonical ID-form）
                #   前端「权限配置」把数据范围写进该表；预览此前漏读 → staging 三个权限集
                #   （子领域/领域/产品维度）在矩阵全部显示"未配置"。
                for dim in self._load_ps_dimension_scopes(ps_id):
                    rt_zh = self._humanize_resource_type(dim['resource_type'])
                    _, display, _names = self._build_dim_scope_display(dim['resource_type'], dim['values'], dim['exclude'])
                    key = ('dim', rt_zh, display, '')
                    if key not in dp_map:
                        dp_map[key] = {
                            'kind': 'dimension',
                            'resource_type': rt_zh,
                            ...
                            'inherit_to_children': dim['inherit_children'],
                            'sources': [src],
                        }
                    elif src not in dp_map[key]['sources']:
                        dp_map[key]['sources'].append(src)
    ...
    def _build_ps_data_scope(self, ps) -> Dict[str, Dict[str, Any]]:
        ...
        rules = self._rows_to_dicts(cursor)
        # [2026-09-05] 权限集数据范围另一种主流来源：维度范围（permission_set_dimension_scopes）
        dim_scopes = self._load_ps_dimension_scopes(ps_id)
        if not rules and not dim_scopes:
        ...
        for row in rules:
            ...
            # [Spec 20 v6 2026-09-12] code IN ('a','b') 业务键锚点同样解析:
            #   键名同显「code（name）」(维度表反查), 无 name 回落裸 code。
            import re as _re
            ...
            if vm:
                ...
                cm = _re.match(r"^code\s+(NOT\s+IN|IN)\s*\(([^)]*)\)\s*$", condition, _re.IGNORECASE) if condition else None
                if cm:
                    codes = [x.strip().strip("'\"") for x in cm.group(2).split(',') if x.strip()]
                    name_map = self._resolve_anchor_names(rt, codes) or {}
                    for k in codes:
                        n = f"{k}（{name_map[k]}）" if name_map.get(k) else k
                        if n not in entry['__value_names']:
                            entry['__value_names'].append(n)
            entry['__rules'].append({
                'condition': condition,
                ...
                'permission_level_display': PREVIEW_LEVEL_ZH.get(row.get('permission_level'), row.get('permission_level')),
                'inherit_to_children': bool(row.get('inherit_to_children')),
                # [2026-09-05 继承方向] 透传向上传播开关，供矩阵「数据范围」列展示继承方向
                'propagate_to_parents': bool(row.get('propagate_to_parents')),
                'source_orgs': org_names,
                'source_permission_set': ps_name,
            })
        # [2026-09-05] 维度范围条目（rt = dimension_code；无 permission_level，语义与权限配置面板一致）
        for dim in dim_scopes:
            rt = dim['resource_type']
            expr, display, names = self._build_dim_scope_display(rt, dim['values'], dim['exclude'])
            entry = scope.setdefault(rt, {'__configured': False, '__expressions': [], '__displays': [], '__rules': [], '__value_names': []})
            entry['__configured'] = True
            if expr not in entry['__expressions']:
                entry['__expressions'].append(expr)
            ...
            entry['__rules'].append({
                ...
                'permission_level_display': '',
                'inherit_to_children': dim['inherit_children'],
                # [2026-09-05 继承方向] 维度范围无向上传播语义（仅向下 inherit_children）
    ...
    def get_fused_permission_config(self, identity_type: str, identity_id: int) -> Dict[str, Any]:
        ...
        row_order: List[str] = []
        row_seen = set()
        ps_rt_contributed: Dict[int, set] = {}
        for ps in ps_list:
            ps_id = ps['permission_set_id']
            ps_name = ps.get('permission_set_name') or ps.get('permission_set_code')
            ...
            for row in rm.get('resources', []):
                ...
                if not rt:
                    continue
                ps_rt_contributed.setdefault(ps_id, set()).add(rt)
        ...
        # [2026-09-05] scope-only 行合成：权限集在某 rt 上配了数据范围，但自身功能矩阵
        #   没有该 rt 的行（如 1214 财资管理子领域编辑：范围挂在 sub_domain，却无 sub_domain
        #   功能授权）。若不做合成，权限预览矩阵将看不到该权限集的数据范围行。
        for ps in ps_list:
            ps_id = ps['permission_set_id']
            ps_name = ps.get('permission_set_name') or ps.get('permission_set_code')
            for rt in (ps_scope.get(ps_id) or {}):
                if rt in ps_rt_contributed.get(ps_id, set()):
                    continue
                if rt not in row_seen:
                    row_seen.add(rt)
                    row_order.append(rt)
                    row_label[rt] = self._resource_label(rt)
                per_rt.setdefault(rt, []).append({'ps_id': ps_id, 'ps_name': ps_name, 'cells': {}})
        ...
        def _scope_covers(super_scope, sub_scope) -> bool:
            """sub_scope 是否 ⊆ super_scope。

            [FIX 2026-09-04] row_scope=None 表示「该行权限集在此资源类型上没有数据范围配置」，
            并不等于「无限制=全集」——若把 None 当全集，会让「未配置数据范围的功能行」错误吞掉
            另一个权限集「带具体范围（如 供应链云）」的同资源行（组织 1046 继承预览缺行的根因）。
            语义收紧：
              - 无范围行(sub=None) 只能被同为无范围的行(super=None) 覆盖；
              - 带范围行(super 有表达式) 才能覆盖更窄/相等的带范围行（表达式相同才判包含，保守避免误伤）；
              - 无范围行(super=None) 不得覆盖任何带范围行(sub 有表达式)。
            """
