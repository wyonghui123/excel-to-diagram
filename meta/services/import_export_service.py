...
class ImportExportService:
    ...
    def export_selected_types(self, selected_types: List[str], filters: Optional[Dict[str, Any]] = None,
        ...
        for i, ot in enumerate(ordered_types):
            ...
            # [FIX 2026-09-04 组织导出] 自引用父级 FK 列 (export_as_code) 数据列直接写业务编码
            code_display_field_ids = {
                fid for fid, info in bo_display_fields.items() if info.get('export_as_code')
            }
            ...
            for row_idx, record in enumerate(sheet_data, 2):
                ...
                for col_idx, header in enumerate(headers[col_offset:], 1 + col_offset):
                    ...
                    if field_id in bo_display_maps and value is not None:
                        ...
                        if display_name:
                            # [FIX 2026-09-04 组织导出] 自引用父级 FK: 只写业务编码
                            if field_id in code_display_field_ids:
                                value = display_name
                            else:
        ...
        for f in export_fields:
            ...
            if field_name not in hierarchy_fields:
                ...
                if vh:
                    ...
                    if vh_source and getattr(vh_source, 'type', None) == 'bo':
                        _vh_target_bo = getattr(vh_source, 'target_bo', '')
                        _vh_code_field = getattr(vh_source, 'code_field', 'code') or 'code'
                        _vh_display_field = getattr(vh_source, 'display_field', 'name') or 'name'
                        # [FIX 2026-09-04 组织导出] 自引用父级 FK (关联目标与当前对象同一张表,
                        # 如 org.parent_id → orgs) 在 Excel 数据列应显示上级记录的业务编码,
                        # 而不是 "显示名 (技术id)" (例: 上级组织列不得出现 "大业务应用架构 (253)")。
                        _export_as_code = False
                        if _vh_target_bo and _vh_code_field and _vh_code_field != _vh_display_field:
                            try:
                                from meta import get_meta_object as _gmo
                                _tmeta = _gmo(_vh_target_bo)
                                _export_as_code = bool(
                                    _tmeta and meta_obj.table_name
                                    and _tmeta.table_name == meta_obj.table_name
                                )
                            except Exception:
                                _export_as_code = False
                        bo_display_fields[f.id] = {
                            'target_bo': _vh_target_bo,
                            'display_field': _vh_display_field,
                            'value_field': getattr(vh_source, 'value_field', 'id') or 'id',
                            'code_field': _vh_code_field,
                            'export_as_code': _export_as_code,
                        }
    ...
    def _build_bo_display_maps(self, sheet_data: List[Dict[str, Any]],
        ...
        for field_id, vh_info in bo_display_fields.items():
            ...
            try:
                ...
                # [FIX 2026-09-04 组织导出] export_as_code 字段 (自引用父级 FK) 查业务编码列,
                # 其余仍查 display_field (显示名)。
                display_field = vh_info['display_field']
                if vh_info.get('export_as_code'):
                    display_field = vh_info.get('code_field') or vh_info['display_field']
