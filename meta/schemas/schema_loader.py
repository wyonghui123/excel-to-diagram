    ...
    def load_schema(self, name: str) -> dict:
        ...
        for f in (meta.fields or []):
            result['fields'].append({
                ...
                # [FIX 2026-09-05 父组织树状 SearchHelp] 字段级 value_help 必须透传:
                # ObjectPageShell (metaService.getUIConfig → /meta/<type>/ui-config) 读取
                # f.value_help 构建 ValueHelpField 配置, 缺失时回退 dropdown → 树状弹窗失效
                'value_help': _serialize_value_help(getattr(f, 'value_help', None)),
            })
...
def _serialize_value_help(vh):
    """[FIX 2026-09-05 父组织树状 SearchHelp] 统一 value_help → dict

    复用 bo value-help 链路的 value_help_to_dict (含 source.hierarchy /
    behavior.exclude_self / presentation.result_type+display_mode),
    保证 ui-config 与 bo_api 两端点序列化口径一致。
    延迟 import 避免循环依赖。
    """
    if vh is None:
        return None
