...
def _create_user_group_with_roles(ds, user_id, permission_set_id):
    ...
    role_exists = ds.execute("SELECT 1 FROM permission_sets WHERE id = ?", [permission_set_id]).fetchone()
    if not role_exists:
        ds.execute("INSERT INTO permission_sets (id, name, code) VALUES (?, ?, ?)",
            (permission_set_id, f'AutoRole_{permission_set_id}', f'auto_role_{permission_set_id}'))
        ds.commit()
    
    # 先清理旧的用户组，避免残留数据
    ds.execute("DELETE FROM org_permission_sets WHERE group_id IN (SELECT id FROM orgs WHERE code LIKE ?)",
        (f'audit_group_{user_id}_%',))
    ds.execute("DELETE FROM org_members WHERE group_id IN (SELECT id FROM orgs WHERE code LIKE ?)",
        (f'audit_group_{user_id}_%',))
    ds.execute("DELETE FROM orgs WHERE code LIKE ?",
        (f'audit_group_{user_id}_%',))
    ds.commit()
    
    ds.execute("INSERT INTO orgs (code, name) VALUES (?, ?)",
        (group_code, f'Audit Group {user_id}'))
    ds.commit()  # 确保 group_id 可用
    
    # v3.18: 使用 SELECT MAX 获取 group_id，避免 last_insert_rowid() 问题
    group_id = ds.execute("SELECT MAX(id) FROM orgs WHERE code = ?", [group_code]).fetchone()[0]
    
    if group_id is None:
        raise RuntimeError(f"Failed to create org with code: {group_code}")
    
    ds.execute("INSERT INTO org_members (user_id, group_id) VALUES (?, ?)",
        (user_id, group_id))
    ds.execute("INSERT INTO org_permission_sets (group_id, permission_set_id) VALUES (?, ?)",
...
def _cleanup_user_group_association(ds, group_id, permission_set_id):
    """清理用户组-角色关联"""
    ds.execute("DELETE FROM org_permission_sets WHERE group_id = ? AND permission_set_id = ?",
        (group_id, permission_set_id))
    ds.execute("DELETE FROM org_members WHERE group_id = ?", [group_id])
    ds.execute("DELETE FROM orgs WHERE id = ?", [group_id])
    ds.commit()
...
class TestUserGroupAssociateAudit:
    ...
    def test_add_member_generates_associate_log(self, ds):
        ...
        ds.execute(
            "INSERT OR IGNORE INTO org_members (user_id, group_id) VALUES (?, ?)",
            [user_id, group_id]
        )
        ds.commit()
        ...
        ds.execute("DELETE FROM org_members WHERE user_id = ? AND group_id = ?",
                    [user_id, group_id])
        ds.commit()
...
class TestRoleAssignAudit:
    ...
    def test_assign_role_generates_associate_log(self, ds):
        ...
        role_row = ds.execute("SELECT id FROM permission_sets LIMIT 1").fetchone()
