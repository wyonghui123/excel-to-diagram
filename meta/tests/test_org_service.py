...
def _insert_group(ds, name='TestGroup', code='test_group', parent_id=None, manager_id=None):
    ...
    cursor = ds.execute("SELECT id FROM orgs WHERE code = ?", [code])
    return cursor.fetchone()[0]
...
def _insert_role(ds, name='TestRole', code='test_role'):
    ...
    cursor = ds.execute("SELECT id FROM permission_sets WHERE code = ?", [code])
    return cursor.fetchone()[0]
...
def test_get_group_roles(svc, ds):
    ...
    ds.execute(
        "INSERT INTO org_permission_sets (group_id, permission_set_id) VALUES (?, ?)",
        [gid, rid]
    )
...
def test_get_user_effective_data_permissions_via_groups(svc, ds):
    ...
    ds.execute(
        "INSERT INTO permission_set_data_permissions (permission_set_id, resource_type, resource_id, permission_level) VALUES (?, ?, ?, ?)",
...
def test_migrate_group_data_permissions_to_roles(svc, ds):
    ...
    ds.execute(
        "INSERT INTO org_data_permissions (group_id, resource_type, resource_id, permission_level, is_deprecated) VALUES (?, ?, ?, ?, 0)",
    ...
    cursor = ds.execute(
        "SELECT is_deprecated FROM org_data_permissions WHERE group_id = ?", [gid]
    )
    row = cursor.fetchone()
    if row is not None:
        # 如果实现选择标记 deprecated
