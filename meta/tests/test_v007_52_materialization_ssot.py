...
class TestV00752MaterializationSSOT:
    ...
    def test_get_updated_at_none_strategy(self):
        ...
        注：Spec 16 后 validate_table_name 白名单不再含 legacy 表名
        （group_data_permissions 已改名 org_data_permissions），
        因此改用仍在白名单内的 change_events（SSOT strategy=none）
        """
        from meta.core.materialization_registry import get_updated_at
        ...
        result = get_updated_at(MockDS(), "change_events", 1, fallback=None)
        assert result is None

        result = get_updated_at(MockDS(), "change_events", 1, fallback="DEFAULT")
    ...
    def test_get_updated_at_strategy_none_returns_fallback(self):
        ...
        已注册但 strategy=none 的中间表（change_events）
        """
        from meta.core.materialization_registry import get_updated_at
        ...
        # change_events 已注册（Spec 16 白名单内）且 strategy=none
        result = get_updated_at(MockDS(), "change_events", 1, fallback=None)
        assert result is None
        result = get_updated_at(MockDS(), "change_events", 1, fallback="X")
        assert result == "X"
