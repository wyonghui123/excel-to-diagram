...
- role_menu_api (3 端点): /<permission_set_id>/menu-permissions (GET/PUT) + /<permission_set_id>/unified-permissions
- role_dimension_scope_api (3 端点): /<permission_set_id>/dimension-scopes (GET/POST) + /<permission_set_id>/derived-permissions
"""
import json
import time
import pytest
...
class TestRoleMenuAPI:
    ...
    def test_get_role_menu_permissions(self, api_client, admin_headers):
        """GET /api/v1/permission-sets/<id>/menu-permissions 角色菜单权限"""
    ...
    def test_get_role_unified_permissions(self, api_client, admin_headers):
        """GET /api/v1/permission-sets/<id>/unified-permissions SAP PFCG 风格"""
        ...
        data = body['data']
        # 核心字段
        assert 'permission_set_id' in data
    ...
    def test_get_role_unified_permissions_404(self, api_client, admin_headers):
        """GET /api/v1/permission-sets/<nonexistent>/unified-permissions 200 (无菜单)"""
        # 注: 端点不返回 404, 仅返回空 menus 列表
        resp = api_client.get('/api/v1/permission-sets/9999999/unified-permissions', headers=admin_headers)
        # 200 (无数据) 或 500 (DB 错误)
    ...
    def test_update_role_menu_permissions_empty_body_400(self, api_client, admin_headers):
        ...
        resp = api_client.put(
            '/api/v1/permission-sets/1/menu-permissions',
            json={},
...
class TestRoleDimensionScopeAPI:
    ...
    def test_get_dimension_scopes_for_role(self, api_client, admin_headers):
        """GET /api/v1/permission-sets/<id>/dimension-scopes 角色维度范围"""
        resp = api_client.get('/api/v1/permission-sets/1/dimension-scopes', headers=admin_headers)
        assert resp.status_code == 200
        body = resp.get_json()
        assert body.get('success') is True
        assert isinstance(body['data'], list)
    ...
    def test_save_dimension_scopes_empty_body_400(self, api_client, admin_headers):
        ...
        resp = api_client.post(
            '/api/v1/permission-sets/1/dimension-scopes',
            json={},
            headers=admin_headers,
        )
        assert resp.status_code == 400
    ...
    def test_get_derived_permissions(self, api_client, admin_headers):
        """GET /api/v1/permission-sets/<id>/derived-permissions 派生权限"""
        resp = api_client.get('/api/v1/permission-sets/1/derived-permissions', headers=admin_headers)
        assert resp.status_code == 200
        body = resp.get_json()
        assert body.get('success') is True
    ...
    def test_get_derived_permissions_for_nonexistent_role(self, api_client, admin_headers):
        ...
        resp = api_client.get('/api/v1/permission-sets/9999999/derived-permissions', headers=admin_headers)
        # 引擎可能抛错, 接受 200 或 500
        assert resp.status_code in (200, 500)
