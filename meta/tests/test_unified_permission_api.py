...
def app_with_db():
    ...
    conn.executescript('''
        ...
        CREATE TABLE permission_set_effective_intents (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            permission_set_id INTEGER NOT NULL,
        ...
        CREATE TABLE permission_sets (id INTEGER PRIMARY KEY, name TEXT);
        INSERT INTO permission_sets VALUES (100, 'PermissionSet A'), (101, 'PermissionSet B');
    ''')
    conn.commit()
    conn.close()
...
class TestEffectiveIntentsCrud:
    ...
    def test_list_intents_by_role(self, client, app_with_db):
        ...
        conn = sqlite3.connect(db_path)
        conn.execute(
            "INSERT INTO permission_set_effective_intents (permission_set_id, bo_id, action_name, data_scope) "
            "VALUES (100, 'product', 'read', '{}'), "
            "       (100, 'product', 'list', '{}'), "
            "       (101, 'product', 'read', '{}')"
        )
    ...
    def test_update_intent_data_scope(self, client, app_with_db):
        ...
        conn = sqlite3.connect(db_path)
        row = conn.execute(
            'SELECT data_scope FROM permission_set_effective_intents WHERE id = ?', [intent_id]
        ).fetchone()
        conn.close()
    ...
    def test_delete_intent(self, client, app_with_db):
        ...
        conn = sqlite3.connect(db_path)
        row = conn.execute(
            'SELECT COUNT(*) FROM permission_set_effective_intents WHERE id = ?', [intent_id]
        ).fetchone()
        conn.close()
        assert row[0] == 0
...
class TestDerivationTrigger:
    ...
    def test_trigger_derivation(self, client, app_with_db):
        ...
        conn = sqlite3.connect(db_path)
        rows = conn.execute(
            'SELECT action_name FROM permission_set_effective_intents WHERE permission_set_id = 100'
        ).fetchall()
        conn.close()
...
class TestCompletenessCheck:
    ...
    def test_completeness_yellow_stale(self, client, app_with_db):
        ...
        conn = sqlite3.connect(db_path)
        conn.execute(
            "INSERT INTO permission_set_effective_intents (permission_set_id, bo_id, action_name, data_scope, is_stale) "
