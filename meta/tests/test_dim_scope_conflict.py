...
from meta.api.permission_set_dimension_scope_api import (
...
class TestConflictCheckSameRole:
    ...
    def test_same_role_wildcard_and_exclude_conflict(self):
        ...
        with pytest.raises(DimScopeConflictError) as exc_info:
            _check_wildcard_exclude_conflict(ds, permission_set_id=1, new_scopes=new_scopes)
    ...
    def test_empty_scopes_pass(self):
        """空配置 → 通过"""
        ds = MockDataSource()
        _check_wildcard_exclude_conflict(ds, permission_set_id=1, new_scopes=[])
...
class TestConflictCheckMultiRole:
    ...
    def test_multi_role_wildcard_vs_exclude_conflict(self):
        ...
        with pytest.raises(DimScopeConflictError) as exc_info:
            _check_wildcard_exclude_conflict(ds, permission_set_id=2, new_scopes=new_scopes)
