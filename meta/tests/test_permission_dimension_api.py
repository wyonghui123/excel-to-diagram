...
def _init_test_database(ds):
    ...
    ds.execute("INSERT INTO permission_sets (id, name, code) VALUES (1, '供应链管理员', 'SUPPLY_CHAIN_ADMIN')")
...
def _setup_engine(ds):
    """设置引擎"""
    import meta.api.permission_dimension_api as api_module
    api_module._data_source = ds
    api_module._engine = PermissionDimensionEngine(ds, ttl_seconds=300)
...
def mgmt_dimension_client():
    ...
    register_permission_dimension_apis(app)
...
class TestPermissionDimensionAPI:
    """管理维度 API 集成测试"""
...
class TestPermissionDimensionAPIErrorHandling:
    """管理维度 API 错误处理测试"""
