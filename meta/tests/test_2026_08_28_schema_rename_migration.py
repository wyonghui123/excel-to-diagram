...
def test_org_functions_table_created(fresh_db):
    ...
    ⚠️ 依赖顺序: create_org_functions 需要 orgs 表存在
    (fresh_db 无 orgs, 必须先跑 rename_user_groups_to_orgs)
    """
    conn = sqlite3.connect(str(fresh_db))

    # 先执行依赖迁移 (与 runner 的 prerequisites 声明一致)
    from meta.migrations.rename_user_groups_to_orgs import upgrade as up_orgs
    up_orgs(conn)
