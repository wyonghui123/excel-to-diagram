"""
RoleDataPermissionFactory (Phase 6 新建)
==========================================

角色数据权限工厂: 用于角色级别数据访问权限测试

yaml: meta/schemas/role_data_permission.yaml
required 字段:
- role_id (关联角色)
- resource_type (资源类型)
- resource_id (资源实例ID)
- permission_level (权限级别, 默认 read)
"""
...
@register_factory
class RoleDataPermissionFactory(BaseFactory):
    """角色数据权限工厂"""

    _OBJECT_TYPE = 'role_data_permission'
    ...
    @classmethod
    def create_for_role(
        cls, role_id: int, resource_type: str, resource_id: int,
    ...
    def create_read_only(
        ...
        return cls.create_for_role(
            role_id, resource_type, resource_id, cls.LEVEL_READ, cookie, **overrides
        )
    ...
    def create_admin(
        ...
        """创建管理员权限"""
        return cls.create_for_role(
            role_id, resource_type, resource_id, cls.LEVEL_ADMIN, cookie, **overrides
        )
