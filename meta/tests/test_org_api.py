...
SUNSET_CRUD = [
    pytest.param('get', '/api/v1/orgs', None, id='list_sunset'),
    pytest.param('post', '/api/v1/orgs', {'name': 'g'}, id='create_sunset'),
    pytest.param('get', '/api/v1/orgs/1', None, id='get_sunset_by_id'),
    pytest.param('put', '/api/v1/orgs/1', {'name': 'g'}, id='update_sunset'),
    pytest.param('delete', '/api/v1/orgs/1', None, id='delete_sunset'),
]

ROLE_CRUD = [
    pytest.param('get', '/api/v1/orgs/1/roles', None, id='get_roles'),
    pytest.param('put', '/api/v1/orgs/1/roles', {'role_ids': [1, 2]}, id='set_roles'),
    pytest.param('post', '/api/v1/orgs/1/roles/1', None, id='add_role'),
    pytest.param('delete', '/api/v1/orgs/1/roles/1', None, id='remove_role'),
]
...
class TestSunsetCRUD:
    ...
    def test_list_user_groups_returns_410(self, api_client):
        ...
        r = expect(api_client, 'get', '/api/v1/orgs', LIST_SUNSET)
...
class TestMembers:
    ...
    def test_get_members(self, api_client):
        r = expect(api_client, 'get', '/api/v1/orgs/1/members', OK_AUTH)
        assert_data_field(r, '_deprecated', True)

    def test_add_member_missing_user(self, api_client):
        expect(api_client, 'post', '/api/v1/orgs/1/members', VALIDATION, json={})

    def test_add_member_valid(self, api_client):
        expect(api_client, 'post', '/api/v1/orgs/1/members', OK_AUTH, json={'user_id': 1})

    def test_set_members(self, api_client):
        expect(api_client, 'put', '/api/v1/orgs/1/members', OK_AUTH,
               json={'user_ids': [1, 2, 3]})

    def test_remove_member(self, api_client):
        expect(api_client, 'delete', '/api/v1/orgs/1/members/1', OK_AUTH)

    def test_get_data_permissions(self, api_client):
        r = expect(api_client, 'get', '/api/v1/orgs/1/data-permissions', OK_AUTH)
        assert_data_field(r, '_deprecated', True)

    def test_add_data_permission_missing(self, api_client):
        expect(api_client, 'post', '/api/v1/orgs/1/data-permissions', VALIDATION, json={})

    def test_remove_data_permission(self, api_client):
        expect(api_client, 'delete', '/api/v1/orgs/1/data-permissions/1', OK_AUTH)
...
class TestRolesAndLogs:
    ...
    def test_available_roles(self, api_client):
        expect(api_client, 'get', '/api/v1/orgs/1/roles/available', OK_AUTH)
    ...
    def test_get_logs_nonexistent_group(self, api_client):
        expect(api_client, 'get', '/api/v1/orgs/999999/logs', NOT_FOUND_AUTH)

    def test_get_logs_pagination(self, api_client):
        expect(api_client, 'get', '/api/v1/orgs/1/logs?page=1&page_size=10', LOGS_PAGINATION)
