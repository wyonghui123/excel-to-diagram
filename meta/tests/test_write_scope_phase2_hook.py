...
def write_hook_db():
    ...
    conn.executescript('''
        ...
        INSERT INTO org_permission_sets VALUES (1, 100);
        INSERT INTO org_members VALUES (999, 1);
...
class TestWriteScopePhase2Hook:
    ...
    def test_hook_no_intent_denies(self, write_mock_context, write_hook_db):
        ...
        # 删除所有 Intent
        conn = sqlite3.connect(write_hook_db)
        conn.execute('DELETE FROM permission_set_effective_intents')
        conn.commit()
        conn.close()
    ...
    def test_hook_multiple_targets_all_must_pass(self, write_mock_context, write_hook_db):
        ...
        conn = sqlite3.connect(write_hook_db)
        conn.execute(
            "INSERT INTO permission_set_effective_intents (role_id, bo_id, action_name, data_scope, source) "
    ...
    def test_hook_empty_include_allows_all(self, write_mock_context, write_hook_db):
        ...
        conn = sqlite3.connect(write_hook_db)
        conn.execute(
            "UPDATE permission_set_effective_intents SET data_scope = "
            "'{\"include\":[],\"exclude\":[]}' "
