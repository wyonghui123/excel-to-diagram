...
def db_full_legacy():
    ...
    conn.execute("INSERT INTO permission_sets (id, code, name) VALUES (1, 'admin', 'Admin')")
    conn.execute("INSERT INTO permission_sets (id, code, name) VALUES (2, 'user', 'User')")
    ...
    conn.execute("INSERT INTO permission_set_dimension_scopes (id, permission_set_id, dimension_code, dimension_values, scope_mode) VALUES (1, 1, 'product', '[1]', 'include')")
    ...
    conn.execute("INSERT INTO user_permission_sets (user_id, permission_set_id) VALUES (1, 1)")
    conn.execute("INSERT INTO user_permission_sets (user_id, permission_set_id) VALUES (2, 2)")
    # permission_set 1 有 product.read 权限, permission_set 2 没有
    conn.execute("INSERT INTO permission_set_permissions (permission_set_id, permission_code, resource_type) VALUES (1, 'product.read', 'product')")
...
class TestP3T6DataConsistency:
    ...
    def test_dimension_consistency(self, db_full_legacy):
        ...
        old_count = ds.execute("SELECT COUNT(*) FROM permission_set_dimension_scopes").fetchone()[0]
...
class TestP3T7DeprecateLegacyTables:
    ...
    def test_deprecate_renames_legacy_tables(self, db_full_legacy):
        ...
        # 旧表不存在
        rows = ds.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='permission_set_dimension_scopes'"
        ).fetchall()
        assert len(rows) == 0
        # _deprecated_ 表存在
        rows = ds.execute(
