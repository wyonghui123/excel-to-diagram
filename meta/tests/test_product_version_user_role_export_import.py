...
class TestUserRoleUserGroupExportImport:
    ...
    def _get_role_data(self):
        from meta.core.datasource import get_data_source
        ds = get_data_source('sqlite', database=get_test_db_path())
        cursor = ds.execute("SELECT id, code, name FROM permission_sets LIMIT 2")
        return cursor.fetchall()

    def _get_user_group_data(self):
        from meta.core.datasource import get_data_source
        ds = get_data_source('sqlite', database=get_test_db_path())
        cursor = ds.execute("SELECT id, code, name FROM orgs LIMIT 2")
        return cursor.fetchall()
    ...
    def test_05_role_cud_upsert_create(self):
        ...
        cursor = ds.execute("SELECT name FROM permission_sets WHERE code = ?", [new_code])
        row = cursor.fetchone()
    ...
    def test_06_user_group_cud_upsert_create(self):
        ...
        cursor = ds.execute("SELECT name FROM orgs WHERE code = ?", [new_code])
        row = cursor.fetchone()
    ...
    def test_07_user_group_hierarchy_parent_child(self):
        ...
        cursor = ds.execute("SELECT id FROM orgs WHERE code = ?", [parent_code])
        ...
        cursor = ds.execute("SELECT parent_id FROM orgs WHERE code = ?", [child_code])
...
class TestBatchAssociation:
    ...
    def test_04_assign_role_to_group(self):
        ...
        cursor = ds.execute("""
            SELECT 1 FROM org_permission_sets WHERE permission_set_id = ? AND group_id = ?
        """, [permission_set_id, group_id])
        if cursor.fetchone():
            pytest.skip("角色已分配给用户组")

        ds.execute("""
            INSERT INTO org_permission_sets (permission_set_id, group_id)
            VALUES (?, ?)
        """, [permission_set_id, group_id])
...
class TestValueHelpForObjects:
    ...
    def test_09_user_group_parent_id_export_has_dv(self):
        ...
        from meta.core.datasource import get_data_source
        ds = get_data_source('sqlite', database=get_test_db_path())
        cursor = ds.execute("SELECT id FROM orgs LIMIT 1")
        if not cursor.fetchone():
            pytest.skip("数据库中无 org 数据")
...
class TestRelatedObjectsValueHelp:
    ...
    def test_05_role_export_is_active_has_dv(self):
        ...
        from meta.core.datasource import get_data_source
        ds = get_data_source('sqlite', database=get_test_db_path())
        cursor = ds.execute("SELECT id FROM permission_sets LIMIT 1")
        if not cursor.fetchone():
            pytest.skip("数据库中无 permission_set 数据")
