...
def init_auth_tables(db_path):
    ...
    # [v088 2026-09-15 soft-delete] manager_id 列保留 (DROP COLUMN 在 SQLite < 3.35 不可用)
    #   但写路径已拒绝 is_manager 入参 (org_api.add_group_member + org_service.add_member);
    #   读路径 fallback 段已删除 (org_service.get_managed_orgs 走 OrgAdminScopeService 主源)。
    if not _table_exists(cursor, ('user_groups', 'orgs')):
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS orgs (
                ...
                code VARCHAR(200) UNIQUE NOT NULL,
                parent_id INTEGER,
                manager_id INTEGER,  -- [v088] deprecated, M4 软删 (建议 2027-Q1 硬删)
                description VARCHAR(200),
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
            )
        """)
        print("[OK] 创建 orgs 表 (Spec16 新名, manager_id 列 deprecated)")

    # 用户组成员关联表 [Spec16] 兼容旧名 user_group_members + 新名 org_members
    # [v088 2026-09-15 soft-delete] is_manager 列保留 (DROP COLUMN 在 SQLite < 3.35 不可用)
    #   写路径已拒绝新增 is_manager=1 (seed 遗留的 admin → org 1 记录保留)。
    if not _table_exists(cursor, ('user_group_members', 'org_members')):
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS org_members (
                ...
                is_manager INTEGER DEFAULT 0,  -- [v088] deprecated, M4 软删
                joined_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
                FOREIGN KEY (org_id) REFERENCES orgs(id) ON DELETE CASCADE
            )
        """)
        print("[OK] 创建 org_members 表 (Spec16 新名, is_manager 列 deprecated)")
    ...
    if not cursor.fetchone():
        ...
        if _table_exists(cursor, ('permission_sets',)):
            ...
            admin_role_id = cursor.lastrowid
            cursor.execute(
                "INSERT INTO user_permission_sets (user_id, role_id) VALUES (?, ?)",
                (admin_id, admin_role_id)
            )
        conn.commit()
