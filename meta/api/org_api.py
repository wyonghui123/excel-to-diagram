...
from meta.services.auth_middleware import login_required, require_permission, require_permission_any, is_admin, get_current_user
...
@login_required
@require_permission_any('user:update', 'org_member:manage')  # [Spec 19 M1 TR-002] 过渡期双码并存
@v1_deprecated(migrated_to='/api/v2/bo/user_group/<org_id>/associations/members')
def add_group_member(org_id):
    ...
    try:
        ...
        # [v088 2026-09-15 soft-delete Spec 19 M4] is_manager 入参已废弃 (FR-011 过渡完成).
        #   受托管理员授权应改用资源矩阵 org 行级规则 (OrgAdminScopeService).
        if is_manager:
            import logging
            logging.getLogger(__name__).warning(
                '[v088 deprecated] add_group_member(is_manager=True) 拒绝; '
                '请改用资源矩阵 org 行级规则配置委托'
            )
            return jsonify({
                'success': False,
                'error_code': 'DEPRECATED_IS_MANAGER',
                'message': (
                    'is_manager 入参已废弃 (Spec 19 M4 软删 v088). '
                    '受托管理员授权应改用资源矩阵 org 行级规则配置委托. '
                    '详细见 docs/spec_权限体系升级/19_org_admin_delegation.md §8 M4.'
                )
            }), 400
...
@login_required
@admin_required  # [Spec 19 M1 FR-005] 权限配置属敏感动作，仅限全局管理员（原 user:update 全局码可横向越权）
...
@login_required
@admin_required  # [Spec 19 M1 FR-005] 同上：敏感动作仅限全局管理员
@v1_deprecated(migrated_to='/api/v2/bo/org/<org_id>/associations/permission_sets')
...
@org_bp.route('/orgs/<int:org_id>/roles', methods=['POST'])
@login_required
@admin_required  # [Spec 19 M1 FR-005] 权限集绑定 = 委托配置本身，仅限全局管理员（原 user:update 全局码可横向越权）
