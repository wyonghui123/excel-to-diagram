...
def _normalize_dim_values_to_ids(values):
    """归一 dimension_values 为纯 id 列表 (canonical ID-form).

    兼容两种存储形态:
      - ID-form (canonical): [13, 14] / ['13'] / ['*'] → 原样保留
      - OBJ-form (旧保存路径残留, 2026-09-04 发现): 
          [{"code": "FIN", "id": 13, "name": "财务云"}] → 取每项 'id'
        (无 id 时兜底取 'code'; 两者皆缺则丢弃该项, 避免把 dict/None 落库)

    用途:
      - GET 富化前归一: 避免把 dict 当 id 绑定进 SQL (查询失败后再二次包裹,
        最终前端渲染成 [object Object])
      - POST 写库前归一: 保证落库形态始终是 canonical ID-form
        (OBJ-form 会使 dimension_scope_engine.expand_dimension_values 因
        int(dict) 失败而完全不展开, 数据权限不生效)
    """
    if not values:
        return []
    out = []
...
# [Spec 20] 业务键锚点白名单: 仅字母/数字/下划线/点/横线
# (防 SQL 注入; 引擎侧 _build_chain_condition 另有单引号翻倍转义双保险)
_ANCHOR_CODE_PATTERN = re.compile(r'^[A-Za-z0-9_.\-]+$')


def _is_anchor_code(v) -> bool:
    """判断单个值是否为业务键锚点 (字符串且非 '*' 且非数字串)"""
...
def _validate_anchor_codes(values) -> tuple:
    ...
    for v in values or []:
        if _is_anchor_code(v) and not _ANCHOR_CODE_PATTERN.match(v.strip()):
...
def get_dimension_scopes(permission_set_id):
    try:
        ...
        for row in cursor.fetchall():
            ...
            # [v084 2026-09-04] 兼容 OBJ-form 存量存储: 先归一为纯 id 再做名称富化.
            # 修复前: dict 元素被当 id 绑定进 SQL → 抛异常走 fallback 二次包裹成
            #   {'id': <dict>, 'name': str(<dict>), 'code': ''} → 前端 [object Object].
            dimension_values = _normalize_dim_values_to_ids(dimension_values)
                ...
                try:
                    ...
                    for vid in dimension_values:
                        ...
                        if vid_str in name_map:
                            ...
                            # [FR-014 伴随修复 2026-09-06] 失效引用诚实标注：
                            #   此前静默 name=str(id)，前端 chip 显示裸数字（用户无法分辨
                            #   是名称还是 ID，实测 PS 1196 domain=13 显示「13」）。
                            #   现明确标注引用已失效 + missing 标志，前端可直接展示/清理。
                            dimension_values_with_names.append(
                                {'id': vid, 'name': f'ID {vid}（引用已失效）', 'code': '', 'missing': True})
...
def save_dimension_scopes(permission_set_id):
    try:
        ...
        # [Spec 20] 业务键锚点预检结果 (warning 不阻塞保存)
        preflight_warnings = []

        with ds.transaction():
            ds.execute("DELETE FROM permission_set_dimension_scopes WHERE permission_set_id = ?", [permission_set_id])
            for item in data:
                # [v084 2026-09-04] 写库前归一为 canonical ID-form.
                # 老客户端会把 GET 富化后的对象数组 ([{"code","id","name"},...]) 原样回传,
                # 直接 json.dumps 落库 → OBJ-form → 读取端二次包裹 ([object Object]) +
                # 引擎 expand_dimension_values 无法展开 (int(dict)), 数据权限不生效.
                normalized_values = _normalize_dim_values_to_ids(item.get('dimension_values', []))
                # [Spec 20] 锚点白名单校验 (非法字符直接 400, 引擎侧转义是双保险)
                ok, err = _validate_anchor_codes(normalized_values)
                if not ok:
                    return jsonify({'success': False, 'message': err}), 400
                anchor_strs = [v for v in normalized_values if _is_anchor_code(v)]
                if anchor_strs:
                    preflight_warnings.extend(_preflight_anchor_warnings(
...
def resolve_dim_scope_preview(permission_set_id):
    ...
    供前端维度值选择器回显: code（名称 · 当前命中 N 个实例）;
    resolved_count=0 时前端红色警示 (先配权限后建对象的合法状态).
    """
    try:
        ...
        for item in payload.get('scopes', []):
            ...
            codes = [v for v in item.get('dimension_values', [])
                     if _is_anchor_code(v)]
            if not dim or not codes or dim not in RESOURCE_TABLE_MAP:
                continue
            ...
            for c in codes:
                ...
                entries = []
                table = RESOURCE_TABLE_MAP[dim]
                for i in ids:
                    try:
                        row = ds.execute(
                            f"SELECT d.code, d.name, v.code FROM {table} d "
                            f"LEFT JOIN versions v ON d.version_id = v.id "
                            f"WHERE d.id = ?",
                            [i]).fetchone()
                    except Exception:
                        row = None
                ...
                entry = {'resolved_count': len(ids), 'instances': entries}
                if not ids:
                    entry['warning'] = '未解析到实例'
                dim_results[c] = entry
