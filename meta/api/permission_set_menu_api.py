...
def update_permission_set_menu_permissions(permission_set_id):
    ...
    try:
        ...
        with ds.transaction():
            # [FIX 2026-09-02] DELETE 也加 try/except: 任何 schema 不一致都不要阻塞整个 PUT
            try:
                ds.execute(
            ...
            # [FIX 2026-09-02] try/except 兜底: Plan D 迁移期间 permission_set_menu_permissions 表 schema
            #   可能不一致 (无 created_at 等列), 任意单条 INSERT 失败不应阻塞整个 PUT
            ...
            # [FIX 2026-09-02] 防御 menu_codes=[] → IN () SQL syntax error: 空数组跳过 SELECT
            if menu_codes:
                cursor = ds.execute(
