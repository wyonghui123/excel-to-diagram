...
_RESOURCE_TYPE_LABELS = {
    ...
    # [FIX Spec16 Phase7 r3 2026-09-02] 删 relationship/annotation:
    #   yaml 已不声明, 矩阵行也已不渲染. 残留 label 容易让前端误展示, 一并清理.
    'audit_log': '审计日志',
    'user': '用户', 'role': '角色',
    # [Spec 19 M2 2026-09-05 FR-001] 委托授权行中文标签
    'org': '组织',
...
# [Spec 19 M2 2026-09-05 FR-001] org/user 从主体白名单移出，作为「委托授权行」进入矩阵：
#   一体化方案（Spec 19 §9.4 方案 B）—— 组织/用户行级范围 = 管理范围（OrgAdminScopeService
#   动态解析子树），矩阵行动作集已由 resource_types.yaml 收窄为 read/create/update。
#   role 保持主体白名单（权限集再绑定 = 委托配置本身，FR-005 仅全局管理员专用路径）。
#   org_member 同为主体白名单：成员关联实体（junction），非可授权业务对象；
#   其 org_member:manage 码作为功能钥匙存在（M1 已收口），不以矩阵行暴露。
_IDENTITY_RESOURCE_TYPES = frozenset({'role', 'org_member'})
...
_ACTION_LABELS = {
    ...
    # [Spec 22 FR-005 2026-09-13 + PM 反馈第十七次] state_transition 通用 action 标签
    # 与 _standard_actions.yaml 的 instance 级 business action 一一对应
    # archive/restore 已撤销（详见 Spec 22 §6.7）—— 长期不活跃由 deactivate 覆盖，
    # 删除数据恢复由 audit_log row_snapshot + restore endpoint 承担（TBD）
...
def _load_object_owd_list() -> List[Dict[str, Any]]:
    """[Spec 21 FR-005 2026-09-12] 从 object_owd 表读取 OWD 列表。
    - 表无数据 / 读失败 → 返回 []
    - 前端据此渲染「对象级基线（OWD）」只读卡
    - 注: 当前不做编辑入口（OWD 配置是 P3 单独 spec 负责）
    """
    try:
        # [Bug fix 2026-09-12] get_data_source(source_type, **kwargs) 必填 source_type
        # 之前漏传导致 runtime TypeError，OWD 列表始终为空
        ds = get_data_source("sqlite", database=get_meta_db_path())
        if ds is None:
            return []
        # 兼容两种返回格式：tuple / sqlite3.Row
        cursor = ds.execute(
        ...
        for row in rows or []:
            if isinstance(row, dict):
                result.append({
                    ...
                    "default_permission_level": row.get("default_permission_level"),
                    "description": row.get("description"),
                })
            else:
                # tuple 格式
                result.append({
                    "bo_id": row[0] if len(row) > 0 else None,
                    "default_visibility": row[1] if len(row) > 1 else None,
                    "default_permission_level": row[2] if len(row) > 2 else None,
...
def _build_action_hints() -> Dict[str, Dict[str, str]]:
    """[Spec 19 FR-012 2026-09-05] 灰化动作说明（说明与限制同源）

    来源: resource_types.yaml 每个 rt 的 action_hints 段。
    语义: 动作未进 actions 可授权集时，矩阵灰化格 tooltip 展示的具体原因
    （"为什么不能勾"）—— 提示与动作集收窄声明在同一文件，改限制时说明自动跟随。
    权限主体（role 等）不下发。
    """
    resource_types_data = _load_schema_yaml("resource_types.yaml")
    ...
    for rt, rt_cfg in resource_types_data.items():
        ...
        if _is_identity_resource(rt):
            continue
        hints = rt_cfg.get("action_hints")
        if isinstance(hints, dict) and hints:
            result[rt] = {str(a): str(h) for a, h in hints.items() if h}
    return result


def _build_resource_hierarchy() -> Dict[str, Dict[str, Any]]:
    """[Spec 19 FR-014 2026-09-06] 资源矩阵类型层树形数据源（SSOT: hierarchies.yaml）

    读 biz_hierarchy levels（kind=='entity'）→ {rt: {parent, level, kind}}。
    - relationship 是 level 6 kind=association（Spec16 r3 已移出矩阵语义），不下发。
    - 与 resource_types.yaml 的 parent 字段做一致性校验：不一致 logger.warning
      （以 hierarchies.yaml 为准，不阻断）——两份声明漂移时先告警暴露。
    - org/user/scheduled_task 等非 biz_hierarchy 资源不在返回值中 → 前端视为顶层行。
    """
    ...
    for lv in (hierarchy or {}).get("levels", []) or []:
        obj = lv.get("object")
        if not obj or lv.get("kind") != "entity":
            continue
        result[obj] = {
            "parent": lv.get("parent_object") or "",
            "level": lv.get("level"),
            "kind": lv.get("kind", "entity"),
        }

    # 一致性校验：resource_types.yaml 的 type==hierarchy 声明应与 biz_hierarchy 链一致
    if result:
        resource_types_data = _load_schema_yaml("resource_types.yaml")
        for rt, rt_cfg in resource_types_data.items():
            if not isinstance(rt_cfg, dict) or rt_cfg.get("type") != "hierarchy":
                continue
            meta_rt = result.get(rt)
            yaml_parent = rt_cfg.get("parent") or ""
            if meta_rt is None:
                logger.warning(
                    "[FR-014 层级漂移] resource_types.yaml 声明 %s 为 hierarchy，"
                    "但 hierarchies.yaml biz_hierarchy 无此对象（以 hierarchies.yaml 为准）", rt)
            elif (meta_rt.get("parent") or "") != yaml_parent:
                logger.warning(
                    "[FR-014 层级漂移] %s parent 不一致: resource_types.yaml=%s vs "
                    "hierarchies.yaml=%s（以 hierarchies.yaml 为准）",
                    rt, yaml_parent, meta_rt.get("parent"))
    return result
...
def _build_role_matrices(permission_set_id: int,
    ...
    [INCIDENT-2026-09-02 元数据未就绪] 各阶段 try/except 隔离：单段失败仅影响该段，
    不再让整个矩阵返回 null 导致前端死文字。常见漂移：permission_rules 缺
    permission_set_id 列（v070-v077 rename 未覆盖），data_permission_rules 部分环境
    也缺该列。两段 SQL 都退化"读全部"语义（忽略 permission_set_id 过滤），不阻断页面。
    """
    if not _data_source:
        return None
    try:
        ...
        try:
            ...
            logger.warning(f"[P2-Matrix-03] include 阶段失败（permission_set_id={permission_set_id}）: {e}")
        ...
        try:
            ...
            logger.warning(f"[P2-Matrix-03] auto 阶段查询权限集失败（permission_set_id={permission_set_id}）: {e}")
        ...
        try:
            ...
            logger.warning(f"[P2-Matrix-03] auto 阶段查询菜单失败（permission_set_id={permission_set_id}）: {e}")
        ...
        try:
            ...
            logger.warning(f"[P2-Matrix-03] derived 阶段失败（permission_set_id={permission_set_id}）: {e}")

        # ---- 3a. [DEPRECATED Spec16 Phase7 r3 2026-09-02] association 资源从端点派生
        # 原逻辑（v42 2026-08-27）: 关系资源权限跟随端点 (version/domain/sub_domain)
        # 业界共识（SAP/Palantir/Salesforce Junction Object）: 关联资源权限跟随端点
        # Spec16 Plan D 决策: relationship 不属于矩阵语义（两端 BO 权限并集），
        #   yaml 已删 relationship 声明，矩阵不应再展示 relationship 行/列来源。
        #   若业务确有跨端点授权需求，由两端 BO 各自的 dimension scope 自然并集生效。
        # 保留 ASSOCIATION_ENDPOINTS 常量以兼容其他代码引用（如 L1314 is_assoc_derive），
        #   但循环体已清空（不向 derived 写入），关系行不再出现在矩阵中。
        ...
        # ---- 3b. [v42 2026-08-27 → Spec16 Phase7 r3 2026-09-02] subordinate 资源 owner-auto-grant 派生
        # 业界共识（Salesforce Lookup + 本项目 inherit_owner: true 一致）：
        #   audit_log 的创建者自动获得该资源的 read+update 权限（owner 自治）。
        # [r3 2026-09-02] 移除 annotation：annotation 是 BO 上的备注（不是独立资源），
        #   yaml 已删 annotation 声明，sources_detail 不再输出 annotation 的 owner_auto 来源。
        ...
        SUBORDINATE_OWNER_AUTO = ('audit_log',)
        ...
        # [INCIDENT-2026-09-02] 两张规则表都做"列存在性"探测：permission_rules 是 v070 之前的
        # 遗留全局表，未参与 rename；data_permission_rules 部分环境也缺 permission_set_id。
        # 任一缺失则只读 is_denied=1 行（不按 ps 过滤），不影响其他阶段结果。
        exclude: Dict[tuple, str] = {}

        def _has_column(table: str, col: str) -> bool:
            try:
                cur = ds.execute(
                    f"SELECT name FROM pragma_table_info('{table}') WHERE name = ?",
                    [col],
                )
        ...
        # data_permission_rules.exclude
        try:
            if _has_column("data_permission_rules", "permission_set_id"):
                cursor = ds.execute(
                ...
                # 列缺失 → 退化"全量 Deny 规则"语义（不阻断）
                logger.warning("[P2-Matrix-03] data_permission_rules 缺 permission_set_id 列，退化为全量 deny 规则")
                cursor = ds.execute(
                    "SELECT resource_type, permission_level FROM data_permission_rules "
                    "WHERE is_denied = 1",
                )
            ...
            logger.warning(f"[P2-Matrix-03] exclude 阶段 data_permission_rules 查询失败（permission_set_id={permission_set_id}）: {e}")

        # permission_rules.exclude
        try:
            if _has_column("permission_rules", "permission_set_id"):
                cursor = ds.execute(
                ...
                # [INCIDENT-2026-09-02] permission_rules 是 v070 之前的遗留全局表，缺 ps 列是常见漂移
                logger.warning("[P2-Matrix-03] permission_rules 缺 permission_set_id 列，退化为全量 deny 规则（v070-v077 rename 未覆盖）")
                cursor = ds.execute(
                    "SELECT resource_type, permission_level FROM permission_rules "
                    "WHERE is_denied = 1",
                )
            ...
            logger.warning(f"[P2-Matrix-03] exclude 阶段 permission_rules 查询失败（permission_set_id={permission_set_id}）: {e}")
        ...
        # [FIX Spec16 Phase7 2026-09-02] 白名单兜底: 数据库历史漂移可能引入 rt (如
        #   user_group/role_dimension_scope 等 v070 之前的旧表名残留), yaml 已声明是
        #   唯一权威源. _ALLOWED_EXTRA_RTS 是 yaml 之外的"真业务 BO"白名单(本 PR yaml 已补声明),
        #   即使 cleanup 脚本未及时跑也不会让脏数据进矩阵. 其他漂移 rt 一律过滤.
        # [FIX Spec16 Phase7 r2 2026-09-02] 移除 menu/enum_value/filter_variant 3 个
        #   系统/用户级 rt (走专用管理界面, 不进功能权限矩阵). 只保留 5 个真业务 BO.
        # [FIX Spec16 Phase7 r3 2026-09-02] yaml 删除 relationship/annotation 声明:
        #   relationship = 两端 BO 权限并集, annotation = 备注挂 BO, 都不属于矩阵语义.
        #   兜底白名单不变 (本就不含这2个).
        _ALLOWED_EXTRA_RTS = frozenset({
            # 5 个真业务 BO (yaml 已补声明, 此处冗余兜底)
            'scheduled_task', 'task_queue', 'task_execution',
            'ai_async_task', 'enum_type',
            # permission_set 是业务对象但 yaml 不声明 (身份/管理类, 走专用界面)
            #   菜单 permission_set-list 引用, 矩阵里不需要展示
            # 不再保留: menu / enum_value / filter_variant (走专用界面)
        })
        ...
        # [FIX Spec16 Phase7 2026-09-02] 严格过滤: 只允许 declared_rts ∪ _ALLOWED_EXTRA_RTS
        allowed_rts = declared_rts | _ALLOWED_EXTRA_RTS
        extra_rts = sorted(merged_rts - allowed_rts)
...
def get_permission_meta():
    ...
    try:
        ...
        # [FIX Spec16 Phase7 r3 2026-09-02] 菜单里引用的 bo 仅当在 yaml SSOT 声明
        # 或 _ALLOWED_EXTRA_RTS 兜底白名单内时, 才写 label. 否则视为"漂移" (如
        ...
        # [Spec 21 FR-003 2026-09-12] action_meta = {action_id: {action_type, name, yaml_id}}
        # 来源: StandardActionLoader 加载的 _standard_actions.yaml
        # 作用: 前端按 action_type 把矩阵列分块渲染（CRUD / 批量 / 业务）
        # 兼容: StandardActionLoader 加载失败 → action_meta 空 → 前端走 crud 兜底
        ...
        try:
            ...
            for ma in StandardActionLoader.get_actions():
                # yaml id 例: 'crud_create', 'export', 'import'
                # 前端 action suffix 例: 'create', 'export' （去掉 crud_ 前缀）
                yaml_id = ma.id
                suffix = yaml_id.replace('crud_', '')
                action_type_value = (
                    ma.action_type.value
                    if hasattr(ma.action_type, 'value')
                    else str(ma.action_type)
                )
                # [Spec 21 PM 反馈 2026-09-12] instance_scope 字段
                #   来自 MetaAction.instance_scope (InstanceScope 枚举)
                #   决定前端列头副标签: object→[对象级], instance→[实例级]
                instance_scope_value = (
                    ma.instance_scope.value
                    if hasattr(ma.instance_scope, 'value')
                    else str(ma.instance_scope)
                )
                action_meta[suffix] = {
                    'action_type': action_type_value,
                    'name': ma.name,
                    'yaml_id': yaml_id,
                    'instance_scope': instance_scope_value,
                }
        except Exception as e:
        ...
        # [Spec 19 FR-012] {resource_type: {action: 灰化原因}}，说明与限制同源（tooltip 用）
        action_hints = _build_action_hints()
        # [Spec 19 FR-014 2026-09-06] 类型层树形数据源（SSOT: hierarchies.yaml）
        resource_hierarchy = _build_resource_hierarchy()
        ...
        return jsonify({
            ...
            "data": {
                ...
                # [Spec 21 FR-003 2026-09-12] 动作元数据（含 action_type），前端按类型分块渲染
                "action_meta": action_meta,
                # [P2-Matrix-01 / A5] {resource_type: [可授权动作]}，前端灰化禁选依据
                "resource_action_matrix": resource_action_matrix,
                # [Spec 19 FR-012] {resource_type: {action: 灰化原因}}，灰化格 tooltip 展示
                "action_hints": action_hints,
                # [Spec 19 FR-014 2026-09-06] {rt: {parent, level, kind}} 类型层树形数据源
                # （SSOT: hierarchies.yaml biz_hierarchy；非层级资源不在其中 → 前端顶层行）
                ...
                # [Spec 21 FR-005 2026-09-12] OWD 只读列表，下发到前端 OWD 卡
                # 来源: object_owd 表（已建表，迁移脚本 add_object_owd_2026.py）
                # 兼容: 表无数据/失败 → 返回 []，前端显示"未配置 OWD"
                "object_owd": _load_object_owd_list(),
            }
        }), 200
    except Exception as e:
...
def _query_dimension_codes(ds, dimension_id: str, search: str, page: int, page_size: int,
    ...
    # [Spec 20 v3 2026-09-12] 父对象中文名 (动态「跨{parent_label}」标签):
    #   domain→版本 / version→产品 / book→图书馆(若注册)。根对象 (product) 无父 → None,
    #   前端回落中性「按业务键」。label 来源 _RESOURCE_TYPE_LABELS (SSOT 中文兜底)。
    parent_label: Optional[str] = None
    parent_info = _PARENT_INFO_MAP.get(dimension_id)
    if parent_info:
        parent_label = _RESOURCE_TYPE_LABELS.get(parent_info[0], parent_info[0])

    zero = {"enabled": True, "parent_label": parent_label, "codes": [], "pagination": {
    ...
    try:
        ...
        cols = []
    if code_field not in cols:
        # 表无注册业务键字段 → 无业务键锚定语义 (前端隐藏跨版本 Tab)
        return zero

    where = f"main.{code_field} IS NOT NULL AND main.{code_field} != ''"
    params: list = []
    if search:
        where += f" AND main.{code_field} LIKE ?"
        params.append(f"%{search}%")
    if scoped_ids is not None:
        if not scoped_ids:
            return zero  # 空 scope: 强一致 0 条可见
        ph = ','.join('?' * len(scoped_ids))
        where += f" AND main.id IN ({ph})"
        params.extend(int(i) for i in scoped_ids)

    total_count = ds.execute(
        f"SELECT COUNT(DISTINCT main.{code_field}) FROM {table_name} main WHERE {where}", params
    ).fetchone()[0]

    rows = ds.execute(
        f"SELECT main.{code_field} AS code, COUNT(*) AS cnt, MIN(main.id) AS min_id "
        f"FROM {table_name} main WHERE {where} "
        f"GROUP BY main.{code_field} ORDER BY main.{code_field} LIMIT ? OFFSET ?",
        params + [page_size, (page - 1) * page_size],
    ).fetchall()

    # sample_name: 每个 code 取最小 id 实例的显示名
    display_field = DISPLAY_FIELD_MAP.get(dimension_id, "name")
    if display_field not in cols:
        display_field = "name" if "name" in cols else code_field
    names: Dict[int, str] = {}
    min_ids = [r[2] for r in rows if r[2] is not None]
    if min_ids:
        ph = ','.join('?' * len(min_ids))
        for rid, nm in ds.execute(
            f"SELECT id, {display_field} FROM {table_name} WHERE id IN ({ph})", min_ids
        ).fetchall():
            names[rid] = str(nm) if nm is not None else ""

    codes = [
        {"code": r[0], "resolved_count": r[1], "sample_name": names.get(r[2], "")}
        for r in rows
    ]
    return {
        "enabled": True,
...
@permission_dimension_bp.route(
    "/<string:dimension_id>/codes", methods=["GET"]
)
@_login_required
def get_dimension_codes(dimension_id: str):
    """获取维度业务键 code 列表 (Spec 20 Task 9-B: Rule Builder FK picker 跨版本 Tab)

    返回: [{code, resolved_count, sample_name}] — resolved_count 为该 code
    当前命中的实例数 (跨版本/跨父对象)。开关 DIM_SCOPE_BIZKEY_ENABLED=0 时
    ...
    try:
        ...
        # 用户 dim scope 过滤 (与 instances 端点同语义, 命中数按可见实例计)
        ...
        logger.error(f"获取维度业务键列表失败 [dimension_id={dimension_id}]: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
# [FIX 2026-09-04] resource-action-matrix 读写契约统一到 /api/v1/permission-sets 前缀。
#   FE permissionService.saveResourceActionMatrix 与兄弟端点(menu-permissions/dimension-scopes/
#   unified-permissions) 全走 /api/v1/permission-sets/<id>/...；此前该矩阵仅注册在 roles_bp
#   (/api/v1/roles/<id>/...) → FE 保存/读取矩阵一律 404→500。新增同前缀 bp 挂 PUT 别名 + GET。
ps_matrix_bp = Blueprint("permission_set_matrix", __name__, url_prefix="/api/v1/permission-sets")
...
@ps_matrix_bp.route("/<int:permission_set_id>/resource-action-matrix", methods=["PUT"])
...
@ps_matrix_bp.route("/<int:permission_set_id>/resource-action-matrix", methods=["GET"])
@_login_required
def get_resource_action_matrix(permission_set_id: int):
    """
    读取角色「资源 × 动作」矩阵（与 PUT 写路径同契约、同数据源）

    [FIX 2026-09-04] 此前该矩阵仅注册 /api/v1/roles/<id>/...（roles_bp）且只有 PUT；
    前端/兄弟端点统一使用 /api/v1/permission-sets/<id>/... 前缀，读取请求会 404→500。
    本 GET 在 /permission-sets 前缀补齐读路径：data.role_resource_action_matrix 结构与
    权限配置元数据(meta)返回一致（_build_role_matrices），便于前端矩阵只读/回显复用。

    返回：
      data.permission_set_id
      data.role_resource_action_matrix  （构建失败为 null，不阻断，与 meta 行为一致）
    """
    try:
        if not _data_source:
            _get_engine()
        matrix = _build_role_matrices(permission_set_id, columns=_matrix_action_columns())
        return jsonify({
            "success": True,
            "data": {
                "permission_set_id": permission_set_id,
                "role_resource_action_matrix": matrix,
            },
        })
    except Exception as e:
        logger.error(f"获取资源×动作矩阵失败 [permission_set_id={permission_set_id}]: {e}")
        return jsonify({"success": False, "message": str(e)}), 500
...
def register_permission_dimension_apis(app):
    ...
    app.register_blueprint(ps_matrix_bp)
