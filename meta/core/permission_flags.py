...
[Plan D 2026-08-29 灰度清理]
  Plan B 引入的 permission_set_refactor_* / org_function_panel_enabled 三个
  flag 已删除: Plan C/D 完成验证后, 默认行为即新 schema, 不再需要灰度开关.
  旧表 (_v1_backup) 与旧 SQL 路径保留作为兜底, 不再通过 flag 切换.
"""
import os
