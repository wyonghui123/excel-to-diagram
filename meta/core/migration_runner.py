...
class MigrationRunner:
    ...
    def is_migration_executed(self, migration_name: str, status: str = "SUCCESS") -> bool:
        """检查迁移是否已执行

        [P0-1 2026-09-14 prod 彩排修复] 默认校验 status='SUCCESS'。
        原实现只查记录存在性 → FAILED 记录被误判为已执行而永久跳过，
        且文件修改后触发 checksum mismatch 拒绝重跑 → 迁移卡死。
        对无 status 列的旧库（P0 阶段 4 字段结构）自动退回存在性检查。
        """
        sql = "SELECT 1 FROM schema_migrations WHERE migration_name = ?"
        params = [migration_name]
        if status:
            if self._schema_migrations_has_status_col():
                sql += " AND status = ?"
                params.append(status)
        cursor = self.data_source.execute(sql, params)
        return cursor.fetchone() is not None

    def _schema_migrations_has_status_col(self) -> bool:
        """探测 schema_migrations 是否已有 status 列 (结果缓存)"""
        if getattr(self, '_sm_status_col_cache', None) is None:
            try:
                cols = {r[1] for r in self.data_source.execute(
                    "PRAGMA table_info(schema_migrations)").fetchall()}
                self._sm_status_col_cache = 'status' in cols
            except Exception:
                self._sm_status_col_cache = False
        return self._sm_status_col_cache

    def is_migration_failed(self, migration_name: str) -> bool:
        """[P0-1] 检查迁移是否处于 FAILED 状态 (供运维工具/部署前检查使用)"""
        if not self._schema_migrations_has_status_col():
            return False
        cursor = self.data_source.execute(
            "SELECT 1 FROM schema_migrations WHERE migration_name = ? AND status = 'FAILED'",
            (migration_name,))
        return cursor.fetchone() is not None
    ...
    def run_migration(self, migration_name: str) -> bool:
        ...
        # 1.5 [P0-1 2026-09-14] 清理同名 FAILED 残留行:
        # 走到这里说明该迁移未成功执行过; 若存在 FAILED 旧行, 其 checksum 是
        # 修改前文件的, 重跑成功 INSERT SUCCESS 后会双行共存,
        # _get_recorded_checksum 的 fetchone 可能取到旧行 → 后续误报 mismatch。
        if self._schema_migrations_has_status_col():
            try:
                self.data_source.execute(
                    "DELETE FROM schema_migrations WHERE migration_name = ? AND status = 'FAILED'",
                    (migration_name,))
                if not self.data_source.in_transaction:
                    self.data_source.commit()
            except Exception as e:
                logger.debug("cleanup FAILED row failed for %s: %s", migration_name, e)
...
def _cli_main():
    ...
    # [2026-09-14 P0-2] 前端-only 部署解耦
    parser.add_argument('--frontend-only', action='store_true',
                        help='[P0-2] 拒绝执行: zip 在 N 天内没动 meta/core 或 meta/migrations (前端 dist-only 部署)')
    parser.add_argument('--since-hours', type=int, default=168,
                        help='[P0-2] 与 --frontend-only 配合: 多久内的修改算"有 backend 改动" (默认 168=7天)')
    parser.add_argument('--zip-path', default=None,
                        help='[P0-2] 与 --frontend-only 配合: 用于探查 mtime 的 zip 路径')
    ...
    # [2026-09-14 P0-2] 前端-only 部署解耦守卫
    # 防止: delta-7 staging 部署时 frontend dist 改动 → 部署脚本又跑 runner → 全链重跑 v070-v083
    # 触发 v071 守卫 bug. 规则: zip mtime N 小时内没有 meta/core 或 meta/migrations 改动则拒绝.
    if args.frontend_only:
        from datetime import datetime, timedelta
        zip_path = args.zip_path or os.environ.get('DEPLOY_ZIP_PATH')
        if not zip_path or not os.path.exists(zip_path):
            print("[P0-2] --frontend-only 需要 --zip-path (或 env DEPLOY_ZIP_PATH)", file=sys.stderr)
            print("       找不到 zip 文件, 拒绝执行以保护 DB", file=sys.stderr)
            return 2

        # 探查 zip 内 backend / migrations 的最新 mtime
        import zipfile
        since_cutoff = time.time() - args.since_hours * 3600
        latest_backend_mtime = 0.0
        latest_frontend_mtime = 0.0
        backend_changed = []
        try:
            with zipfile.ZipFile(zip_path, 'r') as zf:
                for info in zf.infolist():
                    # 转换 ZipDateTime (1980起) 到 epoch
                    ts = info.date_time
                    epoch = (datetime(ts[0], ts[1], ts[2], ts[3], ts[4], ts[5]) - datetime(1970, 1, 1)).total_seconds()
                    if info.filename.startswith('meta/core/') or info.filename.startswith('meta/migrations/'):
                        if epoch > latest_backend_mtime:
                            latest_backend_mtime = epoch
                        if epoch >= since_cutoff:
                            backend_changed.append(info.filename)
                    elif info.filename.startswith('frontend_dist_files/') or info.filename.startswith('dist/'):
                        if epoch > latest_frontend_mtime:
                            latest_frontend_mtime = epoch
        except zipfile.BadZipFile:
            print("[P0-2] zip 文件损坏: %s" % zip_path, file=sys.stderr)
            return 2

        zip_age_h = (time.time() - latest_backend_mtime) / 3600.0 if latest_backend_mtime else float('inf')
        frontend_age_h = (time.time() - latest_frontend_mtime) / 3600.0 if latest_frontend_mtime else float('inf')

        print("[P0-2] frontend-only 检查:")
        print("       zip = %s" % zip_path)
        print("       latest meta/core|migrations mtime = %.1fh ago (内 %d 个文件命中 %dh 窗口)" % (
            zip_age_h, len(backend_changed), args.since_hours))
        print("       latest frontend_dist_files mtime = %.1fh ago" % frontend_age_h)
        if backend_changed:
            print("       [!]  backend 文件命中窗口: %s" % ', '.join(backend_changed[:5]))
            print("       → 这是 backend 改动, 强制执行 migration runner (frontend-only 模式被覆盖)")
        else:
            print("       → 纯前端改动, 跳过 migration runner (delta-7 v071 bug 防御)")
            print("       NO PENDING MIGRATIONS: SKIP (frontend-only)")
            return 0
