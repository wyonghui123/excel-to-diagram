...
export const useDiagramConfigStore = defineStore('diagramConfig', () => {
  ...
  function resetExpandState() {
    ...
    _dbgLog('[resetExpandState] 重置展开状态: expandLevelUserSet=false, groupManualSet=false (当前 expandLevel=' + expandLevel.value + ')')
  }
  ...
  function applyDefaultColorConfig(groupBy, highlight) {
    ...
    _dbgLog('[applyDefaultColorConfig] colorGroupBy: ' + prevGroupBy + ' → ' + colorGroupBy.value + ' | centerScopeHighlight: ' + prevHighlight + ' → ' + centerScopeHighlight.value)
  }
