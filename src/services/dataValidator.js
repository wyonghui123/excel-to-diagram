...
export function validateData(rawData, previewData) {
  if (!rawData) {
    return { items: [], summary: { total: 0, errors: 0, warnings: 0, infos: 0 } }
  }
  ...
  // 6. 校验领域和子领域名称是否相同
  items.push(...validateDomainSubDomainNames(rawData, previewData));
...
export function validateDomainSubDomainNames(rawData, previewData) {
  ...
  rawData.serviceComponentData?.forEach((item, index) => {
    ...
    if (domain && subDomain) {
      ...
      domainSubDomainPairs.get(key).smCodes.push(smCode || smName);
    }
  });
