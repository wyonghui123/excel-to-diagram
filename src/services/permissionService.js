...
// 注意: 角色/权限主体 (role/org_member) 不在此列表 —— 其权限分配走专用路径
// [Spec 19 M2-8 2026-09-05] user/org 已作为「委托授权行」加入列表（FR-001）
export const RESOURCE_LABELS = {
  ...
  annotation: '标注',
  audit_log: '审计日志',
  // [Spec 19 M2-8 2026-09-05 FR-001] 委托授权行（后端 meta 优先，此为兜底）
  org: '组织',
  user: '用户',
}
...
//   这些 rt 不参与功能权限矩阵资源列表
// [Spec 19 M2-8 2026-09-05 FR-001] org/user 移出主体白名单，作为「委托授权行」进矩阵
//   （与后端 permission_dimension_api.py 2026-09-05 修改同步）；org_member 同为主体
export const IDENTITY_RESOURCE_TYPES = new Set(['role', 'org_member'])
...
/**
 * [Spec 20 Task 9-B] 加载维度业务键 code 列表（FK picker 跨版本 Tab 数据源）
 *
 * 返回 [{code, resolved_count, sample_name}] — resolved_count 为该 code
 * 当前命中实例数（跨版本/跨父对象）。开关关闭时 data.enabled=false（前端隐藏 Tab）。
 * @param {string} dimensionId
 * @param {object} [params] - page, page_size, search
 */
export async function loadDimensionCodes(dimensionId, params = {}) {
  return await apiV2.get(`/bo/permission_dimension/${dimensionId}/codes`, { params })
}
...
/**
 * [Spec 20] 业务键锚点解析预览: 返回每个业务键当前命中的实例 (跨版本)
 * @param {number} roleId
 * @param {Array<{dimension_code: string, dimension_values: string[]}>} scopes
 * @returns {Promise<object>} data: { [dim]: { [code]: { resolved_count, instances[], warning? } } }
 */
export async function resolveDimensionScopePreview(roleId, scopes) {
  return await apiV1.post(`/permission-sets/${roleId}/dimension-scopes/resolve-preview`, { scopes })
}
