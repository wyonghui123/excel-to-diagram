...
class BOService extends BaseService {
  ...
  // [Spec 22 FR-005 2026-09-13] 列出对象类型所有 state_transition 引用的 instance 级 action_ref
  // 前端 PermissionConfigPanel 据此把 state_transition 通用 action 合并到 supportedActions
  // 自动在矩阵「更多动作」列展示，可被权限集勾选授权
  async getStateTransitionActions(objectType) {
    if (!objectType) {
      return { success: false, message: 'MISSING_OBJECT_TYPE', data: [] }
    }
    return this._request('GET', `/bo/${objectType}/state-transition-actions`)
  }
