  ...
  const apply = (list) => {
    ;(list || []).forEach(g => {
      ...
      if (src && src.collapsed !== undefined && src.collapsed !== g.collapsed) {
        debugMode.debugTrace('[SYNC] syncVisibilityToChartConfig: syncing collapsed for ' + (g.title || g.elementCode || g.id) + ' from ' + g.collapsed + ' to ' + src.collapsed)
...
// ============================================================
// [VIS-RESET-SYNC 2026-09-03] 用户显式切换"全局展开层级" → 重置为全显 (用户确认的语义)。
//   切层级瞬间把"面板树 + store"的用户隐藏位同步清空 (非 ELK 系统自动分组),
//   保证图例图标与图表在同一时刻一致。仅用户显式选择 (expandLevelUserSet=true) 触发;
//   系统默认展开 (setDefaultExpandLevel) 不重置用户隐藏 (保留 2026-08-14 "reload 后
//   主动隐藏不丢失" 语义)。setTimeout(...,0) 让本轮 resetAll 排在 setExpandLevel 自身的
//   updateLayoutControlConfig 之后, 覆盖其写入的旧隐藏位 (一次收敛即可, 实测验证)。
watch(
  () => configStore.expandLevel,
  (level) => {
    if (!configStore.expandLevelUserSet) return
    if (!level || !chartConfig.layoutControl) return
    const resetAll = () => {
      if (!chartConfig.layoutControl) return
      if (Array.isArray(chartConfig.layoutControl.groups)) {
        expandGroupsToLevel(chartConfig.layoutControl.groups, level, { resetVisible: true })
      }
      const st = configStore.layoutControlConfig
      if (st && Array.isArray(st.groups)) {
        const cfg = JSON.parse(JSON.stringify(st))
        expandGroupsToLevel(cfg.groups, level, { resetVisible: true })
...
async function syncLayoutControlFromDiagramData(force = false, preservedStates = null, genId = null) {
  debugMode.debugLog('[DBG-syncLayout] called force=' + force + ' genId=' + genId + ' chartType=' + chartConfig.chartType + ' stack=' + new Error().stack.split('\n').slice(1,4).join(' < ').slice(0,200))
...
async function reapplyRelationScope() {
  ...
  debugMode.debugLog('[reapplyRelationScope] 关系范围变更触发轻量刷新: relationIds=' + JSON.stringify(props.relationIds || []))
