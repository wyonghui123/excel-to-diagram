<template>
  <div class="menu-permission-matrix" :class="{ 'mpm--readonly': !props.editing, 'mpm--editing': props.editing }">
    ...
    <div v-else class="menu-list" :style="menuListStyle">
      <!-- [Phase 5] 树形菜单视图（扁平 v-for，按 depth 缩进） -->
      <div
        v-for="node in pagedFlatTree"
        ...
        <div class="menu-card-header">
          ...
          <div class="menu-title-area">
            ...
            <!-- [2026-08-30 只读态] 菜单来源权限集（来源 ps 集合，仿资源矩阵「来源：」标签）
                 [Spec 21 PM 反馈 2026-09-12 第七次] 回到嵌套在 menu-title-area 内，
                 与菜单名同行展示（不再换行到第二行）。窄 drawer 下用 ellipsis 截断，
                 tooltip 显示完整来源名（hover 可看全）。 -->
            <span
      ...
      <!-- [Spec 21 PM 反馈 2026-09-12 第四次] 分页器（pageSize>0 且总数超页大小时显示） -->
      <el-pagination
        v-if="showPagination"
        class="menu-list-pagination"
...
const props = defineProps<{
  ...
  editing?: boolean
  /** [Spec 21 PM 反馈 2026-09-12 第四次] 每页显示数量（0/不传=不分页，全展开）
   *  用于聚合视图（组织/角色/用户权限预览）限制菜单列表高度，避免挤掉资源矩阵 */
  pageSize?: number
  /** [Spec 21 PM 反馈 2026-09-12 第四次] 菜单列表最大高度（CSS 像素），超出滚动
   *  默认 0=不限高。聚合视图传 240 ~ 320 让矩阵可见 */
  maxHeight?: number
}>()
...
// [Spec 21 PM 反馈 2026-09-12 第四次] 分页状态（pageSize>0 时启用）
...
// [Spec 21 PM 反馈 2026-09-12 第四次] 分页切片（pageSize>0 时启用）
const pagedFlatTree = computed<MenuTreeNode[]>(() => {
  const size = props.pageSize && props.pageSize > 0 ? props.pageSize : 0
  if (size <= 0) return flatTree.value
  const start = (currentPage.value - 1) * size
  return flatTree.value.slice(start, start + size)
})

const showPagination = computed(() => (props.pageSize ?? 0) > 0 && flatTree.value.length > (props.pageSize ?? 0))

// 当折叠状态变化导致总页数变少时，自动回退到最后一页
watch([flatTree, () => props.pageSize], () => {
  if (!showPagination.value) return
  const totalPages = Math.ceil(flatTree.value.length / (props.pageSize ?? 1))
  if (currentPage.value > totalPages) currentPage.value = totalPages
})

// [Spec 21 PM 反馈 2026-09-12 第四次] 菜单列表内联样式（maxHeight>0 时限制高度 + 滚动）
const menuListStyle = computed(() => {
  if (props.maxHeight && props.maxHeight > 0) {
    return { maxHeight: `${props.maxHeight}px`, overflowY: 'auto' as const }
  }
  return {}
})
...
/* [Spec 21 PM 反馈 2026-09-12 第四次] 分页器样式（与列头副标签灰色调一致） */
...
.menu-card {
  ...
  /* [Spec 21 PM 反馈 2026-09-12 第六次] 移除 overflow: hidden —— 否则第二行的
     来源 tag 文字会被圆角裁切掉（截图：来源：TFSTR88-RFADSCM、采购管理领域编辑
     被截断显示成「来源：TFSTR88-RFADSCM、采购管理领域编辑」后内容看不到）。
     改用 border-radius 视觉效果保留，无 overflow 裁切。 */
...
.menu-card-header {
  display: flex;
  align-items: center;
  /* [Spec 21 PM 反馈 2026-09-12 第七次] 单行展示：来源 tag 与菜单名同行；
     窄 drawer 下用 ellipsis 截断（.menu-source-label），tooltip 显示完整内容 */
...
.menu-name {
  ...
  white-space: nowrap;
  /* [Spec 21 PM 反馈 2026-09-12 第七次] 单行展示，菜单名超出可省略 */
  overflow: hidden;
...
.menu-source-label {
  font-size: var(--font-size-xs);
  color: var(--color-text-tertiary);
  /* [Spec 21 PM 反馈 2026-09-12 第七次] 单行 + ellipsis，窄 drawer 下截断；
     hover tooltip 显示完整来源名 */
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
}
...
.menu-badges {
  ...
  gap: var(--spacing-xs);
  flex-shrink: 0;
  /* [Spec 21 PM 反馈 2026-09-12 第五次] 来源 tag 换到第二行后，badge 与菜单名同行，无需特殊处理 */
}
