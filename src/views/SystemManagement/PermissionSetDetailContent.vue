  ...
  <div class="permission-set-detail">
    ...
    >
      ...
      >
        ...
        <!-- Custom Slot: 数据范围 -->
        <template #section-dimension_scopes>
          <div style="padding: 16px;">
            <p v-if="loadingDimensionScopes">加载数据范围...</p>
            <p v-else-if="dimensionScopes.length === 0" style="color: var(--color-text-tertiary)">
              暂无数据范围配置（全局可见）
            </p>
            <div v-else>
              <div
                v-for="scope in dimensionScopes"
                :key="scope.id"
                style="padding:12px;border:1px solid var(--color-border-light);border-radius:var(--radius-md);margin-bottom:8px"
              >
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                  <span style="font-weight:600">{{ scope.dimension_code }}</span>
                  <el-tag :type="scope.scope_mode === 'exclude' ? 'danger' : 'success'" size="small">
                    {{ scope.scope_mode === 'exclude' ? '排除' : '包含' }}
                  </el-tag>
                  <el-tag v-if="scope.inherit_children" type="info" size="small">继承子级</el-tag>
                </div>
                <div style="display:flex;flex-wrap:wrap;gap:6px">
                  <el-tag
                    v-for="v in scope.dimension_values"
                    :key="v.id || v"
                    size="small"
                    effect="plain"
                  >
                    {{ v.name || v.code || v.id || v }}
                  </el-tag>
                </div>
              </div>
            </div>
...
// [Spec 21 PM 反馈 2026-09-12 第十次] ObjectPageHeader 转发 StateTransitionButtons 的 refresh 事件
// 启用/停用后只刷新当前 is_active 字段，避免重载整个 permissionSet 引发编辑态抖动
async function handleRefresh(payload: any) {
  if (!permissionSetId.value) return
  try {
    const result = await boService.read('permission_set', permissionSetId.value)
    if (result.success) {
    ...
    console.error('Failed to refresh permission set after state transition:', error)
  }
}
