...
export function useDiagramData() {
  ...
  const selectedStats = computed(() => {
    ...
    if (typeof window !== 'undefined' && window.__diagDebug) {
      try {
        ...
        _dbgTrace('centerBoCodes 数量:', centerBoCodes.value?.size, '前 10:', Array.from(centerBoCodes.value || []).slice(0, 10))
        _dbgTrace('externalBoCodes 数量:', externalBoCodes.value?.size, '内容:', Array.from(externalBoCodes.value || []))
        _dbgTrace('selectedBoCodes (并集) 数量:', selectedBoCodes.size)
        _dbgTrace('previewData.businessObjects 总数:', pd.businessObjects?.length)
        _dbgTrace('previewData.relationships 总数:', pd.relationships?.length)
        _dbgTrace('previewData.domainProducts 顶层数:', pd.domainProducts?.length)
        if (pd.domainProducts?.length) {
          const d0 = pd.domainProducts[0]
          _dbgTrace('  domainProducts[0]:', { name: d0?.name, modules数: d0?.modules?.length })
          if (d0?.modules?.length) {
            const sd0 = d0.modules[0]
            _dbgTrace('  domainProducts[0].modules[0]:', { name: sd0?.name, submodules数: sd0?.submodules?.length })
            if (sd0?.submodules?.length) {
              const sm0 = sd0.submodules[0]
              _dbgTrace('  domainProducts[0].modules[0].submodules[0]:', { name: sm0?.name, BO数: sm0?.businessObjects?.length })
            }
          }
        }
        _dbgTrace('centerStats  :', centerStats)
        _dbgTrace('totalStats   :', totalStats)
        _dbgTrace('incremental  :', incrementalStats)
        _dbgTrace('差值对比 (total - center):', {
          domains: totalStats.domains - centerStats.domains,
          subDomains: totalStats.subDomains - centerStats.subDomains,
          serviceModules: totalStats.serviceModules - centerStats.serviceModules,
          businessObjects: totalStats.businessObjects - centerStats.businessObjects,
  ...
  const loadPresetsFromStorage = () => {
    try {
      ...
      if (stored) {
        ...
        _dbgLog('[useDiagramData] Loaded presets from storage:', centerScopePresets.value.length)
      }
    } catch (e) {
  ...
  const savePresetsToStorage = () => {
    try {
      ...
      _dbgLog('[useDiagramData] Saved presets to storage:', centerScopePresets.value.length)
    } catch (e) {
  ...
  const generateDiagram = () => {
    ...
    if (currentDomainCount !== lastFilteredDomainCount.value) {
      ...
      _dbgLog('[generateDiagram] 领域数变化: ' + lastFilteredDomainCount.value + ' → ' + currentDomainCount +
    ...
    if (chartType.value === 'serviceModule') {
      ...
      if (!useLegacy) {
        ...
        if (configStore.useUnifiedRenderer) {
          ...
          _dbgLog('[UnifiedRenderer] SM mermaid code generated, length:', unifiedMermaidCode.length)
      ...
      if (!useLegacy) {
        ...
        if (configStore.useUnifiedRenderer) {
          ...
          _dbgLog('[UnifiedRenderer] BO mermaid code generated, length:', unifiedMermaidCode.length)
  ...
  const deleteCenterScopePreset = (presetId) => {
    ...
    savePresetsToStorage() // 保存到 localStorage
    _dbgLog('[useDiagramData] 已删除预设:', presetId)
    return true
  }
      ...
      if (missingBoCodes.length > 0) {
        try {
          ...
          if (missingRes.success && missingRes.data?.items) {
            ...
            _dbgLog(`[applyRelationScope] 补全 ${converted.length} 个缺失 BO, ${extraDomainProducts.length} 个缺失 domain`)
          }
        } catch (err) {
