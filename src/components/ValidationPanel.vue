  ...
  <div class="validation-panel" v-if="validationResult && validationResult.items.length > 0">
    ...
    <div class="validation-summary" @click="toggleExpand">
      <div class="summary-left">
        <AppIcon name="enabled" size="md" class="summary-icon" />
        ...
        <!-- 因错误被过滤的统计 -->
        <button 
          class="summary-count filtered filter-btn" 
          v-if="filteredStats && (filteredStats.businessObjects > 0 || filteredStats.relations > 0)"
          @click.stop="showFilteredItems"
          title="点击查看被过滤的对象和关系"
        >
          过滤: {{ filteredStats.businessObjects }}对象/{{ filteredStats.relations }}关系
        </button>
      </div>
      <div class="summary-right">
        <AppIcon :name="isExpanded ? 'chevron-down' : 'chevron-right'" size="sm" class="expand-icon" />
      </div>
    </div>
    ...
    <div class="validation-details" v-show="isExpanded">
      <div class="details-header">
        <div class="filter-tabs">
          ...
          <!-- 按 Sheet 类型过滤 -->
          <button 
            :class="['filter-tab', { active: activeFilter === 'businessObject' }]"
            @click.stop="activeFilter = 'businessObject'"
            v-if="sheetStats.businessObject > 0"
          >
            业务对象 ({{ sheetStats.businessObject }})
          </button>
          <button 
            :class="['filter-tab', { active: activeFilter === 'relationship' }]"
            @click.stop="activeFilter = 'relationship'"
            v-if="sheetStats.relationship > 0"
          >
            关系 ({{ sheetStats.relationship }})
          </button>
        </div>
      </div>
      ...
      <div class="details-list">
        <div
          ...
          :key="item.field || item.key || item.name || index"
        ...
        >
          ...
          <div class="item-suggestion" v-if="item.suggestion">
            <AppIcon name="bulb" size="sm" /> {{ item.suggestion }}
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  props: {
    ...
    filteredStats: {
      type: Object,
      default: () => ({ businessObjects: 0, relations: 0 })
    }
  },
  data() {
    return {
  ...
  computed: {
    ...
    sheetStats() {
      const stats = { businessObject: 0, relationship: 0 };
      if (!this.validationResult?.items) return stats;
      
      this.validationResult.items.forEach(item => {
        if (item.sheet === '业务对象') {
          stats.businessObject++;
    ...
    filteredItems() {
      ...
      // 按 Sheet 类型过滤
      if (this.activeFilter === 'businessObject') {
        return this.validationResult.items.filter(item => item.sheet === '业务对象');
      }
      if (this.activeFilter === 'relationship') {
        return this.validationResult.items.filter(item => item.sheet === '业务对象关系');
      }
...
.summary-count.filtered {
  background: #e6f7ff;
  color: #1890ff;
  border: 1px solid #91d5ff;
}

.summary-count.filtered.filter-btn {
  cursor: pointer;
  transition: all 0.2s;
}

.summary-count.filtered.filter-btn:hover {
  background: #1890ff;
  color: #fff;
}
