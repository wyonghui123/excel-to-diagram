<template>
  <div class="config-app">
    ...
    <header class="app-header">
      <div class="header-left">
        <button class="back-btn" @click="goBack">
          <AppIcon name="arrow-left" size="sm" /> 返回
        </button>
        <h1>系统配置</h1>
      </div>
    </header>
          ...
          <div v-if="currentMenu === 'about'" class="config-section">
            ...
            <div class="about-content">
              <div class="about-logo">
                <AppIcon name="enabled" size="md" class="logo-icon" />
                <h3>ArchWorkspace</h3>
              ...
              <div class="about-info">
                ...
                <div class="feature-list">
                  <div class="feature-item">
                    <span class="feature-icon"><AppIcon name="arrow-right" size="md" /></span>
                    <span>AA图 - 业务对象关系图生成工具</span>
                  </div>
                  <div class="feature-item">
                    <span class="feature-icon"><AppIcon name="enabled" size="md" /></span>
                    <span>智能数据验证</span>
                  </div>
                  <div class="feature-item">
                    <span class="feature-icon"><AppIcon name="lightning" size="md" /></span>
                    <span>AI 辅助关系检查</span>
                  </div>
                </div>
              </div>
    ...
    <div v-if="showSuccess" class="success-toast">
      <span class="success-icon"><AppIcon name="check" size="md" /></span>
      <span>配置已保存</span>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      configMenus: [
        ...
        { key: 'feishu', label: '飞书集成', icon: '[SYMBOL]' },
        { key: 'diagram', label: '图表配置', icon: '[DECORATIVE]' },
        { key: 'validation', label: '数据验证', icon: '[SEARCH]' },
        { key: 'export', label: '导出配置', icon: '[SYMBOL]' },
  ...
  methods: {
    ...
    async resetConfig() {
      const confirmed = await this.message.confirm({ content: '确定要恢复默认配置吗？' })
      if (!confirmed) return
