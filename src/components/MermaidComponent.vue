      ...
      <div v-if="debug.isDebug && debug.debugPanelVisible" class="mermaid-debug-panel">
        ...
        <div class="debug-panel-body">
          <div class="debug-section">
            ...
            <button class="debug-btn" @click="debugVerifyChart">[OK] 一键自检 verifyChart</button>
          </div>
          <div class="debug-section">
...
import { markUplift } from '../composables/useMermaid/layouts/upliftDerivation.js'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // [LEVEL-OVERLAY 2026-09-04] 上次已渲染 SVG 对应的全局展开层级 (setRendering(false) 统一记录).
    //   null 表示尚未完成过渲染; 首个用户折叠操作必然发生在初始渲染完成之后, 故不会误判.
    //   声明位置: 与 foldRenderPending/foldBufferLayer 同区, 在 setRendering 之前.
    //   原因: setRendering 闭包内引用本标识符, 历史上 let 写在 setRendering 之后偶发 HMR 期间
    //   setup 重入或 diag.endRender 异步回调 (Promise.then) 触发 setup 半初始化状态时, 引用
    //   本标识符会撞上 TDZ 抛 ReferenceError; 前置到所有引用者之前是稳妥做法.
    ...
    const setRendering = (v) => {
      ...
      if (!v) {
        ...
        // [LEVEL-OVERLAY 2026-09-04] 记录"渲染结束时的全局展开层级", 供 layoutControlConfig
        //   watch 区分"用户切换展开层级"(store.expandLevel 已变 → 整屏渲染遮罩) 与
        //   "单节点折叠/展开"(expandLevel 不变 → A1 fold 缓冲平滑). 所有渲染出口
        //   (全量/折叠/跳过/错误) 都经 setRendering(false), 此处统一记录即等价于
        //   "上次已渲染 SVG 所对应的展开层级".
        lastAppliedExpandLevel = configStore.expandLevel
      }
    ...
    const renderMermaid = async () => {
      ...
      // [BUG-V037 REVERTED 2026-09-03] 原以为 renderMermaid 入口需清 SVG 隐藏残留,
      //   但用户实测 expandLevel=subDomain 后 legend 隐藏仍失效, 说明清残留不是根因.
      //   expandLevel 切换走 executeGlobalExpand, 该路径调 expandGroupsToLevel({resetVisible:true})
      //   重置 visible=false (不是 BUG-V037 处理的 display:none 残留). 撤回该修改以减少干扰面.
      // clearSvgHidden()  // 已撤回
    ...
    const updateVisibilityOnly = (groups) => {
      ...
      // [FIX BUG-V036 2026-09-03] 应用层隐性契约破坏修复:
      //   collectHiddenState 通过 group._uplift 字段判定"子分组是否被上提为聚合节点 (g.node 而非 g.cluster)".
      //   该字段由 useBusinessObjectSyntax.js:1242 markUplift(virtualGroups) 原地写入, 而 virtualGroups
      //   是 workingGroups (深拷贝). store 里的 layoutControlConfig.groups 始终没有 _uplift 字段.
      //   → updateVisibilityOnly 拿到的 groups 不含 _uplift → collectHiddenState 中 ch._uplift === true
      //     永远 false → 上提节点的 COLLAPSE_<id> 全部进了 hiddenContainerCodes (对 g.cluster 生效),
      //     而 SVG 中它是 g.node → 用户看到"父隐藏后, 上提的子领域/服务模块聚合节点仍可见".
      //   修复: 入口对 groups 深拷贝 + markUplift 补标记, 既不污染 store, 又让 collectHiddenState
      //   拿到与渲染层一致的上提信息 (纯函数测试已覆盖此分支).
      let groupsForHidden = groups
      try {
        groupsForHidden = JSON.parse(JSON.stringify(groups || []))
        markUplift(groupsForHidden)
      } catch (e) {
        console.warn('[updateVisibilityOnly] markUplift prep failed:', e)
      }
      ...
      // [FIX BUG-V035 2026-09-03] 原实现只对 nodeColorMappings 中的 BO 隐藏, 但 useBusinessObjectSyntax.js:1230
      //   用 hiddenBoIds 过滤掉折叠分组下的 BO (不渲染时也不进 nodeColorMappings); 此外, 当父分组 visible=false
      //   时其子 BO 可能仍由 mermaid 渲染但未关联颜色映射. 兜底遍历所有 g.node[data-code], 命中
      //   hiddenNodeCodes 的 SVG 节点一律隐藏. 同时保留 nodeColorMappings 循环以确保颜色联动刷新.
      const codeToNodeEl = new Map()
      svg.querySelectorAll('g.node[data-code]').forEach((el) => {
        const code = el.getAttribute('data-code')
        if (code) codeToNodeEl.set(code, el)
      })
      codeToNodeEl.forEach((el, code) => {
        el.style.display = hiddenNodeCodes.has(code) ? 'none' : ''
      })
      // 兼容: nodeColorMappings 里有但 codeToNodeEl 查不到的 (极端边角), 不再单独处理 — codeToNodeEl
      //   已遍历所有当前 SVG 中存在的 g.node[data-code].
    ...
    // [BUG-V038 REVERTED 2026-09-03] 原以为"重渲染 = 重置可见性"是用户期望, 实测用户场景:
//   expandLevel 切换走 expandGroupsToLevel(items, key, { resetVisible: true }) 路径,
//   用户在切到 subDomain/serviceModule 后, legend 隐藏失效 — 但根因不是这里.
//   撤回: 不再重置 store.visible=false (用户隐藏语义与重渲染无关, 误清会让"隐藏采购云"丢失).
    ...
    // [收敛 2026-09-03] executeGlobalExpand(右键) 与 debug.setExpandLevel(调试) 原是同一段
    //   "setExpandLevel + clone + resetVisible + updateLayoutControlConfig" 的复制粘贴实现,
    //   新增层级入口极易漏 reset → 图例隐藏残留 (2026-09-03 bug 根因之一). 收敛到唯一实现
    //   expandGlobalToLevel. 注意: LayoutControlPanel.handleExpandToLevel 面板路径的本地
    //   localConfig reset 不可删 — AADiagramApp/LayoutSelector 场景无 EmbeddedChartView 的
    //   VIS-RESET-SYNC watcher, 面板树必须自 reset 后 emit; 该处与 executeGlobalExpand 处理
    //   的是不同树 (面板 localConfig vs 渲染配置树), 不属重复.
    function expandGlobalToLevel(key) {
      configStore.setExpandLevel(key)
      const cfg = effectiveLayoutControlConfig.value || configStore.layoutControlConfig
      if (!cfg || !Array.isArray(cfg.groups)) return { applied: false }
      const newConfig = JSON.parse(JSON.stringify(cfg))
    ...
    watch(
      ...
      (newVal, oldVal) => {
        ...
        debug.debugTrace('[WATCH] layoutControlConfig sig changed=' + (newSig !== oldSig) + ', newSig=' + newSig.slice(0, 200) + ', oldSig=' + oldSig.slice(0, 200))
        if (newSig !== oldSig) {
          // [A1 2026-08-10] 仅折叠/展开变化 → 双缓冲平滑过渡 (不闪整屏 loading)
          if (sigNoFoldStr(newVal) === sigNoFoldStr(oldVal)) {
            // [LEVEL-OVERLAY 2026-09-04] 仅折叠类变化时, 再区分「用户切换全局展开层级」与
            //   「单节点折叠/展开」. 用户要求: 切层级 (domain/subDomain/serviceModule/
            //   businessObject) 必须展示与初始渲染同款的整屏"图表渲染中"遮罩; 实测折叠路径
            //   (531ms) 只静置旧缓冲层且角落 indicator 有 500ms 延迟 → 用户感知"页面冻结无
            //   反馈". 单节点折叠/展开 (右键/双击/URL fold) 保留 A1 缓冲平滑, 不闪遮罩.
            //   区分信号: 全局层级切换必先 configStore.setExpandLevel → expandLevel 变化;
            //   单节点操作只 markGroupManualSet, 不触碰 expandLevel.
            //   故 store.expandLevel ≠ lastAppliedExpandLevel → 判定层级切换 → 不置
            //   foldRenderPending, renderMermaid 落入 setRendering(true) 整屏遮罩路径.
            const levelChanged = configStore.expandLevel !== lastAppliedExpandLevel
            if (!levelChanged) {
              foldRenderPending = true
            } else {
              debug.debugLog('[LEVEL-OVERLAY] expandLevel changed ' + lastAppliedExpandLevel + ' → ' + configStore.expandLevel + ' → 全量渲染遮罩 (非 fold)')
            }
          }
    ...
    // [DBG 2026-09-04] 调试面板回调返回值是大对象 (整棵 group 树 / svg clusters+nodes 全量),
    //   console 端打 5 KB+ JSON 既无意义也拖慢浏览器; 改用 debugTrace 仅入 buffer, 排查者按需
    //   调 __archPage.debug.getLogs() 即可取回. 面板仅 1 行动作名 (debugLog) 保留可肉眼跟踪.
    const debugDump = () => { const r = window.__archPage?.debug?.dump?.(); debug.debugLog('[DBG-Panel] dump'); debug.debugTrace('[DBG-Panel] dump result:', r) }
    const debugInspectGroups = () => { const r = window.__archPage?.debug?.inspectGroups?.(); debug.debugLog('[DBG-Panel] inspectGroups'); debug.debugTrace('[DBG-Panel] inspectGroups result:', r) }
    const debugInspectRendering = () => { const r = window.__archPage?.debug?.inspectRenderingGroups?.(); debug.debugLog('[DBG-Panel] inspectRenderingGroups'); debug.debugTrace('[DBG-Panel] inspectRenderingGroups result:', r) }
    const debugIdentifyCollapse = () => { const r = window.__archPage?.debug?.identifyCollapseNodes?.(); debug.debugLog('[DBG-Panel] identifyCollapseNodes'); debug.debugTrace('[DBG-Panel] identifyCollapseNodes result:', r) }
    const debugVerifyChart = () => { const r = window.__archPage?.debug?.verifyChart?.(); debug.debugLog('[DBG-Panel] verifyChart'); debug.debugTrace('[DBG-Panel] verifyChart result:', r) }
    const debugTestRightClick = () => { const r = window.__archPage?.debug?.testRightClick?.('g.cluster, g.node'); debug.debugLog('[DBG-Panel] testRightClick'); debug.debugTrace('[DBG-Panel] testRightClick result:', r) }
    const debugTestDblClick = () => { const r = window.__archPage?.debug?.testDblClick?.('g.node[id*="COLLAPSE_"], g.cluster'); debug.debugLog('[DBG-Panel] testDblClick'); debug.debugTrace('[DBG-Panel] testDblClick result:', r) }
    const debugExpandSCP = () => { const r = window.__archPage?.debug?.expandGroup?.('SCP', 2); debug.debugLog('[DBG-Panel] expandGroup SCP→2'); debug.debugTrace('[DBG-Panel] expandGroup SCP→2 result:', r) }
    const debugCollapseSCP = () => { const r = window.__archPage?.debug?.collapseGroup?.('SCP'); debug.debugLog('[DBG-Panel] collapseGroup SCP'); debug.debugTrace('[DBG-Panel] collapseGroup SCP result:', r) }
    ...
    const installDebugHelpers = () => {
      ...
      const api = {
        ...
        // [VIS-SNAPSHOT 2026-09-03 P0-2] 可见性"四层统一快照": 一次读取
        //   store(渲染配置权威) / chartConfig(面板树) / collectHiddenState 分类 / SVG display 实况,
        //   供回归探针断言, 消除探针各自拼装导致"拿错层断言"的假失败 (2026-09-03 v11 根因).
        //   分类规则与 collectHiddenState/expandLevel.js 保持一致: ELK 系统自动分组
        //   (groupType='custom' && _elkGroup in {inner,boundary}) 的 visible=false 是布局语义,
        //   不算"用户隐藏" (storeUserHidden/chartUserHidden 均已排除).
        visibilitySnapshot: () => {
          const flatItems = (groups) => {
            ...
            const walk = (list) => {
              ;(list || []).forEach(g => {
                if (!g || typeof g !== 'object') return
                const elk = g._elkGroup || ''
                const elkAuto = g.groupType === 'custom' && (elk === 'inner' || elk === 'boundary')
                out.push({
                  code: g.elementCode || g.id || null,
                  title: g.title || null,
                  groupType: g.groupType || null,
                  visible: g.visible === undefined ? null : !!g.visible,
          ...
          const userHiddenOf = (groups) => flatItems(groups)
            .filter(i => i.visible === false && !i.elkAuto)
            .map(i => i.code)
          const storeGroups = (configStore.layoutControlConfig && configStore.layoutControlConfig.groups) || []
          const chartGroups = (window.__archPage && window.__archPage.chartConfig
            && window.__archPage.chartConfig.layoutControl
            && window.__archPage.chartConfig.layoutControl.groups) || null
          // SVG 实况: 记录所有被 updateVisibilityOnly 置为 display:none 的节点/聚合/容器
          const svgState = { hasSvg: false, displayNone: [] }
          const svg = mermaidContainer.value && mermaidContainer.value.querySelector('svg')
          if (svg) {
            svgState.hasSvg = true
            svg.querySelectorAll('g.node[data-code], g.node[id*="COLLAPSE_"], g.cluster[data-container-code]').forEach((el) => {
              if (el.style && el.style.display === 'none') {
                const id = el.id || ''
                const code = el.getAttribute('data-code') || el.getAttribute('data-container-code') || ''
                const m = id.match(/COLLAPSE_(.+)/)
                svgState.displayNone.push({ id, code: code || (m ? m[1] : null), tag: el.tagName })
              }
            })
          }
          // collectHiddenState 分类 (与 updateVisibilityOnly 同输入: store groups + markUplift)
          let hiddenMeta = null
          try {
            const cloned = JSON.parse(JSON.stringify(storeGroups))
            markUplift(cloned)
            const r = collectHiddenState(cloned, { isScopeProtected })
            hiddenMeta = {
              nodeCodes: Array.from(r.hiddenNodeCodes || []),
              containerCodes: Array.from(r.hiddenContainerCodes || []),
              collapseIds: Array.from(r.hiddenCollapseIds || [])
            }
          } catch (e) {
            hiddenMeta = { error: String((e && e.message) || e) }
          }
          return {
            ctx: {
            ...
            store: { count: storeGroups.length, items: flatItems(storeGroups) },
            chart: chartGroups ? { count: chartGroups.length, items: flatItems(chartGroups) } : null,
            storeUserHidden: userHiddenOf(storeGroups),
            chartUserHidden: chartGroups ? userHiddenOf(chartGroups) : null,
            svg: svgState,
            hiddenMeta
          }
        },
        ...
        setExpandLevel: (key) => {
          ...
          // [收敛 2026-09-03] 统一走 expandGlobalToLevel (与 executeGlobalExpand 同一实现),
          //   避免复制粘贴逻辑再次漂移 (2026-09-03 bug 根因之一: 入口漏 reset).
          const r = expandGlobalToLevel(key)
          if (!r.applied) return { error: 'no groups in config' }
          debug.debugLog('[DBG] setExpandLevel: key=' + key)
          debug.recordInteraction('setExpandLevel', { key })
          const cfg = effectiveLayoutControlConfig.value || configStore.layoutControlConfig
          return { key, collapsedCount: flatten((cfg && cfg.groups) || []).filter(g => g.collapsed === true).length }
        },
        ...
        // [LEGEND-PROBE 2026-09-03] 图例隐藏一键探针: 定位"点图例隐藏后子节点仍显示"断链在哪一环.
        //   在真实图例项上派发 click → 走 handleToggleGroupVisible → store → updateVisibilityOnly 全链路;
        //   随后对比"按 store 复算的应隐藏集合(深拷贝+markUplift+collectHiddenState, 与 updateVisibilityOnly
        //   同一套)"与"SVG 实际 display", 一次输出四类结论:
        //     storeHidden        : store 中 visible=false 的分组
        //     collapseNodes      : SVG 聚合节点 (COLLAPSE_*), 含 display + 是否命中复算 hiddenCollapseIds
        //     containerCodes     : SVG 容器 (g.cluster), 含 display + 是否命中复算 hiddenContainerCodes
        //     computed           : 复算的 { hiddenNodeCount, hiddenContainerCodes[], hiddenCollapseIds[] }
        //   若 collapseNodes 中某节点 inComputed=true 但 display≠none → 应用层漏设 (3.5 节查询/id 解析问题);
        //   若 inComputed=false 但该节点位于某隐藏分组下 → 收集层漏收集 (markUplift/_uplift 判定问题).
        //   用法 (浏览器控制台, 需 ?mode=debug):
        //     __archPage.debug.legendHideProbe()                     // 隐藏第一个可点击图例项
        //     __archPage.debug.legendHideProbe('采购商城')            // 隐藏指定名称的图例项
        //   返回 Promise<object>; 浏览器 console 可直接 await:
        //     (await __archPage.debug.legendHideProbe()).then ? null : null   // 或直接 await
        legendHideProbe: async (legendName) => {
          const wait = (ms) => new Promise((r) => setTimeout(r, ms))
          const parseCollapseId = (id) => {
            const prefix = 'flowchart-'
            if (!id || !id.startsWith(prefix)) return null
            const cut = id.indexOf('-', (prefix + 'COLLAPSE_').length)
            return cut === -1 ? id.substring(prefix.length) : id.substring(prefix.length, cut)
          }
          const read = () => {
            const cfg = configStore.layoutControlConfig
            const groups = cfg?.groups || []
            const flat = (list, depth = 0) => {
              const out = []
              ;(list || []).forEach((g) => {
                if (!g || typeof g !== 'object') return
                out.push({ code: g.elementCode || g.id, id: g.id, title: g.title || g.name, groupType: g.groupType, visible: g.visible, collapsed: g.collapsed === true, depth })
                out.push(...flat(g.children, depth + 1))
                out.push(...flat(g.containers, depth + 1))
              })
              return out
            }
            const all = flat(groups)
            const svg = mermaidContainer.value?.querySelector('svg')
            // 与 updateVisibilityOnly 同一套隐藏集合复算: 深拷贝 + markUplift + collectHiddenState
            let computed = null
            try {
              const cloned = JSON.parse(JSON.stringify(groups || []))
              markUplift(cloned)
              const { hiddenNodeCodes, hiddenContainerCodes, hiddenCollapseIds } = collectHiddenState(cloned, { isScopeProtected })
              computed = {
                hiddenNodeCount: hiddenNodeCodes.size,
                hiddenContainerCodes: Array.from(hiddenContainerCodes),
                hiddenCollapseIds: Array.from(hiddenCollapseIds)
              }
            } catch (e) {
              computed = { error: e?.message || String(e) }
            }
            const hiddenContainerSet = new Set(computed?.hiddenContainerCodes || [])
            const hiddenCollapseSet = new Set(computed?.hiddenCollapseIds || [])
            const collapseNodes = svg
              ? Array.from(svg.querySelectorAll('g.node[id^="flowchart-COLLAPSE_"]')).map((el) => ({
                  svgId: el.id,
                  collapseId: parseCollapseId(el.id),
                  display: el.style.display || 'inline',
                  inComputed: !!hiddenCollapseSet.has(parseCollapseId(el.id))
                }))
              : []
            const containerCodes = svg
              ? Array.from(svg.querySelectorAll('g.cluster')).map((el) => {
                  const code = el.getAttribute('data-container-code') || ''
                  return { code, display: el.style.display || 'inline', inComputed: !!code && hiddenContainerSet.has(code) }
                }).filter((c) => c.code)
              : []
            const storeHidden = all.filter((g) => g.visible === false)
            // 推论: 若存在 inComputed=true 但 display 非 none 的节点 → 应用层漏设
            const appliedMismatches = collapseNodes.filter((n) => n.inComputed && n.display !== 'none')
              .concat(containerCodes.filter((c) => c.inComputed && c.display !== 'none'))
            return { expandLevel: configStore.expandLevel, storeHidden, collapseNodes, containerCodes, computed, appliedMismatches }
          }
          const before = read()
          // 找到图例面板中可点击项 (cursor=pointer, 排除节标题/对象范围不可点击项)
          const candidates = Array.from(document.querySelectorAll('.color-legend-panel div'))
            .filter((d) => getComputedStyle(d).cursor === 'pointer')
          const target = legendName ? candidates.find((d) => (d.textContent || '').trim() === legendName) : candidates[0]
          if (!target) {
            return { error: 'no clickable legend item found', requested: legendName || '(first)', candidates: candidates.map((d) => (d.textContent || '').trim()).slice(0, 20), before }
          }
          const clickedName = (target.textContent || '').trim()
          target.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }))
          await wait(400)
          const after = read()
          return { legendName: clickedName, before, after }
        },
      }
