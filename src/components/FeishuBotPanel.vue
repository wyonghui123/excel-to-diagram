<template>
  <div class="feishu-bot-panel">
    <div class="panel-header">
      <h3><AppIcon name="lightning" size="sm" /> 飞书机器人交互</h3>
      <button class="close-btn" @click="$emit('close')"><AppIcon name="close" size="sm" /></button>
    </div>
      ...
      <div class="messages-container" ref="messagesContainer">
        ...
        >
          <div class="message-avatar">
            <AppIcon v-if="msg.type === 'incoming'" name="lightning" size="sm" />
            <AppIcon v-else name="enabled" size="sm" />
          </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const quickCommands = [
      { key: 'help', label: '[SYMBOL] 帮助', text: 'help' },
      { key: 'generate', label: '[DECORATIVE] 生成图表', text: '生成图表' },
      { key: 'export', label: '[SYMBOL] 导出PNG', text: '导出 png' },
      { key: 'send', label: '[SYMBOL] 发送飞书', text: '发送飞书' }
    ]
