...
/* [通用高度基准 2026-09-06] actions / chart-config slot 注入的所有控件高度统一 32px。
   背景: 本组件 scoped 的 .gt-actions .el-button 够不着 slot 内容(scope 隔离),
   各业务方(如图表展示 toggle、ChartMiniToolbar 备注类型/布局方向/高级按钮)自带高度
   (24/28px)导致与 32px 图标按钮参差。此处用 :deep 一次覆盖所有注入控件:
   - 特意写成 .el-select .el-select__wrapper 两级类, 以特异性(0,4,0)稳定压过业务侧
     自身 scoped 高度(如 .cmt-dir-btn 28px), 不依赖样式表加载顺序;
   - 只定高度不定宽度: 图标按钮 32px 方块由上方自有规则约束, 文字按钮宽度自适应;
   - 新增 slot 控件类型(如 el-input/el-dropdown)时按需在此扩展, 业务侧无需自带高度。 */
.gt-actions :deep(.el-button) {
  height: 32px;
  box-sizing: border-box;
}

.gt-actions :deep(.el-select .el-select__wrapper) {
  height: 32px;
  min-height: 32px;
  max-height: 32px;
  box-sizing: border-box;
