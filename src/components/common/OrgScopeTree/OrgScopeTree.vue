...
async function loadTreeData(options = {}) {
  ...
  try {
    // [FIX 2026-09-01 Spec16 Plan D] user_group → org (后端 alias 兼容旧名, 但前端应直接用新名)
    // [FIX 2026-09-04 组织树不全] 后端 query_bo 强制 page_size 封顶 MAX_USER_PAGE_SIZE=500
    //   单次请求最多 500 条 → orgs(959 行) 只加载前 500 → 树结构/节点不全.
    //   改为循环分页拉全量 (page_size=500, 上限 40 页 = 20000 行, 防死循环 DoS).
    const PAGE_SIZE = 500
    const MAX_PAGES = 40
    const allItems = []
    let total = null
    for (let page = 1; page <= MAX_PAGES; page++) {
      const result = await boService.query('org', { page, page_size: PAGE_SIZE })
    ...
    const tree = buildOrgTree(allItems)

    // silent 模式保留已选/展开状态
    if (silent && treeRef.value) {
      const currentCheckedKeys = treeRef.value.getCheckedKeys?.() || []
      if (currentCheckedKeys.length > 0) {
        // [FIX 2026-09-04] 结构等价判定需考虑层级关系 (parentId 映射)
        //   原实现仅比对节点 id 集合, 导致变更父组织后刷新被误判为"结构未变" → 树不更新.
        //   用户场景: 改某组织 parent_id → silent 刷新 → 同 id 集合 → return → UI 仍显示旧层级.
        //   修复: 增加 parentId 映射比对, 任何节点父级变化都触发树重建.
        if (isSameStructure(treeData.value, tree)) return
        guard.enter()
        treeData.value = tree
...
// [FIX 2026-09-04] 真正的结构等价判定: 节点 id 集合 + 每个节点 parentId 映射都一致.
//   原 collectAllKeys 比较只覆盖节点 id 集合, 无法识别层级变化 (parent_id 变更).
//   递归遍历两颗树, 对每个 (id, parentId) 一一比对. 复杂度 O(n), 与 refresh 频率匹配.
function isSameStructure(oldNodes, newNodes) {
  function mapParents(nodes, acc = new Map(), parentId = null) {
    for (const n of nodes || []) {
      acc.set(String(n.id), parentId == null ? null : String(parentId))
      if (n.children?.length) mapParents(n.children, acc, n.id)
    }
    return acc
  }
  const oldMap = mapParents(oldNodes)
  const newMap = mapParents(newNodes)
  if (oldMap.size !== newMap.size) return false
  for (const [id, oldParent] of oldMap) {
    if (!newMap.has(id)) return false
    if (newMap.get(id) !== oldParent) return false
  }
  return true
}
