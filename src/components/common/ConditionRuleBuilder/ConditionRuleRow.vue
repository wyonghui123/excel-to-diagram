          ...
          <div v-if="pickTags.length > 0 && !disabled" class="rule-value-tags">
            <span
              ...
              :class="['rule-value-tag', {
                'rule-value-tag--bizkey': tag.bizkey,
                'rule-value-tag--zero-hit': tag.zeroHit,
              }]"
...
const pickerPlaceholder = computed(() => {
  ...
  const col = String(r.field || '').toLowerCase()
  // [Spec 20 v5 L2 字段即模式] placeholder 随字段值域语义对偶切换:
  //   「按xx业务键锚定」(动态) vs 「选择xx实例」(快照) — self-ref 判定与
  //   rule-helpers 同源 (无 relationObject 才是 self-ref); 不绑定具体维度词
  if (r.isBusinessKey && !r.relationObject) {
    return (col === 'code' || col.endsWith('_code'))
      ? `按${props.resourceType || '记录'}业务键锚定...`
      : `选择${props.resourceType || '记录'}实例...`
  }
  if (r.isBusinessKey && r.relationObject) {
    return `按${r.relationObject}业务键锚定...`   // FK code 虚拟字段 (v5 白名单)
  }
  return `选择${r.relationObject || '数据'}实例...` // FK id
})
...
// [Spec 20 v6 2026-09-12] tag 键名同显 (key+name format, 用户确认):
//   - 实例项 (ID 模式):   「id（name）」— 键在前, 与业务键 format 对称; 无 name 回落 id
//   - 业务键项 (code 模式): 「code（name）」— 业务键主体 (锚点本体), 示例名辅助;
//     命中数/0 命中警示移入 hover title (tag 简洁化, 0 命中仍红标 fail-closed)
// [Spec 20 v6b 2026-09-12] 用户反馈调整: 实例项与业务键对称改为「id（name）」
//   (键在前名在后); 业务键 title 简化为「code（name），命中 N 个实例」
const pickTags = computed(() => {
  const r = props.rule
  if (Array.isArray(r.pickerSelectedItems) && r.pickerSelectedItems.length > 0) {
    return r.pickerSelectedItems.map((i) => {
      if (i.bizkey) {
        const n = i.resolvedCount
        const code = i.code || i.name || String(i.id)
        const label = i.name && i.name !== code ? `${code}（${i.name}）` : code
        const hit = n === undefined ? '' : n === 0 ? '，当前未命中任何实例（fail-closed）' : `，命中 ${n} 个实例`
        return {
          id: i.id,
          label,
          bizkey: true,
      ...
      const name = i.name && String(i.name) !== idStr ? i.name : ''
      const label = idStr && name ? `${idStr}（${name}）` : (idStr || i.name || '')
      return { id: i.id, label, title: label }
    })
  }
  if (!r.value) return []
  const hasBizkeyToken = isBizkeyRuleValue(r)
  return parseRuleValueIds(r).map((id) => {
    const s = String(id)
    const isCode = hasBizkeyToken && !/^-?\d+$/.test(s)
    return { id, label: isCode ? `${s}（业务键）` : s }
  })
})
...
/* [Spec 20 Task 9-B] 业务键锚点 tag: 蓝色系区分 ID 项; 0 命中红色 fail-closed 预告 */
.rule-value-tag--bizkey {
  background: var(--el-color-primary-light-9, #ecf5ff);
  color: var(--el-color-primary, #409eff);
}
.rule-value-tag--zero-hit {
  background: var(--color-error-bg, #fef2f2);
  color: var(--color-error, #dc2626);
}
