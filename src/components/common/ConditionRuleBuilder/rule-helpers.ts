...
export function getRuleValueHelpConfig(rule, ctx = {}) {
  ...
  // [Spec 20 v5 2026-09-12] 字段即模式 (field IS the mode) — SAP F4 原则:
  //   F4 是字段的函数, 不是弹窗里的二次选择。字段类型 → 值域模式 一一对应:
  //   - self-ref 业务键 (code)          → bizkey_only 直出业务键列表 → code IN ('AM') 原生动态
  //   - self-ref 技术主键 (id)          → 仅实例树 → id IN (64,88) 快照
  //   - FK code 虚拟字段 (version_code) → bizkey_only 直出目标对象业务键列表 (v5 白名单放行)
  //   - FK id 字段 (version_id)         → 仅实例树 (双 Tab 退役: 业务键入口由 FK code 字段承担)
  //   判定通用性: self-ref 必须无 relationObject; FK code = isBusinessKey + relationObject
  const isSelfRefBizkey = !!(rule.isBusinessKey && form && !rule.relationObject)
  const isFkBizkey = !!(rule.isBusinessKey && rule.relationObject)  // FK code 字段 (v5)
  const selfRefCol = String(rule.field || '').toLowerCase()
  const selfRefIsCode = isSelfRefBizkey &&
    (selfRefCol === 'code' || selfRefCol.endsWith('_code'))
  const isBizkeyOnly = selfRefIsCode || isFkBizkey

  let targetBo = isSelfRefBizkey && form ? form.resource_type : rule.relationObject
  let displayColumns = [
    { field: 'name', label: '名称' },
    { field: 'code', label: '编码' },
  ]
  ...
  } else if (HIERARCHY_DIMENSIONS.has(targetBo) && !isBizkeyOnly) {
  ...
  // [Spec 20 v3 2026-09-12] 业务键 Tab 配置 (父对象无关, 不绑定「版本」叙事):
  //   [v5 字段即模式] enabled 分派:
  //   - self-ref code 字段   → enabled + bizkey_only=true (仅业务键列表, 无 Tab)
  //   - FK code 虚拟字段     → enabled + bizkey_only=true (仅目标对象业务键列表, 无 Tab)
  //   - self-ref id 字段     → enabled=false (仅实例树)
  //   - FK id 字段           → enabled=false (双 Tab 退役 — 业务键入口由 FK code 字段承担,
  //     存量 version_id+锚点规则的展开逻辑保留, 仅 UI 入口移除)
  //   - tab_label 默认 null → SearchHelpDialog 按后端 parent_label 动态渲染
  //     「跨{父对象}·业务键」(domain→跨版本, version→跨产品), 无父回落「按业务键」;
  //     调用方可显式覆盖。hint 为通用表述, columns 调用方可覆盖
  //   - 后端开关关闭 (codes.enabled=false) 时 Tab 自动隐藏, 双重门控
  const bizkeyConfig = {
    enabled: isBizkeyOnly,
    bizkey_only: isBizkeyOnly,
    tab_label: null as string | null,  // null = 动态: 跨{parent_label}·业务键 / 按业务键
    // hint 缺省不传 → SearchHelpDialog 用简洁通用文案 (单一事实源, 避免双份漂移)
    columns: [
  ...
  return {
    source: { type: sourceType, target_bo: targetBo, bizkey: bizkeyConfig },
    presentation: {
...
/**
 * [Spec 20 Task 9-B] 判定规则当前值是否为业务键模式
 *
 * 模式不落存储 — 由 value 内容自动判定: 含非数字 token 即为业务键锚点值
 * （与后端 _is_anchor_code / dimension_scope_engine 数字+锚点分离逻辑同源）。
 */
export function isBizkeyRuleValue(rule) {
  if (!rule || !rule.value || typeof rule.value !== 'string') return false
  return rule.value
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
...
export function createRuleValueFetcher(permService = permServiceDefault) {
  return async function ruleValueFetcher(rule, ctx = {}, params = {}) {
    ...
    // [Spec 20 v5] self-ref 判定须无 relationObject（FK code 字段不走实例 fetch,
    // bizkey_only 模式由 SearchHelpDialog 的 codes 端点供数）
    const targetBo = rule.isBusinessKey && form && !rule.relationObject
      ? form.resource_type
      : rule.relationObject
    const serviceParams = {
      page: page || 1,
...
export function handleRulePickerConfirm(rule, selection) {
  ...
  rule.pickerSelectedItems = items.map((item) => ({
    ...
    code: item.code || '',
    // [Spec 20 Task 9-B] 业务键锚点项: id=code (非数字), 附命中数供规则行 tag 展示
    bizkey: !!item.bizkey,
    resolvedCount: item.bizkey ? (item.resolvedCount ?? 0) : undefined,
  }))
