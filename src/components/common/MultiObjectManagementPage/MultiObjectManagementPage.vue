      ...
      <template #detail>
        <div class="momp-detail-content">
          <template v-if="showVersionScopedContent">
            <!-- FIX 2026-07-30: hide tabs and list in chart view, show detailContent slot only -->
...
const scopeTreeKey = ref(0)  // remount RelationScopeTree after returning from chart, restore selection
...
async function tryApplyShortcut() {
  ...
  //   [NOTE] scenarios requiring relation selection (e.g. mm-cross-domain) cannot be
  //   expressed via scopeCode; use base64 ?scope= URL from test_helpers/scenario.py:
...
// [MOMP 2026-08-30 + FIX 2026-09-02] 空态文案：
//   - 未选择范围: 不再强制空集守卫, filterStrategies 不过滤, 后端返回全量
//     此时空列表才是真正"暂无数据", 给出操作提示
//   - 行为: 未选 = 展示全部组织 (符合组织管理页业务预期)
const emptyState = computed(() => {
  if (!page.hasScopeSelection) {
    return {
      text: '当前未选择范围',
      hint: '当前展示全部数据，可在左侧树中勾选要查看的组织范围'
    }
  }
...
/* 高度由 GlobalToolbar 的通用高度基准统一(.gt-actions :deep 规则), 此处不再自带 height */
.gt-btn-chart-toggle {
