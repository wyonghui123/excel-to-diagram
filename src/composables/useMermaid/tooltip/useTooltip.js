...
//   替换 v40.3 历史诊断 [v40.3 诊断] preloadEnums ... 共 7 处 (历史排障残留).
...
export async function preloadEnums() {
  ...
  _dbgLog('[v40.3 诊断] preloadEnums STARTED')

  try {
    const EnumService = await getEnumService()
    _dbgLog('[v40.3 诊断] preloadEnums EnumService =', EnumService ? 'OK' : 'NULL', '_cache=', !!EnumService?._cache)
    if (!EnumService || !EnumService._cache) return
    if (!EnumService._cache.has('direction')) {
      _dbgLog('[v40.3 诊断] preloadEnums loading direction...')
      const dirResult = await EnumService.loadOptions('direction', { cache: true, throwError: false })
      _dbgLog('[v40.3 诊断] preloadEnums direction loaded:', dirResult?.length, 'options')
      _enumMapCache.delete('direction')
    }
    if (!EnumService._cache.has('relation_type')) {
      _dbgLog('[v40.3 诊断] preloadEnums loading relation_type...')
      const typeResult = await EnumService.loadOptions('relation_type', { cache: true, throwError: false })
      _dbgLog('[v40.3 诊断] preloadEnums relation_type loaded:', typeResult?.length, 'options')
      _enumMapCache.delete('relation_type')
    }
    _dbgLog('[v40.3 诊断] preloadEnums DONE cacheSize=', EnumService._cache.size)
  } catch (e) {
