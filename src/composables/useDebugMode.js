...
export function useDebugMode() {
  ...
  // [DBG 2026-09-04] 静默标签: 首参文本含这些前缀的日志只入 buffer, 不打 console.
  //   一次右键/双击/关系高亮触发的循环 trace 每次操作可刷 4~12 条, 排查时调
  //   __archPage.debug.getLogs() 即可拿到全量; 操作类标签 (executeContextMenuAction /
  //   [CTX-GLOBAL]/[DBG]/[REC]/[REPLAY]/[STEP]/[VERIFY]) 仍打 console, 保证动作可被肉眼跟踪.
  const CONSOLE_QUIET_PREFIXES = [
    '[CTX] identifyGroupFromSvg',
    '[DBL] handleDblClick',
    '[REL-HL] highlightRelations',
    '[REL-HL] scope subtree size',
    '[REL-HL] connected nodes'
  ]
  function isQuiet(args) {
    if (!args || !args.length) return false
    const first = args[0]
    if (typeof first !== 'string') return false
    return CONSOLE_QUIET_PREFIXES.some(p => first.startsWith(p))
  }
  const debugLog = (...args) => {
    if (!isDebug) return
    if (isQuiet(args)) { pushLog('log', args); return }
    console.log(...args); pushLog('log', args)
  }
  const debugWarn = (...args) => {
    if (!isDebug) return
    if (isQuiet(args)) { pushLog('warn', args); return }
    console.warn(...args); pushLog('warn', args)
  }
  const debugError = (...args) => {
    if (!isDebug) return
    if (isQuiet(args)) { pushLog('error', args); return }
    console.error(...args); pushLog('error', args)
  }
  // [DBG 2026-09-04] debugTrace: buffer-only, 不打 console.
  //   用途: DBG-Panel 按钮回调返回的大对象 (整棵 group 树 / svg clusters/nodes 全量).
  //   console 端打 5 KB+ JSON 既无意义也拖慢浏览器; 排查者按需调 __archPage.debug.getLogs().
  const debugTrace = (...args) => { if (isDebug) pushLog('log', args) }
