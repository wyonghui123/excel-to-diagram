...
test.describe('S-BF-ENUM_TYPE-AUTO: 枚举类型 - 业务流 (AI 派生)', () => {
   ...
   * 业务规则: BR-enum_type-PAG-default
   */
  test('PAG_DEFAULT: 验证分页默认配置', async ({
    page, navigateTo, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PAG_enum_type', async () => {
  ...
  /**
   * deep_link 规则: detail=/detail/enum_type
   * 业务规则: BR-enum_type-DL-detail
   */
  test('DL_DETAIL: 直接访问详情页深链 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'DL_enum_type', async () => {
      const obj = await dataFinder.enum_type().catch(() => null)
      if (obj && obj.id) {
        await navigateToDeepLink(page, 'enum_type', obj.id)
        await page.waitForURL('**/detail/enum_type**', { timeout: 5000 })
        console.log(`  [DL] 深链访问成功`)
      } else {
        console.log(`  [DL] 跳过: 无 dataFinder.enum_type`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-enum_type-BADGE-category
   */
  test('BADGE_CATEGORY: 验证 [category] 标签颜色 (软断言)', async ({
    page, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'BADGE_enum_type_category', async () => {
      await navigateTo(page, '/enum_type-management')
  ...
  /**
   * ui_badge 规则: mutability 字段彩色标签
   * 业务规则: BR-enum_type-BADGE-mutability
   */
  test('BADGE_MUTABILITY: 验证 [mutability] 标签颜色 (软断言)', async ({
    page, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'BADGE_enum_type_mutability', async () => {
      await navigateTo(page, '/enum_type-management')
      const tag = page.locator('.el-tag').first()
      const visible = await tag.isVisible({ timeout: 3000 }).catch(() => false)
      console.log(`  [BADGE] mutability tag visible=${visible}`)
    }, { softOn: ['5xx', '404'] })
    if (r.healed) console.log(`[Healer] BADGE 软断言: ${r.reason}`)
  })
  ...
  /**
   * nested_transaction 规则: children=['enum_value']
   * 业务规则: BR-enum_type-NEST-atomic
   */
  test('NEST_CREATE: 深插入 [枚举类型] + 子对象 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'NEST_enum_type', async () => {
      const parent = await dataFinder.enum_type().catch(() => null)
      if (parent) {
        const nestedPOM = new NestedPOM(page)
        console.log(`  [NEST] 父对象 ID=${parent.id}, 模拟深插入`)
      } else {
        console.log(`  [NEST] 跳过: 无 dataFinder.enum_type`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-enum_type-PER-survives_reload
   */
  test('PER_RELOAD: [枚举类型] 刷新后数据仍存在 (软断言)', async ({
    page, dataFinder, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PER_enum_type', async () => {
      const obj = await dataFinder.enum_type().catch(() => null)
      if (obj) {
        await navigateTo(page, '/enum_type-management')
        ...
        console.log(`  [PER] 跳过: 无 dataFinder.enum_type`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
