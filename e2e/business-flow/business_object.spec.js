/**
 * S-BF-BUSINESS_OBJECT-AUTO: 业务对象 - 业务流 E2E (AI 派生, 阶段三)
 *
 * [自动生成] 从 business_object.yaml 自动生成
 * [E2E v2 铁律合规 (8 项)]
 ...
 *
 * 业务规则:
 *   BR-business_object-FLD-REQ-version_id  (版本 必填)
 *   BR-business_object-FLD-REQ-code  (编码 必填)
 *   BR-business_object-FLD-REQ-name  (名称 必填)
 *   BR-business_object-FLD-PAT-code  (格式: ^[A-Z][A-Z0-9_]*$)
 *   BR-business_object-DEL-condition  (存在关联关系的业务对象不能删除)
 *   BR-business_object-AUDIT-create/update/delete  (审计日志)
 *
 * 自动生成时间: 2026-06-25
...
const BUSINESS_OBJECT_URL = '/business_object-management'

test.describe('S-BF-BUSINESS_OBJECT-AUTO: 业务对象 - 业务流 (AI 派生)', () => {

  /**
   * 必填字段校验: 版本 (version_id)
   * 业务规则: BR-business_object-FLD-REQ-version_id
  ...
  /**
   * 必填字段校验: 编码 (code)
   * 业务规则: BR-business_object-FLD-REQ-code
   * [4 维度评估] API/UI/Business/Multi
   */
  ...
  /**
   * 必填字段校验: 名称 (name)
   * 业务规则: BR-business_object-FLD-REQ-name
   * [4 维度评估] API/UI/Business/Multi
   */
  ...
  /**
   * 格式校验: 编码 (code)
   * 业务规则: BR-business_object-FLD-PAT-code
   * 正则: ^[A-Z][A-Z0-9_]*$
   */
  ...
  /**
   * 删除约束: 存在关联关系的业务对象不能删除
   * 业务规则: BR-business_object-DEL-condition
   * 条件: self.relation_count == 0
   * [Healer.L3] createTracked 失败时软断言 (FK 关联缺失)
   */
  test('C_DEL: 删除 [业务对象] 业务规则', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    const TS = Date.now()
    let obj = null
    const cr = await withStep(page, testInfo, '创建 [业务对象] (Healer 守护)', async () => {
      return await AIHealer.guard(page, 'C_DEL_business_object_create', async () => {
        return await isolation.createTracked('business_object', {
        version_id: null,
        code: `DEL_CODE_${TS}`,
        name: `del_name_${TS}`,
        })
    ...
    await withStep(page, testInfo, '业务断言: 无关联时可删除 (Healer 守护)', async () => {
      const r = await AIHealer.guard(page, 'C_DEL_business_object_check', async () => {
        const result = await BusinessRuleAssertor.assertDeletable(
          page, 'business_object', obj.id, { relatedCount: 0 }
        )
  ...
  /**
   * 审计日志: 创建 [业务对象] 应记录 audit_log
   * 业务规则: BR-business_object-AUDIT-create
   * [Healer.L1+L3] createTracked + audit_log 失败都软断言
   */
  test('C_AUDIT: [业务对象] 创建应生成 audit_log', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    const TS = Date.now()
    let obj = null
    const cr = await withStep(page, testInfo, '创建 [业务对象] (Healer 守护)', async () => {
      return await AIHealer.guard(page, 'C_AUDIT_business_object_create', async () => {
        return await isolation.createTracked('business_object', {
        version_id: null,
        code: `AUD_CODE_${TS}`,
        name: `aud_name_${TS}`,
    ...
    await withStep(page, testInfo, '业务断言: audit_log 应记录创建事件 (Healer 守护)', async () => {
      const r = await AIHealer.guard(page, 'C_AUDIT_business_object_check', async () => {
        const valid = await BusinessRuleAssertor.assertAuditLogExists(
          page, 'business_object', obj.id, 'create'
        )
  ...
  test('AUD_CREATE: create 应产生 INFO 审计', async ({
    ...
    const r = await AIHealer.guard(page, 'AUD_business_object_create', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'business_object', null, 'create'
      )
   ...
   * 业务规则: BR-business_object-AUDIT-update
   */
  test('AUD_UPDATE: update 应产生 INFO 审计', async ({
    page, dataFinder, isolation
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'AUD_business_object_update', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'business_object', null, 'update'
      )
   ...
   * 业务规则: BR-business_object-AUDIT-delete
   */
  test('AUD_DELETE: delete 应产生 WARN 审计', async ({
    page, dataFinder, isolation
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'AUD_business_object_delete', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'business_object', null, 'delete'
      )
   ...
   * 业务规则: BR-business_object-PAG-default
   */
  test('PAG_DEFAULT: 验证分页默认配置', async ({
    page, navigateTo, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PAG_business_object', async () => {
      await navigateTo(page, '/business_object-management')
  ...
  /**
   * deep_link 规则: detail=/detail/business_object/business_object-detail
   * 业务规则: BR-business_object-DL-detail
   */
  test('DL_DETAIL: 直接访问详情页深链 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'DL_business_object', async () => {
      const obj = await dataFinder.business_object().catch(() => null)
      if (obj && obj.id) {
        await navigateToDeepLink(page, 'business_object', obj.id)
        await page.waitForURL('**/detail/business_object/business_object-detail**', { timeout: 5000 })
        console.log(`  [DL] 深链访问成功`)
      } else {
        console.log(`  [DL] 跳过: 无 dataFinder.business_object`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-business_object-HEALTH
   */
  test('HEALTH: [业务对象] 列表健康检查', async ({
    page, navigateTo
  }, testInfo) => {
    const errors = []
    page.on('pageerror', e => errors.push('pageerror: ' + e.message))
    page.on('console', msg => { if (msg.type() === 'error') errors.push('console: ' + msg.text()) })
    const r = await AIHealer.guard(page, 'HEALTH_business_object', async () => {
      await navigateTo(page, '/business_object-management')
   ...
   * 业务规则: BR-business_object-NEST-atomic
   */
  test('NEST_CREATE: 深插入 [业务对象] + 子对象 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'NEST_business_object', async () => {
      const parent = await dataFinder.business_object().catch(() => null)
      if (parent) {
        const nestedPOM = new NestedPOM(page)
        console.log(`  [NEST] 父对象 ID=${parent.id}, 模拟深插入`)
      } else {
        console.log(`  [NEST] 跳过: 无 dataFinder.business_object`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-business_object-PER-survives_reload
   */
  test('PER_RELOAD: [业务对象] 刷新后数据仍存在 (软断言)', async ({
    page, dataFinder, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PER_business_object', async () => {
      const obj = await dataFinder.business_object().catch(() => null)
      if (obj) {
        await navigateTo(page, '/business_object-management')
        ...
        console.log(`  [PER] 跳过: 无 dataFinder.business_object`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
  ...
  /**
   * UI 导航: 进入 [业务对象] 列表 (Healer 守护)
   */
  test('C_UI_NAV: 导航到 [业务对象] 列表', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    await withStep(page, testInfo, '导航到 [业务对象] 列表 (软断言)', async () => {
      const r = await AIHealer.guard(page, 'C_UI_NAV_business_object', async () => {
        await navigateTo(page, '/business_object-management')
      }, { softOn: ['404'] })
