...
test.describe('S-BF-RELATIONSHIP-AUTO: 业务关系 - 业务流 (AI 派生)', () => {
  ...
  test('AUD_CREATE: create 应产生 INFO 审计', async ({
    ...
    const r = await AIHealer.guard(page, 'AUD_relationship_create', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'relationship', null, 'create'
      )
   ...
   * 业务规则: BR-relationship-AUDIT-update
   */
  test('AUD_UPDATE: update 应产生 INFO 审计', async ({
    page, dataFinder, isolation
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'AUD_relationship_update', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'relationship', null, 'update'
      )
   ...
   * 业务规则: BR-relationship-AUDIT-delete
   */
  test('AUD_DELETE: delete 应产生 WARN 审计', async ({
    page, dataFinder, isolation
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'AUD_relationship_delete', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'relationship', null, 'delete'
      )
   ...
   * 业务规则: BR-relationship-PAG-default
   */
  test('PAG_DEFAULT: 验证分页默认配置', async ({
    page, navigateTo, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PAG_relationship', async () => {
  ...
  /**
   * deep_link 规则: detail=/detail/relationship/relationship-detail
   * 业务规则: BR-relationship-DL-detail
   */
  test('DL_DETAIL: 直接访问详情页深链 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'DL_relationship', async () => {
      const obj = await dataFinder.relationship().catch(() => null)
      if (obj && obj.id) {
        await navigateToDeepLink(page, 'relationship', obj.id)
        await page.waitForURL('**/detail/relationship/relationship-detail**', { timeout: 5000 })
        console.log(`  [DL] 深链访问成功`)
      } else {
        console.log(`  [DL] 跳过: 无 dataFinder.relationship`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
  ...
  /**
   * ui_badge 规则: is_in_scope 字段彩色标签
   * 业务规则: BR-relationship-BADGE-is_in_scope
   */
  test('BADGE_IS_IN_SCOPE: 验证 [is_in_scope] 标签颜色 (软断言)', async ({
    page, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'BADGE_relationship_is_in_scope', async () => {
      await navigateTo(page, '/relationship-management')
      const tag = page.locator('.el-tag').first()
      const visible = await tag.isVisible({ timeout: 3000 }).catch(() => false)
      console.log(`  [BADGE] is_in_scope tag visible=${visible}`)
    }, { softOn: ['5xx', '404'] })
   ...
   * 业务规则: BR-relationship-NEST-atomic
   */
  test('NEST_CREATE: 深插入 [业务关系] + 子对象 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'NEST_relationship', async () => {
      const parent = await dataFinder.relationship().catch(() => null)
      if (parent) {
        const nestedPOM = new NestedPOM(page)
        console.log(`  [NEST] 父对象 ID=${parent.id}, 模拟深插入`)
      } else {
        console.log(`  [NEST] 跳过: 无 dataFinder.relationship`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-relationship-PER-survives_reload
   */
  test('PER_RELOAD: [业务关系] 刷新后数据仍存在 (软断言)', async ({
    page, dataFinder, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PER_relationship', async () => {
      const obj = await dataFinder.relationship().catch(() => null)
      if (obj) {
        await navigateTo(page, '/relationship-management')
        ...
        console.log(`  [PER] 跳过: 无 dataFinder.relationship`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
