/**
 * S-BF-EI: 枚举管理 - 集成/边界 E2E 测试 (P1 补齐)
 *
 * 从 features/enum-integration.spec.js 适配到 v2 风格
 * 覆盖 (8 测):
 *   E31: 持久化: 刷新页面后值仍存在
 *   E32: 多 tab: 开 2 个 enum_type 详情, 互不干扰
 *   E33: URL 深链: 直接访问 /detail/enum_type/{id}
...
import { findSystemEnum, findEnumValues, findSystemEnumValue } from '../helpers/enum-finder.js'

// ============================================================
// E31-E34: 持久化/多 tab/深链/i18n
// ============================================================

test.describe('S-BF-EI: 枚举管理 - 集成/边界 (P1 补齐)', () => {
  ...
  test('E33: URL 深链 - 直接访问 /detail/enum_type/{id}', async ({ page, navigateTo, waitForApiFn }, testInfo) => {
    const target = await findSystemEnum(page)
    if (!target) {
      test.skip(true, 'no system enum')
      return
...
test.describe('S-BF-EI: 枚举管理 - 性能/健康/审计 (P1 补齐)', () => {
  ...
  test('E37: 审计日志 - 创建 enum_value 产生 operation INFO', async ({ page, isolation }, testInfo) => {
    const target = await findSystemEnum(page)
    if (!target) {
      test.skip(true, 'no system enum')
      return
    ...
    await withStep(page, testInfo, 'V2 BO 创建 enum_value', async () => {
      try {
        const v = await isolation.createTracked('enum_value', {
        ...
        test.skip(true, `create failed: ${e.message}`)
      }
    })
    ...
    await withStep(page, testInfo, '查审计日志 (operation INFO)', async () => {
      ...
      if (!r.ok()) {
        console.log(`[E37] audit log API 不可用: ${r.status()}`)
        return
      }
  ...
  test('E38: 审计日志 - 失败 system_value update 产生 operation ERROR', async ({ page }, testInfo) => {
    ...
    await withStep(page, testInfo, 'V2 BO PUT 尝试编辑 system_value', async () => {
    ...
    await withStep(page, testInfo, '查审计日志 (operation ERROR)', async () => {
      ...
      if (!r.ok()) {
        console.log(`[E38] audit API 不可用: ${r.status()}`)
        return
      }
      const b = await r.json()
      const items = b?.data?.items || b?.data?.records || b?.data || []
