...
test.describe('S-BF-PERMISSION_BUNDLE-AUTO: 权限包 - 业务流 (AI 派生)', () => {
   ...
   * 业务规则: BR-permission_bundle-PAG-default
   */
  test('PAG_DEFAULT: 验证分页默认配置', async ({
    page, navigateTo, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PAG_permission_bundle', async () => {
  ...
  /**
   * deep_link 规则: detail=/detail/permission_bundle/permission_bundle-detail
   * 业务规则: BR-permission_bundle-DL-detail
   */
  test('DL_DETAIL: 直接访问详情页深链 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'DL_permission_bundle', async () => {
      const obj = await dataFinder.permission_bundle().catch(() => null)
      if (obj && obj.id) {
        await navigateToDeepLink(page, 'permission_bundle', obj.id)
        await page.waitForURL('**/detail/permission_bundle/permission_bundle-detail**', { timeout: 5000 })
        console.log(`  [DL] 深链访问成功`)
      } else {
        console.log(`  [DL] 跳过: 无 dataFinder.permission_bundle`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-permission_bundle-HEALTH
   */
  test('HEALTH: [权限包] 列表健康检查', async ({
    page, navigateTo
  }, testInfo) => {
    const errors = []
    page.on('pageerror', e => errors.push('pageerror: ' + e.message))
    page.on('console', msg => { if (msg.type() === 'error') errors.push('console: ' + msg.text()) })
    const r = await AIHealer.guard(page, 'HEALTH_permission_bundle', async () => {
      await navigateTo(page, '/permission_bundle-management')
   ...
   * 业务规则: BR-permission_bundle-BADGE-is_active
   */
  test('BADGE_IS_ACTIVE: 验证 [is_active] 标签颜色 (软断言)', async ({
    page, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'BADGE_permission_bundle_is_active', async () => {
      await navigateTo(page, '/permission_bundle-management')
   ...
   * 业务规则: BR-permission_bundle-BADGE-is_system
   */
  test('BADGE_IS_SYSTEM: 验证 [is_system] 标签颜色 (软断言)', async ({
    page, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'BADGE_permission_bundle_is_system', async () => {
      await navigateTo(page, '/permission_bundle-management')
      const tag = page.locator('.el-tag').first()
      const visible = await tag.isVisible({ timeout: 3000 }).catch(() => false)
   ...
   * 业务规则: BR-permission_bundle-PER-survives_reload
   */
  test('PER_RELOAD: [权限包] 刷新后数据仍存在 (软断言)', async ({
    page, dataFinder, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PER_permission_bundle', async () => {
      const obj = await dataFinder.permission_bundle().catch(() => null)
      if (obj) {
        await navigateTo(page, '/permission_bundle-management')
        ...
        console.log(`  [PER] 跳过: 无 dataFinder.permission_bundle`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
