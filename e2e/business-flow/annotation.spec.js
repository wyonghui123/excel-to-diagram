...
test.describe('S-BF-ANNOTATION-AUTO: 备注信息 - 业务流 (AI 派生)', () => {
  ...
  test('AUD_CREATE: create 应产生 INFO 审计', async ({
    ...
    const r = await AIHealer.guard(page, 'AUD_annotation_create', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'annotation', null, 'create'
      )
   ...
   * 业务规则: BR-annotation-AUDIT-update
   */
  test('AUD_UPDATE: update 应产生 INFO 审计', async ({
    page, dataFinder, isolation
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'AUD_annotation_update', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'annotation', null, 'update'
      )
   ...
   * 业务规则: BR-annotation-AUDIT-delete
   */
  test('AUD_DELETE: delete 应产生 WARN 审计', async ({
    page, dataFinder, isolation
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'AUD_annotation_delete', async () => {
      const valid = await BusinessRuleAssertor.assertAuditLogExists(
        page, 'annotation', null, 'delete'
      )
   ...
   * 业务规则: BR-annotation-PAG-default
   */
  test('PAG_DEFAULT: 验证分页默认配置', async ({
    page, navigateTo, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PAG_annotation', async () => {
  ...
  /**
   * deep_link 规则: detail=/detail/annotation/annotation-detail
   * 业务规则: BR-annotation-DL-detail
   */
  test('DL_DETAIL: 直接访问详情页深链 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'DL_annotation', async () => {
      const obj = await dataFinder.annotation().catch(() => null)
      if (obj && obj.id) {
        await navigateToDeepLink(page, 'annotation', obj.id)
        await page.waitForURL('**/detail/annotation/annotation-detail**', { timeout: 5000 })
        console.log(`  [DL] 深链访问成功`)
      } else {
        console.log(`  [DL] 跳过: 无 dataFinder.annotation`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
   ...
   * 业务规则: BR-annotation-HEALTH
   */
  test('HEALTH: [备注信息] 列表健康检查', async ({
    page, navigateTo
  }, testInfo) => {
    const errors = []
    page.on('pageerror', e => errors.push('pageerror: ' + e.message))
    page.on('console', msg => { if (msg.type() === 'error') errors.push('console: ' + msg.text()) })
    const r = await AIHealer.guard(page, 'HEALTH_annotation', async () => {
      await navigateTo(page, '/annotation-management')
   ...
   * 业务规则: BR-annotation-PER-survives_reload
   */
  test('PER_RELOAD: [备注信息] 刷新后数据仍存在 (软断言)', async ({
    page, dataFinder, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PER_annotation', async () => {
      const obj = await dataFinder.annotation().catch(() => null)
      if (obj) {
        await navigateTo(page, '/annotation-management')
        ...
        console.log(`  [PER] 跳过: 无 dataFinder.annotation`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
