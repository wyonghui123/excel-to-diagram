...
test.describe('S-BF-TEST_TABLE-AUTO: 测试表 - 业务流 (AI 派生)', () => {
   ...
   * 业务规则: BR-test_table-PAG-default
   */
  test('PAG_DEFAULT: 验证分页默认配置', async ({
    page, navigateTo, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PAG_test_table', async () => {
  ...
  /**
   * deep_link 规则: detail=/detail/test_table/test_table-detail
   * 业务规则: BR-test_table-DL-detail
   */
  test('DL_DETAIL: 直接访问详情页深链 (软断言)', async ({
    page, dataFinder
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'DL_test_table', async () => {
      const obj = await dataFinder.test_table().catch(() => null)
      if (obj && obj.id) {
        await navigateToDeepLink(page, 'test_table', obj.id)
        await page.waitForURL('**/detail/test_table/test_table-detail**', { timeout: 5000 })
        console.log(`  [DL] 深链访问成功`)
      } else {
        console.log(`  [DL] 跳过: 无 dataFinder.test_table`)
      }
   ...
   * 业务规则: BR-test_table-HEALTH
   */
  test('HEALTH: [测试表] 列表健康检查', async ({
    page, navigateTo
  }, testInfo) => {
    const errors = []
    page.on('pageerror', e => errors.push('pageerror: ' + e.message))
    page.on('console', msg => { if (msg.type() === 'error') errors.push('console: ' + msg.text()) })
    const r = await AIHealer.guard(page, 'HEALTH_test_table', async () => {
      await navigateTo(page, '/test_table-management')
   ...
   * 业务规则: BR-test_table-PER-survives_reload
   */
  test('PER_RELOAD: [测试表] 刷新后数据仍存在 (软断言)', async ({
    page, dataFinder, navigateTo
  }, testInfo) => {
    const r = await AIHealer.guard(page, 'PER_test_table', async () => {
      const obj = await dataFinder.test_table().catch(() => null)
      if (obj) {
        await navigateTo(page, '/test_table-management')
        ...
        console.log(`  [PER] 跳过: 无 dataFinder.test_table`)
      }
    }, { softOn: ['5xx', '404', 'fk_missing'] })
