/**
 * S-BF-ROLE-AUTO: 角色 - 业务流 E2E (AI 派生, 阶段三)
 *
 * [自动生成] 从 role.yaml 自动生成
 * [E2E v2 铁律合规 (8 项)]
 ...
 *
 * 业务规则:
 *   BR-role-FLD-REQ-code  (角色编码 必填)
 *   BR-role-FLD-REQ-name  (角色名称 必填)
...
const ROLE_URL = '/role-management'

test.describe('S-BF-ROLE-AUTO: 角色 - 业务流 (AI 派生)', () => {

  /**
   * 必填字段校验: 角色编码 (code)
   * 业务规则: BR-role-FLD-REQ-code
   * [4 维度评估] API/UI/Business/Multi
   */
  test('C_REQ_CODE: 缺少必填字段 [角色编码] 应被拒绝', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    await withStep(page, testInfo, '业务断言: 缺少 [角色编码] 应被拒绝 (API 4xx/5xx)', async () => {
      const result = await BusinessRuleAssertor.assertFieldRequired(page, 'role', {
        name: "placeholder_name",
      }, 'code')
      expect(result, '[API 维度] 缺少 [角色编码] 应返回 4xx/5xx 或 success=false').toBe(true)
    })
  })
  ...
  /**
   * 必填字段校验: 角色名称 (name)
   * 业务规则: BR-role-FLD-REQ-name
   * [4 维度评估] API/UI/Business/Multi
   */
  test('C_REQ_NAME: 缺少必填字段 [角色名称] 应被拒绝', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    await withStep(page, testInfo, '业务断言: 缺少 [角色名称] 应被拒绝 (API 4xx/5xx)', async () => {
      const result = await BusinessRuleAssertor.assertFieldRequired(page, 'role', {
        code: "TEST_CODE_PLACEHOLDER",
      }, 'name')
      expect(result, '[API 维度] 缺少 [角色名称] 应返回 4xx/5xx 或 success=false').toBe(true)
    })
  })
  ...
  /**
   * 唯一性校验: 角色编码 (code)
   * 业务规则: BR-role-FLD-UNQ-code
   */
  test('C_UNQ_CODE: 重复 [角色编码] 应被拒绝', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    const TS = Date.now()
    const UNQ_VALUE = `$UNQ_TEST_PLACEHOLDER_' + '${TS}`
    await withStep(page, testInfo, '业务断言: 重复 [角色编码] 应被拒绝', async () => {
      let failed = false
      try {
        await isolation.createTracked('role', {
        code: UNQ_VALUE,
        name: "placeholder_name",
        })
        // 再创建一次相同值
      ...
      if (!failed) {
        console.warn('[C_UNQ_CODE] 后端未拒绝重复 [角色编码], 跳过验证')
      }
    })
  })
  ...
  /**
   * 枚举值校验: 启用状态 (is_active)
   * 业务规则: BR-role-FLD-ENUM-is_active
   * 允许值: [{'value': 1, 'label': '启用中', 'color': 'success'}, {'value': 0, 'label': '已停用', 'color': 'default'}]
   */
  test('C_ENUM_IS_ACTIVE: [启用状态] 非法枚举值应被拒绝', async ({
    page
  }, testInfo) => {
    await withStep(page, testInfo, '业务断言: [启用状态] 非法枚举应被拒', async () => {
      const result = await BusinessRuleAssertor.assertFieldEnum(
        page, 'role', {
        code: "TEST_CODE_PLACEHOLDER",
        name: "placeholder_name",
          is_active: 'INVALID_ENUM_VALUE_999'
        }, [{'value': 1, 'label': '启用中', 'color': 'success'}, {'value': 0, 'label': '已停用', 'color': 'default'}]
      )
  ...
  /**
   * 枚举值校验: 系统角色 (is_system)
   * 业务规则: BR-role-FLD-ENUM-is_system
   * 允许值: [{'value': 1, 'label': '系统角色', 'color': 'info'}, {'value': 0, 'label': '自定义角色', 'color': 'default'}]
   */
  test('C_ENUM_IS_SYSTEM: [系统角色] 非法枚举值应被拒绝', async ({
    page
  }, testInfo) => {
    await withStep(page, testInfo, '业务断言: [系统角色] 非法枚举应被拒', async () => {
      const result = await BusinessRuleAssertor.assertFieldEnum(
        page, 'role', {
        code: "TEST_CODE_PLACEHOLDER",
        name: "placeholder_name",
          is_system: 'INVALID_ENUM_VALUE_999'
        }, [{'value': 1, 'label': '系统角色', 'color': 'info'}, {'value': 0, 'label': '自定义角色', 'color': 'default'}]
      )
      expect(result, '[Enum] 非法枚举值应被拒').toBe(true)
    })
  })
  ...
  /**
   * 审计日志: 创建 [角色] 应记录 audit_log
   * 业务规则: BR-role-AUDIT-create
   * [Healer.L1+L3] createTracked + audit_log 失败都软断言
   */
  test('C_AUDIT: [角色] 创建应生成 audit_log', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    const TS = Date.now()
    let obj = null
    const cr = await withStep(page, testInfo, '创建 [角色] (Healer 守护)', async () => {
      return await AIHealer.guard(page, 'C_AUDIT_role_create', async () => {
        return await isolation.createTracked('role', {
        code: `AUD_CODE_${TS}`,
        name: `aud_name_${TS}`,
        })
    ...
    await withStep(page, testInfo, '业务断言: audit_log 应记录创建事件 (Healer 守护)', async () => {
      const r = await AIHealer.guard(page, 'C_AUDIT_role_check', async () => {
        const valid = await BusinessRuleAssertor.assertAuditLogExists(
          page, 'role', obj.id, 'create'
        )
   ...
   * 业务规则: BR-role-HEALTH
   */
  test('HEALTH: [角色] 列表健康检查', async ({
    page, navigateTo
  }, testInfo) => {
    const errors = []
    page.on('pageerror', e => errors.push('pageerror: ' + e.message))
    page.on('console', msg => { if (msg.type() === 'error') errors.push('console: ' + msg.text()) })
    const r = await AIHealer.guard(page, 'HEALTH_role', async () => {
      await navigateTo(page, '/role-management')
  ...
  /**
   * UI 导航: 进入 [角色] 列表 (Healer 守护)
   */
  test('C_UI_NAV: 导航到 [角色] 列表', async ({
    page, dataFinder, navigateTo, isolation, waitForApiFn
  }, testInfo) => {
    await withStep(page, testInfo, '导航到 [角色] 列表 (软断言)', async () => {
      const r = await AIHealer.guard(page, 'C_UI_NAV_role', async () => {
        await navigateTo(page, '/role-management')
      }, { softOn: ['404'] })
