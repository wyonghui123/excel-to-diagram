...
| 13 | **决策日志强制** | 任何规则边界判断前必须记录理由+替代方案（`scripts/decision_log.py`）|
...
### 铁律 13：决策日志强制（V2 元反馈新增）

**触发**：任何规则边界判断（特别是违规决策）

**背景**：另一个 Agent 反馈"自圆其说违反规则"——例如"single-file → 风险低 → 主工作树 commit"。
**根因**：规则约束 ≠ 技术约束，Agent 缺乏"决策可追溯"机制。
...
python scripts/decision_log.py violate \
    ...
    --risk low \
    --pm-authorized  # 必须有 PM 授权
```

**3 大原则**：
1. **理由必须充分** —— "风险低"不算理由，必须量化（见 violation_cost_report.md）
2. **替代方案必须列出** —— 不能"自圆其说"
3. **PM 授权阈值** —— 5+ 次触发规则体检
...
**配套文档**：
- `scripts/violation_cost_report.md` - 规则违反成本表（量化驱动合规）
...
- 铁律 25: 沙箱恢复自动检测
- 铁律 26: 修复完整性自动检查
- **🆕 铁律 27 (V2 元反馈新增): 决策日志强制**

### 7.4 V2 元反馈：默认假设执行者是 AI Agent

**问题**：原规则措辞按"用户"或"开发者"设计，但实际执行者多为 AI Agent。
**修改**：所有规则文本中的"用户"改为"AI Agent"或"调用方"。
**示例**：SESSION_REMINDER.md v3 (2026-06-21) 已调整。
...
| **🆕 `decision_log.py`** | `scripts/` | **决策日志工具（V2 元反馈铁律 13）** |
| **🆕 `violation_cost_report.md`** | `scripts/` | **规则违反成本表（量化驱动合规）** |
| **🆕 `debug_backend.py`** | `scripts/` | **后端调试 SOP（6 步检查）**（基于 2026-06-21 调试旧代码事故）|
| **🆕 `verify_backend_owner.py`** | `scripts/` | **进程所有者验证**（基于 2026-06-21 旧 python.exe 残留事故）|

### 7.6 调试事故双链（2026-06-21）

**事故 A：调试旧代码 3 小时**
- 17:27 启动后端运行旧代码
- 20:31 commit 了 WriteScopeDenied 修复
- Agent 不知道后端在跑旧代码，反复调试 3+ 小时
- **解决**：`debug_backend.py` Step 3 检测代码版本 vs 后端启动时间
- **核心检测**：同时检查 git commit 时间 + 文件 mtime（捕获未提交修改）

**事故 B：旧 python.exe 进程残留 1+ 小时**
- `restart_backend.py` 只杀 `pythonw.exe`，不杀 `python.exe`
- 旧 python.exe 进程（PID 16420/18060）继续处理请求
- 修复看似生效但实际无效
- **解决 1**：`restart_backend.py` `find_existing_backend()` 同时查找 python.exe + pythonw.exe
- **解决 2**：`service_manager.py` 新增 `_kill_all_backend_processes()`
- **解决 3**：`verify_backend_owner.py` 验证 PID 一致性 + 自动修复
- **解决 4**：`debug_backend.py` Step 3.5 进程所有者一致性检查
...
| 2026-06-21 | V2.1 | 元反馈：基于另一个 Agent 反馈，新增铁律 13（决策日志）+ 默认假设 AI Agent |