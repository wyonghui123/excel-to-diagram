...
```
[1] python scripts/debug/check_log_files.py
    → 自动识别哪个日志文件是活跃的（避免读错文件）

[2] python scripts/debug/env/diagnose.py
    → 端口监听 + health + PID 一致性

[3] python scripts/debug/log/extractor.py --since "01:25" --tail 50
    → 最近 50 条日志（支持短时间 pattern）

[4] python scripts/debug/inspect/user_context.py <test_user>
    → 测试用户的完整调试上下文

[5] python scripts/debug/inspect/table_schema.py <关键表> --check-code-fields
    → 表结构 + 字段映射错误检测
...
> **🚨 V3.1 重要变更（2026-06-21 调试事故复盘）**：
> - **不要用 `python scripts/service_manager.py restart-be`**（会丢日志！）
> - **必须用 `python scripts/debug/restart/restart_safe.py restart`**
> - 重启前先跑 `check_log_files.py` 识别活跃日志文件
> - **禁止在根目录写 debug_*.py / check_*.py / _restart*.ps1**（用 `check_debug_script_in_root.py` 检测）
> - **DATABASE_URL 未配置时，table_schema.py / user_context.py 会自动加载 .env**
> - 调试列名前先用 `python scripts/debug/inspect/table_schema.py <table> --check-code-fields`
> - **修复 caller bug 前用 `python scripts/debug/check_unused_return.py`**（检测 ActionResult 未检查的 caller）
> - 用 `python scripts/debug/inspect/code_map.py --show-flow` 一次性看完整执行流（避免反复读同一段代码）
