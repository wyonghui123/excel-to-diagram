...
- **Affected specs**:
  ...
  - `condition-based-permission` - 条件权限服务（已实现 permission_rule）
...
- **WHEN** 管理员在"管理维度配置"页面添加新的维度"成本中心（cost_center）"
...
- **AND** 点击"添加条件规则"
- **THEN** 显示条件规则编辑器，包含：
  - 条件字段选择器（如：id, name, code, domain_type）
...
- **WHEN** 用户选择"产品"管理维度
- **AND** 配置条件："product_id IN (1, 2, 3)"
- **THEN** 系统保存该条件规则到 `permission_rule` 表
- **AND** resource_type = 'product'
- **AND** condition = 'product_id IN (1, 2, 3)'

#### Scenario: 配置权限级别和继承规则

- **WHEN** 用户配置完条件表达式后
- **THEN** 可以选择权限级别（read/write/admin）
- **AND** 可以配置向下继承（inherit_to_children）
- **AND** 可以配置向上传播（propagate_to_parents）
- **AND** 可以配置是否禁止（is_denied）
...
- **AND** 启用"向下继承"（inherit_to_children = true）
- **THEN** 系统计算影响范围：
  - 直接影响：2 个领域（ID=1, 2）
...
- **AND** 启用"向上传播"（propagate_to_parents = true）
- **THEN** 系统计算受影响的业务对象
...
- 支持编辑、删除、启用/禁用
- 支持排序、过滤
- 显示规则的 resource_type 和 condition
