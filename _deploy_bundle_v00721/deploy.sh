...
# ========================= PHASE 0.5: 解压 zip =========================
# [V007.20 patch] 2026-07-06 23:07:24 - elif fix + content check
...
if [ "$SKIP_UNZIP" != "true" ]; then
    ...
    # [V007.20 patch] 内容检查: 关键修复未部署则触发解压
    if [ -f "$SERVER_DIR/core/sql_connection_pool.py" ]; then
        if ! grep -q "busy_timeout.*30000" "$SERVER_DIR/core/sql_connection_pool.py" 2>/dev/null; then
            NEED_UNZIP=true
            info "触发解压: V007.20 busy_timeout=30000 修复未部署"
        fi
    fi
    if [ -f "$SERVER_DIR/services/import_export_service.py" ]; then
        if ! grep -q "skip_audit=True" "$SERVER_DIR/services/import_export_service.py" 2>/dev/null; then
            NEED_UNZIP=true
            info "触发解压: V007.20 skip_audit 修复未部署"
        fi
    fi
    # [V007.21 patch] 2026-07-06 - cache_manager sync lock 检查
    if [ -f "$SERVER_DIR/core/enums/cache_manager.py" ]; then
        # 期望 0 个 async with self._lock, 至少 3 个 sync with self._lock:
        if grep -q "async with self._lock" "$SERVER_DIR/core/enums/cache_manager.py" 2>/dev/null; then
            NEED_UNZIP=true
            info "触发解压: V007.21 cache_manager async with self._lock 未修复 (Python 3.14 TypeError)"
        fi
    fi
fi
if [ "$NEED_UNZIP" = "true" ]; then
    if [ -f "$ZIP_PATH" ]; then
        ...
        # [FIX v007.21 2026-07-06] 把根目录解压的内容移到 VERSION_PATH
        #   背景: PHASE 0.5 unzip 到 $DEPLOYMENTS_DIR/ 根 (共享 meta/ + frontend_dist_files/),
        #   但 PHASE 7 切链接 target 是 $DEPLOYMENTS_DIR/$VERSION/ (如 v20260706_021),
        #   之前这个目录没创建 → current 断链 + 旧版代码继续跑
        #   修法: 创建 $VERSION_PATH 后 mv 根目录内容进去
        VERSION_PATH="$DEPLOYMENTS_DIR/$VERSION"
        if [ ! -d "$VERSION_PATH" ]; then
            mkdir -p "$VERSION_PATH"
            info "创建 VERSION_PATH: $VERSION_PATH"
        fi
        # mv 根目录的子目录和顶层文件 (排除版本子目录自身)
        for item in meta frontend_dist_files MANIFEST telemetry mcp; do
            if [ -e "$DEPLOYMENTS_DIR/$item" ] && [ ! -e "$VERSION_PATH/$item" ]; then
                mv "$DEPLOYMENTS_DIR/$item" "$VERSION_PATH/$item"
                info "mv $DEPLOYMENTS_DIR/$item → $VERSION_PATH/$item"
            fi
        done

        # [FIX v007.21 2026-07-06] 验证 VERSION_PATH/m
        [ -d "$VERSION_PATH/meta" ] && ok "VERSION_PATH/meta 就绪" || err "VERSION_PATH/meta 仍缺"
        [ -d "$VERSION_PATH/frontend_dist_files" ] && ok "VERSION_PATH/frontend_dist_files 就绪" || err "VERSION_PATH/frontend_dist_files 仍缺"
