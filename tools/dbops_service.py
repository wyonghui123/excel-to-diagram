...
VERSION = "v0.2"
# [L13.3] 端口 9214: 与 dbops_service V1.0 (9204) 共存, 各自职责不同
# 9204 = DB 物理层 (backup/restore), 9214 = 审计逻辑层 (audit_recovery)
PORT = int(os.environ.get("DBOPS_AUDIT_SERVICE_PORT", 9214))
BIND = os.environ.get("DBOPS_AUDIT_SERVICE_BIND", "0.0.0.0")
SECRET = os.environ.get("DBOPS_AUDIT_SERVICE_SECRET", "v007.63-audit-recovery")
    ...
    def do_GET(self):
        ...
        if url.path == "/api":
            return self._json(200, {
                "service": "dbops_audit_service",
                "version": VERSION,
                "port": PORT,
                "bind": BIND,
                "role": "audit_recovery (业务逻辑层, 与 9204 dbops_service V1.0 共存)",
...
def main():
    print(f"[dbops_service] v{VERSION} on {BIND}:{PORT}", flush=True)
