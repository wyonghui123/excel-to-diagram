#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Write WILDCARD_SUPPORT_RESEARCH.md"""
import os

filepath = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'docs', 'WILDCARD_SUPPORT_RESEARCH.md')

content = r"""# Dimension Scope 和 Condition 的 `*` 通配符支持研究
...
|------|---------|---------|-------------------|
| **AWS IAM** | `Resource: "*"` / `Action: "*"` = 全量访问 | Implicit Deny（未显式允许=拒绝） | Yes |
| **SAP CAP** | `@restrict` 无 where 子句 = 全量访问 | 默认无访问 | Yes |
| **Salesforce** | OWD "Public Read/Write" = 全量 | OWD 默认 "Private" | Yes |
| **SpiceDB** | `relation view: user:*` = 所有人可访问 | 无关系=无权限 | Yes |
| **Cedar** | `resource in Group::*` = 组内所有资源 | 默认拒绝 | Yes |
| **XACML** | `<AnyResource/>` / `<AnyAction/>` = 全量 | Deny-overrides = 默认拒绝 | Yes |
...
def expand_dimension_values(self, role_id: int):
    ...
    if isinstance(raw_dv, str):
        values = set(json.loads(raw_dv))  # FAIL: 只支持 JSON list, 不支持 '*'
```
...
| 层级 | 当前支持 `*` | 需要支持 |
|------|------------|---------|
| 功能权限 (PermissionInterceptor) | Yes | - |
| Dimension Scope (DimensionScopeEngine) | No | `scope_mode='all'` 或 `dimension_values='*'` |
| Condition (ConditionEvaluator) | No | `condition='*'` 表示全量匹配 |
| 写路径 (WriteScopeInterceptor) | Yes 功能权限 `*` 跳过 | dimension scope `*` 也需跳过 |
...
- [INDUSTRY_PERMISSION_RESEARCH_OVERVIEW.md](INDUSTRY_PERMISSION_RESEARCH_OVERVIEW.md)
- [CLOUD_IAM_ARCHITECTURE_RESEARCH.md](CLOUD_IAM_ARCHITECTURE_RESEARCH.md)
- [PERMISSION_ACADEMIC_MODELS_RESEARCH.md](PERMISSION_ACADEMIC_MODELS_RESEARCH.md)
- [PERMISSION_TODOS.md](PERMISSION_TODOS.md)
"""

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print(f'OK: wrote {len(content)} chars to {filepath}')
