    ...
    try:
        ...
        if meta.exists():
            def _ignore_exclude_runtimes(directory, files):
                ...
                for f in files:
                    ...
                    # [V007.35 FIX 22:30] 之前 _tmp / _test 都误杀, 导致测试脚本进不了 zip
                    # 修正: 只排除 tmp_ / temp_ 开头, 不排除 _test_ / _v*
                    if f.startswith('tmp_') or f.startswith('temp_') or f == '_tmp':
                        excluded.append(f); continue
                return excluded
        ...
        if not is_delta and not args.skip_bundle_sync:
            ...
            if not deploy_bundle_dir.exists():
                ...
                # [V007.35 AUTO-DELTA] 自动从 working tree + git 拿当前所有 meta/ + tools/ 文件
                #   验证每个文件是否真在 zip 内
                #   目的: 防止 V007.34/V007.35 类 bug 复发 (新文件进 working tree 但没进 zip)
                #   反思: 之前 V8/V8b 都是手动加, 永远滞后. 现在自动从 working tree 拿.
                #   修正: 之前用 git log 历史, 抓到已删除文件. 现改用 working tree 实际存在.
                print(f"[V007.35] ========== AUTO-DELTA verification (working tree vs zip) ==========")
                try:
                    delta_result = subprocess.run(
                        [sys.executable, "-c", f"""
import subprocess, zipfile, sys, os
from pathlib import Path

# 从 working tree 实际存在的文件拿 (不用 git log, 避免抓到已删除)
files = set()
for sub in ('meta', 'tools'):
    sub_path = Path(r'{ROOT}') / sub
    if not sub_path.exists():
        continue
    for f in sub_path.rglob('*'):
        if f.is_file():
            rel = str(f.relative_to(Path(r'{ROOT}'))).replace('\\\\', '/')
            files.add(rel)

# 检查每个文件是否在 zip 内
zip_path = Path(r'{out_path}')
...
# 排除已知 zip 不含的 (sub module, tests, db 主文件, .pyc, .bak, .env, .lock, __pycache__, backups/)
def _is_excluded(rel: str) -> bool:
    skip_patterns = [
        '__pycache__/', '/__pycache__/', '.pyc', '.db', '.db-wal', '.db-shm',
        '.bak', '.backup', '.env', '.lock',
        'meta/backups/', 'meta/logs/', 'meta/screenshots/', 'meta/db_monitor_logs/',
        'meta/migrations/', 'meta/scripts/init_menu_permissions.py',  # runtime
        'meta/scripts/migrate_', 'meta/scripts/seed_',
    ]
...
files_in_scope = {{f for f in files if not _is_excluded(f)}}
missing = sorted(files_in_scope - zip_files)
if missing:
    print(f'  [FAIL] {{len(missing)}} 个 working tree 文件没在 zip 内:')
    for m in missing[:15]:
        print(f'    - {{m}}')
    sys.exit(1)
else:
    print(f'  [OK] {{len(files_in_scope)}} 个 working tree 文件全在 zip 内')
"""],
                        capture_output=True, text=True, timeout=30
                    )
                    for line in delta_result.stdout.splitlines():
                        print(f"  {line}")
                    if delta_result.returncode != 0:
                        print(f"  [X] AUTO-DELTA 失败! git delta 文件没全进 zip")
                        print(f"  [IMPORTANT] 新 commit 的文件没被打包, 不能部署")
                        print(f"  [FIX] 1) 看 git log 哪些文件 2) 修 rebuild_zip.py exclude 规则 3) 重打包")
                        sys.exit(1)
                except Exception as e:
                    print(f"  [WARN] AUTO-DELTA 异常: {{e}} (跳过)")
