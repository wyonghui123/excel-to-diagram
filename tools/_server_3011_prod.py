...
""" % (port, existing_info), file=sys.stderr)
    ...
    try:
        ...
        # [L4 NSFOCUS 2026-07-14] host 默认 0.0.0.0, 改读 SERVER_BIND_HOST env (默认 172.20.59.7)
        bind_host = os.environ.get("SERVER_BIND_HOST", "172.20.59.7")
        print(f"[SERVER] BIND host: {bind_host} (L4 NSFOCUS)", flush=True)
        app.run(
            host=bind_host,
            port=port,
            debug=debug_mode,
