...
Provides:
  ...
  - verify_self_integrity() -> bool  (P0-4: detect file corruption)

[P0-4 2026-09-01] Self-integrity verification:
  On 2026-09-01 delta deployment, tools/_build_delta.py was found corrupted
  (UTF-16 LE BOM + U+FFFD replacement bytes - 145KB file unreadable).
  This function checks for known corruption signatures:
    - UTF-16 LE BOM without valid ASCII content
    - Null bytes in code (indicates wrong encoding)
    - Truncated content (file < 1KB)
  Run at startup of every delta CLI: `manifest_utils.verify_self_integrity()`
"""
import os
import hashlib
import yaml
...
_SELF_MIN_SIZE = 1024  # 1KB minimum (any smaller = truncated/corrupt)
_SELF_KNOWN_BAD_SIGNATURES = [
    b'\xff\xfe',  # UTF-16 LE BOM (this file is UTF-8, not UTF-16)
    b'\xfe\xff',  # UTF-16 BE BOM
    b'\xef\xbb\xbf\xef',  # Double UTF-8 BOM (corruption)
]


def verify_self_integrity(path: Optional[Path] = None) -> bool:
    """Check if manifest_utils.py itself is not corrupted.

    Detects:
    - UTF-16 LE BOM (trae-sandbox write bug from 2026-09-01)
    - File too small (< 1KB = truncated)
    - Excessive null bytes (wrong encoding)

    Args:
        path: optional override (defaults to this file)

    Returns:
        True if file appears intact.

    Raises:
        RuntimeError: if file is corrupted (caller should NOT proceed).
    """
    p = path or _SELF_PATH
    if not p.exists():
        raise RuntimeError(f"manifest_utils.py missing: {p}")

    raw = p.read_bytes()
    size = len(raw)

    if size < _SELF_MIN_SIZE:
        raise RuntimeError(
            f"manifest_utils.py too small ({size} bytes < {_SELF_MIN_SIZE}). "
            f"Likely truncated/corrupted. Restore from git: "
            f"`git checkout HEAD -- tools/manifest_utils.py`"
        )

    # Detect wrong encoding (BOM mismatches)
    for sig in _SELF_KNOWN_BAD_SIGNATURES:
        if raw.startswith(sig):
            raise RuntimeError(
                f"manifest_utils.py has wrong encoding signature {sig.hex()}. "
                f"trae-sandbox wrote this file as UTF-16 LE. "
                f"Fix: re-save as UTF-8 no-BOM, or `git checkout HEAD -- tools/manifest_utils.py`"
            )

    # Detect UTF-16 LE pattern (every other byte is null)
    if size >= 10:
        null_count = sum(1 for i in range(0, min(size, 100), 2) if raw[i+1] == 0)
        if null_count > 30:
            raise RuntimeError(
                f"manifest_utils.py appears to be UTF-16 LE encoded "
                f"({null_count}/50 odd bytes are null). "
                f"Fix: convert to UTF-8 no-BOM."
            )

    # Try to parse as Python source
    try:
        compile(raw.decode('utf-8'), str(p), 'exec')
    except (SyntaxError, UnicodeDecodeError) as e:
        raise RuntimeError(
            f"manifest_utils.py has Python syntax error: {e}. "
            f"File likely corrupted."
        )
...
try:
    ...
    # Print to stderr but don't raise - allow tests to skip
    import sys
...
class Manifest:
    ...
    extra: dict = field(default_factory=dict)  # [P1-10] holds db_fingerprint, etc.
