...
def main():
    ...
    try:
        ...
        nodes_info = page.evaluate("""() => {
            ...
            for (const [key, node] of Object.entries(store.nodesMap || {})) {
                nodes.push({
                    ...
                    parentName: node.parent?.data?.name?.substring(0, 30) || ''
                })
            }
            ...
            return { total: nodes.length, nodes: nodes.slice(0, 50) }
        }""")
        print(f"节点信息:")
        ...
        for n in nodes_info.get('nodes', []):
            ...
            else: flags.append('branch')
            if n['expanded']: flags.append('展开')
            if n['checked']: flags.append('勾选')
            if n['visible'] is False: flags.append('隐藏')
            flag_str = f" [{', '.join(flags)}]"
            parent_str = f" (父: {n['parentName']})" if n['parentName'] else ""
            print(f"    {n['name']}{flag_str}{parent_str}")

        # 截图
        cli.screenshot('d:/filework/excel-to-diagram/tests/e2e/diag_tree_structure.png')
