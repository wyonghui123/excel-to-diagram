...
def main():
    ...
    db_path = Path(args.deployments) / "meta" / "architecture.db"
    if db_path.exists():
        try:
            sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent.parent / "tools"))
            try:
                from audit_coverage_check import check_coverage as _check_cov
            except ImportError:
                _check_cov = None
            if _check_cov:
                cov = _check_cov(str(db_path), lookback_days=30)
                ov = cov.get("overall", {})
                print(f"  ok: {ov.get('ok', 0)}, warn: {ov.get('warn', 0)}, fail: {ov.get('fail', 0)}")
                if ov.get("fail", 0) > 0:
                    print("[FAIL] audit 覆盖率 < 50%, 需要补全")
                    report["summary"]["drift"] += 1
            else:
                print("[SKIP] audit_coverage_check 模块不可用")
        except Exception as e:
            print(f"[WARN] audit_coverage_check 失败: {e}")
    else:
        print(f"[SKIP] db 不存在: {db_path}")

    sys.exit(1 if report["summary"]["drift"] > 0 else 0)
