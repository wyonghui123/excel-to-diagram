...
# -*- coding: utf-8 -*-
"""
测试 value_help 过滤器 E2E - 使用正确 URL (tab 参数)
"""
import os
import sys
                    ...
                    if vh_field.count() > 0:
                        ...
                        if el_select.count() > 0:
                            ...
                            if opt_count > 0:
                                ...
                                # 点击确认按钮
                                confirm_btn = page.locator('.filter-panel .el-button--primary')
                                if confirm_btn.count() > 0:
                                    confirm_btn.first.click()
                                    page.wait_for_timeout(1000)
                                    print(f"   [DECORATIVE] 点击了确认按钮")
                    else:
                        # 检查其他过滤器类型
                        el_select = page.locator('.el-select:visible')
                        el_input = page.locator('.el-input:visible')
                        date_picker = page.locator('.el-date-editor:visible')
                        if el_select.count() > 0:
                            print(f"   [WARNING]  过滤器类型是 select（不是 value_help）")
                        elif el_input.count() > 0:
                            print(f"   [WARNING]  过滤器类型是 search（不是 value_help）")
                        elif date_picker.count() > 0:
                            print(f"   [WARNING]  过滤器类型是 date-range（不是 value_help）")
                        else:
                            print(f"   [DECORATIVE] 没有找到任何过滤器组件")
