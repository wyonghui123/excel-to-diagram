"""
_diag_bo_switch.py - 诊断 BO 图切换颜色后节点变灰问题 (2026-08-02)

用户反馈: 业务对象图除了初始有颜色, 切换任何颜色选项后节点变灰, 连线颜色正确。
本脚本在 shortcut 路径复现: 进入 BO 图 → 依次切换 配色/颜色分组/中心范围,
dump 每步的节点 fill 分布 + 前几个节点 rect HTML + console 日志。

如果 shortcut 路径无法复现 (全彩), 说明问题只在手动"图表展示"按钮入口路径。
"""
import sys
import json
import base64
...
def get_node_stats(cli):
    """统计节点 fill 分布 + 中心边框 (兼容 attribute 与 style 两种 dasharray 形态)"""
    return cli.evaluate("""() => {
        ...
        rects.forEach((r, i) => {
            ...
            if (fill === '#808080' || fill === 'rgb(128, 128, 128)' || fill === '#fafafa' || fill === 'rgb(250, 250, 250)') gray++
            if (i < 3) samples.push(r.outerHTML.slice(0, 260))
        })
...
def main():
    ...
    output_dir = Path('test_helpers/scripts/_diag_bo_switch_out')
    ...
    with PlaywrightCLI(headless=True) as cli:
        ...
        warns = [l for l in console_msgs if 'updateColorsOnly' in l or 'updateNodeColors' in l or '[warn]' in l]
