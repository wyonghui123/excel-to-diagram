"""
复制 test_store_history.py 完整流程，验证 silent refresh 保持
"""
import sys
...
def main():
    ...
    try:
        ...
        """)
        print(f"  立即 (2s): {s_click}")

        # 6. 触发 silent refresh (切换 财务管理)
        print("\n[Step 5] 触发 silent refresh (toggle 财务管理)")
        cli.evaluate("""
            () => {
                const trees = document.querySelectorAll('.el-tree');
                const objectTree = trees[0];
        ...
        if last:
            print(f"\n[最终状态] checked={last['checked']}, indet={last['indet']}")
            if last['checked'] > 0:
        ...
        # 7. setData 触发的次数
        setdata_events = [h for h in history if h['event'] in ('before_setData', 'after_setData')]
        print(f"\n[setData 次数] {len(setdata_events)} (期望 0 触发 silent refresh 时)")
