...
try:
    ...
    page.evaluate("""() => {
        ...
        console.log = function(...a){ window.__log.push(a.map(x=>String(x)).join(' ')); orig.apply(console,a); };
        return true;
    }""")
    ...
    logs = page.evaluate("() => window.__log")
    diag = [l for l in logs if '[DIAG-layoutWatch]' in l]
    print("[DIAG-layoutWatch count]", len(diag))
    for l in diag:
        print("  ", l)
finally:
    cli.close()