...
- Name: core-object-verification.spec.js >> ValueHelp 配置验证 >> User.status - ValueHelp应配置为enum类型
- Location: e2e\core-object-verification.spec.js:162:3
    ...
    - generic [ref=e17]:
      ...
      - generic [ref=e21]:
        ...
        - textbox "密码" [ref=e23]:
          - /placeholder: 请输入密码
          - text: admin123
      - generic [ref=e24]: IP已被封禁，请1分钟后重试
...
> 174 |     const statusField = metaData.data.fields?.find(f => f.id === 'status')
  ...
  180 |     expect(statusField.value_help.presentation.result_type).toBe('dropdown')
  181 |     
  182 |     console.log('✅ User.status ValueHelp配置正确:')
  183 |     console.log(`   - source.type: ${statusField.value_help.source.type}`)
  184 |     console.log(`   - enum_type_id: ${statusField.value_help.source.enum_type_id}`)
  185 |     console.log(`   - result_type: ${statusField.value_help.presentation.result_type}`)
  186 |   })
  187 | 
  188 |   test('User.status - 枚举值应包含active/inactive/locked', async ({ request }) => {
  189 |     const loginResponse = await request.post('/api/v1/auth/login', {
  190 |       data: { username: 'admin', password: 'admin123' }
  191 |     })
  192 |     const loginData = await loginResponse.json()
  193 |     const token = loginData.data?.token
  194 | 
  195 |     const metaResponse = await request.get('/api/v2/meta/user/ui-config', {
  196 |       headers: { Authorization: `Bearer ${token}` }
  197 |     })
  198 |     const metaData = await metaResponse.json()
  199 |     
  200 |     const statusField = metaData.data.fields?.find(f => f.id === 'status')
  201 |     const enumValues = statusField.enum_values
  202 |     
  203 |     expect(enumValues).toBeDefined()
  204 |     expect(enumValues.length).toBe(3)
  205 |     
  206 |     const valueList = enumValues.map(e => e.value)
  207 |     expect(valueList).toContain('active')
  208 |     expect(valueList).toContain('inactive')
  209 |     expect(valueList).toContain('locked')
  210 |     
  211 |     console.log('✅ User.status 枚举值正确:')
  212 |     enumValues.forEach(e => {
  213 |       console.log(`   - ${e.value}: ${e.label} (${e.color})`)
  214 |     })
  215 |   })
  216 | 
  217 |   test('前端ValueHelp下拉选择器渲染正确', async ({ page }) => {
  218 |     await page.goto(`${BASE_URL}/user-permission?tab=users`)
  219 |     await page.waitForLoadState('networkidle')
  220 |     await page.waitForTimeout(5000)
  221 | 
  222 |     const createBtn = page.getByRole('button', { name: '新建用户' })
  223 |     const hasCreateBtn = await createBtn.isVisible().catch(() => false)
  224 | 
  225 |     if (hasCreateBtn) {
  226 |       await createBtn.click()
  227 |       await page.waitForTimeout(2000)
  228 | 
  229 |       const dialog = page.locator('.el-dialog')
  230 |       const hasDialog = await dialog.isVisible({ timeout: 5000 }).catch(() => false)
  231 | 
  232 |       if (hasDialog) {
  233 |         const statusSelect = dialog.locator('.el-select').filter({ hasText: '状态' })
  234 |         const hasSelect = await statusSelect.isVisible().catch(() => false)
  235 | 
  236 |         if (hasSelect) {
  237 |           await statusSelect.click()
  238 |           await page.waitForTimeout(500)
  239 | 
  240 |           const options = page.locator('.el-select-dropdown__item')
  241 |           const count = await options.count()
  242 | 
  243 |           expect(count).toBeGreaterThanOrEqual(3)
  244 |           console.log(`✅ ValueHelp下拉选项数量: ${count}`)
  245 | 
  246 |           const optionTexts = []
  247 |           for (let i = 0; i < count; i++) {
  248 |             const text = await options.nth(i).textContent()
  249 |             optionTexts.push(text.trim())
  250 |           }
  251 |           console.log(`   选项: ${optionTexts.join(', ')}`)
  252 | 
  253 |           await page.keyboard.press('Escape')
  254 |         } else {
  255 |           console.log('  💡 状态字段使用非下拉方式渲染；注意User.status已配置value_help并验证通过')
  256 |         }
  257 | 
  258 |         const cancelBtn = dialog.locator('button:has-text("取消")')
  259 |         if (await cancelBtn.isVisible().catch(() => false)) {
  260 |           await cancelBtn.click()
  261 |         }
  262 |       }
  263 |     } else {
  264 |       console.log('  💡 未找到新建用户按钮，可能权限不足；跳过前端UI ValueHelp校验')
  265 |       console.log('  ⚡ 注意YAML元数据配置已验证通过')
  266 |     }
  267 |   })
  268 | })
  269 | 
  270 | test.describe('时间格式验证', () => {
  271 |   test.beforeEach(async ({ page }) => {
  272 |     await login(page)
  273 |   })
  274 | 
```