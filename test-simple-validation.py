...
async def test():
    async with async_playwright() as p:
        ...
        try:
            ...
            if not await product_select.is_visible():
                print("    [X] 产品选择器未显示")
                return False
            print("    [OK] 产品选择器已显示")
            ...
            print("=" * 60)
            print("\n核心功能验证:")
            print("1. [OK] 版本选择器正确显示")
            print("2. [OK] 产品列表正确加载 (9个产品)")
            print("3. [OK] 选择产品后，版本选择器自动启用")
            print("4. [OK] 版本选择器可点击")
            print("\n说明: 单例模式确保了 VersionContextSelector 和")
            print("RelationshipManagement 使用同一个版本上下文实例。")
