#!/usr/bin/env bash
# ============================================================
# ONE-SHOT DEPLOY SCRIPT - v20260630_002 (Fresh Init)
# Run on server: bash /tmp/deploy-full-v20260630_002.sh
#
# ROBUST v2:
...
export PYTHONPATH="${DEPLOY_DIR}:${DEPLOY_DIR}/meta"
...
# Now backend/ exists; create meta/ as alias (init scripts do "from meta.xxx")
if [[ -d "tmp_extract/backend" ]]; then
    cp -r tmp_extract/backend tmp_extract/meta
    info "Created tmp_extract/meta/ as alias of tmp_extract/backend/"
fi
...
if [[ -d "tmp_extract/meta" ]] && [[ -d "tmp_extract/backend" ]]; then
    # Case A: merge wheels into meta, then rename all to backend, then re-create meta
    ...
    # [fix] Re-create meta/ as alias of backend/ (init scripts do "from meta.xxx")
    cp -r tmp_extract/backend tmp_extract/meta
    info "Re-created tmp_extract/meta/ as alias of tmp_extract/backend/"
elif [[ -d "tmp_extract/meta" ]] && [[ ! -d "tmp_extract/backend" ]]; then
    cp -r tmp_extract/meta tmp_extract/backend
    cp -r tmp_extract/backend tmp_extract/meta
fi
...
[[ -d "tmp_extract/meta" ]] && cp -r "$DEPLOY_DIR/backend" "$DEPLOY_DIR/meta" && info "Created $DEPLOY_DIR/meta as alias of backend/"
...
WHEELS_DIR="${DEPLOY_DIR}/backend/wheels"
...
for script in "${SCRIPTS[@]}"; do
    ...
    if [[ ! -f "scripts/$script" ]]; then
        ...
        log_line "  $script: SKIPPED (not found)"
        continue
    fi
    info "Running: $script"
...
cd "${DEPLOY_DIR}/backend" 2>/dev/null || cd "${DEPLOY_DIR}" || die "Cannot cd to $DEPLOY_DIR"
