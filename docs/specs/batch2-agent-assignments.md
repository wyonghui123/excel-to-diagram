...
1. [〇、前置状态（2026-06-07）](#〇-前置状态（2026-06-07）)
2. [〇.B、Agent A 战报（5 任务已落地）](#〇b-agent-a-战报（5-任务已落地）)
3. [一、4 Agent 分配（FR-3 拆分为 B+C）](#一-4-agent-分配（fr-3-拆分为-bc）)
4. [二、Agent 串行任务链](#二-agent-串行任务链)
5. [三、启动模板（3 个 Agent 通用）](#三-启动模板（3-个-agent-通用）)
6. [四、每个 Agent 详细任务卡](#四-每个-agent-详细任务卡)
7. [五、合并顺序（4 Agent 串行 → main）](#五-合并顺序（4-agent-串行-main）)
8. [六、关键风险与缓解（v1.1 更新）](#六-关键风险与缓解（v11-更新）)
9. [七、状态检查命令（PM 视角，单仓库版）](#七-状态检查命令（pm-视角，单仓库版）)
10. [八、关键检查清单（Agent 复制）](#八-关键检查清单（agent-复制）)
11. [九、附录](#九-附录)
