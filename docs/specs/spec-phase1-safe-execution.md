...
1. [0. "安全执行"的 5 维定义](#0-"安全执行"的-5-维定义)
2. [1. 全局铁律（不可违反）](#1-全局铁律（不可违反）)
3. [2. 实施前预检（每个 PR 开始前必做）](#2-实施前预检（每个-pr-开始前必做）)
4. [3. v3 各 Phase 实施安全清单](#3-v3-各-phase-实施安全清单)
5. [4. 端到端验证矩阵](#4-端到端验证矩阵)
6. [5. 异常诊断流程](#5-异常诊断流程)
7. [6. 紧急联系点](#6-紧急联系点)
8. [7. 提交前检查清单](#7-提交前检查清单)
9. [8. 红线（绝对不能做）](#8-红线（绝对不能做）)
10. [9. 安全执行的口诀](#9-安全执行的口诀)
11. [10. 变更记录](#10-变更记录)
