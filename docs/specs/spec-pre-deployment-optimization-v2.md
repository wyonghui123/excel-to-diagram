...
1. [总览](#总览)
2. [FR-1: 自动 DDL 集成（不变）](#fr-1-自动-ddl-集成（不变）)
3. [FR-2: OpenAPI 规范自动生成（不变）](#fr-2-openapi-规范自动生成（不变）)
4. [FR-3: 格式化值服务端返回（不变）](#fr-3-格式化值服务端返回（不变）)
5. [FR-4: 条件必填（不变）](#fr-4-条件必填（不变）)
6. [FR-5: server.py 与 AppBuilder 统一（不变）](#fr-5-serverpy-与-appbuilder-统一（不变）)
7. [FR-6: UI 完整适配（新增）](#fr-6-ui-完整适配（新增）)
8. [实施顺序（更新）](#实施顺序（更新）)
9. [验证总清单](#验证总清单)
10. [工作量统计](#工作量统计)
