...
1. [方案 1：WAL Checkpoint 改 TRUNCATE 模式](#方案-1：wal-checkpoint-改-truncate-模式)
2. [方案 2：Migration 事务原子化](#方案-2：migration-事务原子化)
3. [方案 3：写操作串行化（`BEGIN IMMEDIATE`）](#方案-3：写操作串行化（begin-immediate）)
4. [三方案联合部署时间线](#三方案联合部署时间线)
5. [监控指标](#监控指标)
6. [风险与回退](#风险与回退)
