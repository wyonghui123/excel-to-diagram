...
1. [一、问题溯源：今天修复的三个 Bug](#一-问题溯源：今天修复的三个-bug)
2. [二、数据管线全景图](#二-数据管线全景图)
3. [三、颜色系统：五套定义、四种 groupKey 策略](#三-颜色系统：五套定义-四种-groupkey-策略)
4. [四、isCenter 判断逻辑不一致](#四-iscenter-判断逻辑不一致)
5. [五、nodeColorMappings 结构差异](#五-nodecolormappings-结构差异)
6. [六、配置组件差异](#六-配置组件差异)
7. [七、统计系统数据模型](#七-统计系统数据模型)
8. [八、统一方向思考](#八-统一方向思考)
9. [九、与 unified-model-refactor-plan 的关系](#九-与-unified-model-refactor-plan-的关系)
10. [十、补充发现：历史分析中的深层问题](#十-补充发现：历史分析中的深层问题)
11. [十一、附录：关键代码位置索引](#十一-附录：关键代码位置索引)
